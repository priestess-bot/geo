...
class InfraContractsTest(unittest.TestCase):
    ...
    def test_default_stack_runs_incremental_db_migrations_before_api(self) -> None:
        ...
        self.assertNotIn("postgres db-migrate minio api customer-web admin-web dashboard-web", makefile)
    ...
    def test_report_export_jobs_migration_is_covered_by_rls_and_db_smoke(self) -> None:
        ...
        self.assertIn("CREATE TABLE runtime_notification_email_feedback_events", init_sql)
        self.assertIn("recipient_hash text", init_sql)
        self.assertIn("provider_event_id_hash text", init_sql)
        self.assertIn("idx_runtime_notification_email_feedback_project", init_sql)
        self.assertIn("idx_runtime_notification_email_feedback_delivery", init_sql)
        self.assertIn("CREATE TABLE runtime_notification_email_suppressions", init_sql)
        self.assertIn("UNIQUE(project_id, recipient_hash)", init_sql)
        self.assertIn("idx_runtime_notification_email_suppressions_project", init_sql)
        self.assertIn("idx_runtime_notification_email_suppressions_hash", init_sql)
        self.assertIn("'runtime_notification_subscriptions'", rls_sql)
        self.assertIn("'runtime_notification_deliveries'", rls_sql)
        self.assertIn("'runtime_notification_email_feedback_events'", rls_sql)
        self.assertIn("'runtime_notification_email_suppressions'", rls_sql)
        self.assertIn("'runtime_notification_subscriptions'", rls_down_sql)
        self.assertIn("'runtime_notification_deliveries'", rls_down_sql)
        self.assertIn("'runtime_notification_email_suppressions'", rls_down_sql)
        self.assertIn('"runtime_notification_email_suppressions"', db_smoke_source)
        self.assertIn("'runtime_notification_email_feedback_events'", rls_down_sql)
        ...
        self.assertIn('"runtime_notification_email_feedback_events"', db_smoke_source)
    ...
    def test_browser_fidelity_plan_make_target_outputs_machine_readable_json(self) -> None:
        ...
        self.assertNotIn("npm --prefix apps/dashboard-web run typecheck", makefile)
        ...
        self.assertNotIn("npm --prefix apps/dashboard-web run build", makefile)
        ...
        self.assertIn("$(MAKE) au-next-work-item", makefile)
        self.assertIn("$(MAKE) au-delivery-progress", makefile)
    ...
    def test_github_ci_runs_runtime_contract_build_compose_and_e2e_gates(self) -> None:
        ...
        self.assertNotIn("npm --prefix apps/dashboard-web ci", install_web_dependencies)
