...
class FakeRepository:
    ...
    def create_destination(self, **values: Any) -> Destination:
        item = Destination(id=uuid4(), policy_status="approved", **values)
    ...
    def create_opportunities(self, **values: Any) -> tuple[Opportunity, ...]:
        created = tuple(
            Opportunity(
                id=uuid4(), project_id=values["project_id"],
                campaign_id=values["campaign_id"], destination_id=destination_id,
                opportunity_ref=f"destination:{destination_id}", rationale=values["rationale"],
            )
    ...
    def get_campaign(self, *, project_id: UUID, campaign_id: UUID) -> Campaign | None:
        return next(
            (item for item in self.campaigns if item.project_id == project_id and item.id == campaign_id),
            None,
        )
    ...
    def create_brief_version(self, **values: Any) -> BriefVersion:
        item = BriefVersion(
            id=uuid4(), project_id=values["project_id"], brief_id=uuid4(),
            version_number=1, goals=values["goals"], constraints=values["constraints"],
            content_hash=values["content_hash"], base_version_id=values["base_version_id"],
        )
    ...
    def create_evidence_attempt(self, **values: Any) -> tuple[EvidencePackAttempt, JobReference]:
        attempt = EvidencePackAttempt(
            id=uuid4(), project_id=values["project_id"],
            brief_version_id=values["brief_version_id"], attempt_number=len(self.attempts) + 1,
        )
    ...
    def create_skill_version(self, **values: Any) -> SkillVersion:
        version = 1 + sum(item.skill_version_id == values["skill_id"] for item in self.releases.values())
    ...
    def create_template_release(self, **values: Any) -> PromptReleaseView:
        ...
        return PromptReleaseView(
            template.id, values["project_id"], values["skill_version_id"],
            len(self.releases), template.release_hash,
        )
    ...
    def create_prompt_bundle(self, **values: Any) -> PromptBundleView:
        ...
        assert set(release.required_variables) <= set(values["variables"])
        bundle_id = uuid4()
        ...
        return PromptBundleView(
            ...
            values["evidence_pack_attempt_id"], release.id, bundle_hash,
    ...
    def get_package_version(self, *, project_id: UUID, version_id: UUID) -> PackageVersion | None:
        return next(
            (item for item in self.packages if item.project_id == project_id and item.id == version_id),
            None,
        )
    ...
    def save_edited_version(self, **values: Any) -> PackageVersion:
        ...
        self.packages[self.packages.index(old)] = replace(old, workflow_status=WorkflowStatus.SUPERSEDED)
        self.packages.append(values["version"])
        return values["version"]

    def list_claims(self, **values: Any) -> tuple[Claim, ...]:
        return tuple(item for item in self.claims if item.package_version_id == values["version_id"])

    def save_review(self, *, review: Review) -> Review:
        self.reviews.append(review)
        index = next(i for i, item in enumerate(self.packages) if item.id == review.package_version_id)
        self.packages[index] = replace(self.packages[index], workflow_status=WorkflowStatus(review.decision))
        return review
    ...
    def export_package(self, **values: Any) -> ExportReceipt:
        ...
        receipt = ExportReceipt(version.id, version.content_hash, values["exported_at"])
    ...
    def create_publication_request(self, **values: Any) -> PublicationRequest:
        ...
        destination = next(item for item in self.destinations if item.id == values["destination_id"])
        item = PublicationRequest(
            id=uuid4(), project_id=values["project_id"], package_version_id=version.id,
            destination_id=destination.id, publication_channel=destination.publication_channel,
    ...
    def create_submission(self, **values: Any) -> Submission:
        item = Submission(
            uuid4(), values["project_id"], values["publication_request_id"],
            "submitted" if values["submitted_url"] else "awaiting_url",
            values["submitted_url"], values["provider_submission_id"],
        )
    ...
    def list_measurements(self, **values: Any) -> tuple[Measurement, ...]:
        return tuple(item for item in self.measurements if item.submission_id == values["submission_id"])
...
def test_full_application_chain_keeps_export_and_publication_separate() -> None:
    ...
    destinations = [
        app.create_destination(
            project_id=project_id, publication_channel=channel,
            destination_key=f"account:{channel}", operation_mode="manual",
            destination_account_id=None, canonical_url=None,
        )
        for channel in ("reddit", "youtube", "productreview")
    ]
    campaign, opportunities = app.create_campaign(
        project_id=project_id, market_profile_id=uuid4(),
        primary_product_entity_id=uuid4(), name="Robot vacuum recommendations",
        objective="recommendation_influence", actor_id=actor_id,
        destination_ids=tuple(item.id for item in destinations), rationale="Audience fit",
    )
    assert {item.destination_id for item in opportunities} == {item.id for item in destinations}

    brief = app.create_brief_version(
        project_id=project_id, opportunity_id=opportunities[0].id,
        primary_brand_entity_id=uuid4(), goals={"query": "best robot vacuum"}, constraints={},
        compared_entity_ids=(), allowed_subject_entity_ids=(), actor_id=actor_id,
        base_version_id=None,
        consumer_experience=ConsumerExperience(
            "It cleaned a two-bedroom home daily.", "customer supplied note",
            "authorised_experience", "Customer wording was edited for clarity.",
        ),
        authenticity_risks=(),
    )
    attempt, _ = app.create_evidence_attempt(
        project_id=project_id, brief_version_id=brief.id,
    ...
    release = app.publish_skill_version(
        ...
        source="Write for {{channel}} using {{evidence}}.", actor_id=actor_id,
        output_schema={"type": "object"},
    )
    bundle = app.create_prompt_bundle(
        project_id=project_id, brief_version_id=brief.id,
        evidence_pack_attempt_id=attempt.id, release_id=release.id,
        variables={"channel": "reddit", "evidence": "pack-1"}, model_policy_hash="f" * 64,
    )
    job = app.request_generation(
        project_id=project_id, prompt_bundle_id=bundle.id, configured_model="deepseek-chat",
        model_call_budget=2, idempotency_key="generation-job-0001",
    )
    assert job.status == "queued"

    version = PackageVersion(
        uuid4(), project_id, uuid4(), bundle.id, 1, {"title": "A practical review"},
        "A practical review", "a" * 64, WorkflowStatus.PENDING_HUMAN_REVIEW,
    )
    repository.packages.append(version)
    repository.claims.append(
        Claim(uuid4(), project_id, version.id, "Daily cleaning was tested.", "factual", "supported", (uuid4(),))
    )
    reviewer_id = uuid4()
    app.submit_review(
        review=Review(
            uuid4(), project_id, version.id, actor_id, reviewer_id, "approved", True, True,
        )
    )
    app.export_package(project_id=project_id, version_id=version.id)
    assert repository.publications == []
    publication = app.request_publication(
        project_id=project_id, version_id=version.id, destination_id=destinations[0].id,
        requested_by=actor_id, publication_attempt=1,
    ...
    measurement = app.record_measurement(
        project_id=project_id, submission_id=submission.id, monitoring_query_id=uuid4(),
        measured_at=datetime.now(UTC), citation_present=True, recommendation_position=2,
        result_snapshot_uri="snapshots/result.json", metrics={"share_of_voice": 0.3},
    )
...
def test_prompt_releases_are_independent_and_old_release_is_unchanged() -> None:
    ...
    first = app.publish_skill_version(
        project_id=project_id, skill_id=skill.id, source="Write {{product}}.",
        actor_id=actor_id, output_schema={},
    )
    first_template = repository.releases[first.id]
    second = app.publish_skill_version(
        project_id=project_id, skill_id=skill.id, source="Compare {{product}} using {{evidence}}.",
        actor_id=actor_id, output_schema={},
    )
    assert first.release_hash != second.release_hash
    assert repository.releases[first.id] is first_template
    assert first_template.template == "Write {{product}}."
...
def test_edit_uses_exact_base_hash_and_new_version_invalidates_old_review_lineage() -> None:
    ...
    with pytest.raises(ConcurrencyConflict):
        app.edit_package_version(
            project_id=project_id, package_id=base.package_id,
            base_version_id=base.id, base_content_hash="0" * 64,
            content_json={"body": "new"}, rendered_text="new", edited_by=uuid4(), reason="Fix",
        )
    edited = app.edit_package_version(
        project_id=project_id, package_id=base.package_id,
        base_version_id=base.id, base_content_hash=base.content_hash,
        content_json={"body": "new"}, rendered_text="new", edited_by=uuid4(), reason="Fix",
    )
...
def test_authenticity_hard_blocks_cannot_be_overridden() -> None:
    ...
    with pytest.raises(PlacementRuleViolation, match="synthetic_testimonial"):
        app.create_brief_version(
            project_id=uuid4(), opportunity_id=uuid4(), primary_brand_entity_id=uuid4(),
            goals={}, constraints={}, compared_entity_ids=(), allowed_subject_entity_ids=(),
            actor_id=uuid4(), base_version_id=None, consumer_experience=None,
...
def test_approval_requires_claim_inventory_and_support_confirmation() -> None:
    ...
    with pytest.raises(PlacementRuleViolation, match="both claim review gates"):
        Review(
            ids[0], ids[1], ids[2], ids[3], ids[4], "approved",
            claim_inventory_complete=True, extracted_claim_support_confirmed=False,
        )
    app, repository = _application()
    version = PackageVersion(
        uuid4(), ids[1], uuid4(), uuid4(), 1, {"body": "claim"}, "claim", "c" * 64,
    ...
    with pytest.raises(PlacementRuleViolation, match="factual claims"):
        app.submit_review(
            review=Review(uuid4(), ids[1], version.id, ids[3], ids[4], "approved", True, True)
        )
