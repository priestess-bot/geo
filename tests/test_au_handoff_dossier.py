...
class AuHandoffDossierTest(unittest.TestCase):
    ...
    def test_dossier_records_blocked_handoff_with_mapped_work_items(self) -> None:
        ...
        self.assertFalse(dossier["p0a_execution_checklist"]["credential_handoff_ready"])
        self.assertEqual(dossier["p0a_execution_checklist"]["credential_handoff_missing_required_count"], 3)
        self.assertEqual(
            sorted(dossier["p0a_execution_checklist"]["credential_handoff_missing_required"]),
            ["DATABASE_URL", "OPENAI_API_KEY", "PERPLEXITY_API_KEY"],
        )
        self.assertTrue(dossier["p0a_execution_checklist"]["credential_handoff_secret_redacted"])
    ...
    def test_verifier_detects_p0a_credential_handoff_summary_tampering(self) -> None:
        with TemporaryDirectory() as temp_dir:
            launch_status_path, remediation_plan_path = self._write_launch_status_and_plan(temp_dir, ready=False)
            checklist_path = self._write_p0a_environment_checklist(temp_dir)
            p0a_execution_checklist_path = self._write_p0a_execution_checklist(temp_dir)
            ...
            dossier["summary"]["p0a_credential_handoff_missing_required_count"] = 0  # type: ignore[index]
            dossier["handoff_dossier_hash"] = compute_handoff_dossier_hash(dossier)
            verification = verify_au_handoff_dossier(dossier)
