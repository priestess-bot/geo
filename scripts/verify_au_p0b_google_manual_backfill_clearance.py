...
REQUIRED_FIELDS = (
    "p0b_google_manual_backfill_clearance_version",
    "generated_at",
    "status",
    "manual_backfill_clearance_packet_ready",
    ...
    "p0b_google_manual_backfill_fulfillment_verifier",
    "summary",
    "manual_backfill_clearance_items",
    "operator_steps",
...
def verify_au_p0b_google_manual_backfill_clearance(
    ...
    *,
    ...
    if not isinstance(payload, dict):
        return {
            "status": "fail",
            "errors": ["p0b_google_manual_backfill_clearance_not_json_object"],
            "hash_valid": False,
            "manual_backfill_clearance_packet_ready": False,
            "manual_backfill_clearance_ready": False,
    ...
    if payload.get("p0b_google_manual_backfill_clearance_version") != CLEARANCE_VERSION:
        errors.append("p0b_google_manual_backfill_clearance_version_invalid")
    for forbidden_path in _find_forbidden_fields(payload):
        errors.append(f"forbidden_manual_payload_field:{forbidden_path}")

    expected_hash = payload.get("p0b_google_manual_backfill_clearance_hash")
    computed_hash = compute_p0b_google_manual_backfill_clearance_hash(payload)
    hash_valid = isinstance(expected_hash, str) and expected_hash == computed_hash
    if not hash_valid:
        errors.append("p0b_google_manual_backfill_clearance_hash_mismatch")
    ...
    fulfillment_verifier = _as_dict(payload.get("p0b_google_manual_backfill_fulfillment_verifier"))
    source_artifacts = _as_dict(payload.get("source_artifacts"))
    request_source = _as_dict(source_artifacts.get("manual_backfill_request"))
    verification_source = _as_dict(source_artifacts.get("manual_backfill_verification"))
    fulfillment_source = _as_dict(source_artifacts.get("manual_backfill_fulfillment"))
    ...
    items = [_as_dict(item) for item in _as_list(payload.get("manual_backfill_clearance_items"))]
    ...
    expected_packet_ready = request_ok and verification_ok and fulfillment_ok and clearance_ok
    if payload.get("manual_backfill_clearance_packet_ready") is not expected_packet_ready:
        errors.append("manual_backfill_clearance_packet_ready_mismatch")
    ...
    expected_next_ready = manual_backfill_fulfilled and not blocked_by_prerequisite
    expected_clearance_ready = (
        manual_backfill_fulfilled and clearance_step.get("step_ready") is True and not blocked_by_prerequisite
    )
    ...
    if payload.get("manual_backfill_clearance_ready") is not expected_clearance_ready:
        errors.append("manual_backfill_clearance_ready_mismatch")
    if require_cleared and not expected_clearance_ready:
        errors.append("p0b_google_manual_backfill_not_cleared")
    ...
    if request_source.get("hash") != request_verifier.get("p0b_google_manual_backfill_request_packet_hash"):
        errors.append("source_manual_backfill_request_hash_mismatch")
    if verification_source.get("hash") != verification_verifier.get("verification_hash"):
        errors.append("source_manual_backfill_verification_hash_mismatch")
    if fulfillment_source.get("hash") != fulfillment_verifier.get("p0b_google_manual_backfill_fulfillment_hash"):
        errors.append("source_manual_backfill_fulfillment_hash_mismatch")
    expected_hash_fields = {
        "manual_backfill_request": "p0b_google_manual_backfill_request_packet_hash",
        "manual_backfill_verification": "verification_hash",
        "manual_backfill_fulfillment": "p0b_google_manual_backfill_fulfillment_hash",
    ...
    if current_file_check_enabled:
        _check_current_source_file(
            errors,
            source_key="source_manual_backfill_request",
        ...
        _check_current_source_file(
            errors,
            source_key="source_manual_backfill_verification",
            source=verification_source,
            hash_field="verification_hash",
            expected_hash=str(verification_source.get("hash") or ""),
        )
        _check_current_source_file(
            errors,
            source_key="source_manual_backfill_fulfillment",
    ...
    for key in required_item_keys:
        if key not in item_keys:
            errors.append(f"manual_backfill_clearance_item_missing:{key}")
    ...
    if summary.get("manual_backfill_fulfillment_ready") is not (
        fulfillment_verifier.get("manual_backfill_fulfillment_ready") is True
    ):
        errors.append("summary_manual_backfill_fulfillment_ready_mismatch")
    ...
    if summary.get("manual_jsonl_raw_path_allowed") is not False:
        errors.append("summary_manual_jsonl_path_policy_invalid")

    required_step_ids = {
        "clear_p0b_google_environment",
        "refresh_manual_backfill_request",
        "build_manual_backfill_template",
        "verify_manual_jsonl",
        "refresh_manual_backfill_fulfillment",
        "apply_current_manual_backfill_fix",
        "verify_manual_backfill_fulfillment",
        "run_strict_gate",
    ...
    for item in items:
        ...
        for field in (
            ...
            if field not in item:
                errors.append(f"manual_backfill_clearance_item_field_missing:{key}:{field}")
        if item.get("fulfilled") is True and _as_list(item.get("blocking_reasons")):
            errors.append(f"manual_backfill_clearance_item_blocking_reasons_on_fulfilled:{key}")
        if item.get("presence_mismatch") is not False:
            errors.append(f"manual_backfill_clearance_item_presence_mismatch_invalid:{key}")
    ...
    if not any("--require-manual-backfill-ready" in command for command in validation_sequence):
        errors.append("post_update_validation_missing_require_manual_backfill_ready")
    ...
    if redaction_policy.get("manual_jsonl_raw_path_allowed") is not False:
        errors.append("redaction_policy_manual_raw_path_invalid")
    if redaction_policy.get("manual_records_reference_counts_hashes_and_error_codes_only") is not True:
        errors.append("redaction_policy_manual_record_reference_invalid")

    return {
        "status": "pass" if not errors else "fail",
        "errors": errors,
        "path": str(path) if path else "",
        "p0b_google_manual_backfill_clearance_version": payload.get(
            "p0b_google_manual_backfill_clearance_version",
            "",
        ),
        "p0b_google_manual_backfill_clearance_hash": expected_hash if isinstance(expected_hash, str) else "",
        "computed_p0b_google_manual_backfill_clearance_hash": computed_hash,
        "hash_valid": hash_valid,
        "current_file_check_enabled": current_file_check_enabled,
        "manual_backfill_clearance_packet_ready": expected_packet_ready,
        "manual_backfill_clearance_ready": expected_clearance_ready,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify an AU P0b Google manual backfill clearance JSON")
    parser.add_argument(
        "path",
        nargs="?",
    ...
    parser.add_argument("--require-cleared", action="store_true", help="Fail unless P0b Google manual backfill is cleared.")
...
def main() -> None:
    ...
    try:
        ...
        result = {
            "status": "fail",
            "path": str(path),
            "errors": ["p0b_google_manual_backfill_clearance_file_missing"],
            "hash_valid": False,
            "manual_backfill_clearance_packet_ready": False,
            "manual_backfill_clearance_ready": False,
        }
        ...
        result = {
            "status": "fail",
            "path": str(path),
            "errors": [f"p0b_google_manual_backfill_clearance_json_invalid:{exc.msg}"],
            "hash_valid": False,
            "manual_backfill_clearance_packet_ready": False,
            "manual_backfill_clearance_ready": False,
        }
    else:
        result = verify_au_p0b_google_manual_backfill_clearance(payload, path=path, require_cleared=args.require_cleared)
