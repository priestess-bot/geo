...
from scripts.verify_au_p0b_google_env_template import verify_au_p0b_google_env_template  # noqa: E402


BOOTSTRAP_VERSION = "au_p0b_google_env_file_bootstrap_v1"
DEFAULT_TEMPLATE_PATH = ".env.au-p0b-google.example"
DEFAULT_ENV_FILE = ".env.au-p0b-google"
DEFAULT_OUTPUT_PATH = "docs/runtime_preflight/au-p0b-google-env-bootstrap-latest.json"
...
def bootstrap_au_p0b_google_env_file(
    *,
    ...
    template_verification = verify_au_p0b_google_env_template(template_path=template_path, generated_at=generated_at)
    errors: list[str] = []
    warnings: list[str] = []
    ...
    report: dict[str, Any] = {
        ...
        "summary": {
            ...
            "next_action": "fill_google_selectors_session_manual_path_and_database_url"
            if not errors
            else "fix_env_file_bootstrap",
        },
        "next_commands": [
            "edit .env.au-p0b-google",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Bootstrap the local AU P0b Google env file from the committed template")
    parser.add_argument(
        "--template-path",
        default=os.environ.get("GENO_AU_P0B_GOOGLE_ENV_TEMPLATE_PATH", DEFAULT_TEMPLATE_PATH),
        help="Path to the committed AU P0b Google env template.",
    )
    parser.add_argument(
        "--env-file",
        default=os.environ.get("GENO_AU_P0B_GOOGLE_ENV_FILE", DEFAULT_ENV_FILE),
        help="Path to the local AU P0b Google env file.",
    )
    parser.add_argument(
        "--output-path",
        default=os.environ.get("GENO_AU_P0B_GOOGLE_ENV_BOOTSTRAP_OUTPUT_PATH", DEFAULT_OUTPUT_PATH),
