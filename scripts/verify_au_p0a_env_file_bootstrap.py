...
from scripts.bootstrap_au_p0a_env_file import (  # noqa: E402
    BOOTSTRAP_VERSION,
    DEFAULT_OUTPUT_PATH,
...
def _find_forbidden_secret_fields(value: object, *, prefix: str = "") -> list[str]:
    ...
    if isinstance(value, dict):
        for key, child in value.items():
            ...
            if key in {"value", "raw_value", "secret", "api_key", "database_url"}:
...
def verify_au_p0a_env_file_bootstrap(
    payload: Any,
    *,
    path: Path | None = None,
    ...
    if "make verify-au-p0a-env-bootstrap" not in [str(item) for item in _as_list(payload.get("verification_commands"))]:
        errors.append("verification_command_missing:make verify-au-p0a-env-bootstrap")
    if "make au-p0a-env" not in [str(item) for item in _as_list(payload.get("next_commands"))]:
        errors.append("next_command_missing:make au-p0a-env")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify an AU P0a env-file bootstrap audit JSON")
    parser.add_argument(
        "path",
        nargs="?",
...
def main() -> None:
    ...
    try:
        ...
        result = verify_au_p0a_env_file_bootstrap(payload, path=path, require_ready=args.require_ready)
