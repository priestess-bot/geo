...
- `[~]` (P1) 预警：负面/品牌缺失/竞品压制 — `E9-04/05/06`（Runtime Alerts 只读派生模型已落：`GET /v1/runtime-alerts` 从最新 `VisibilityScoreSnapshot`、`ScoreContribution`、`SourceGap`、`CompetitorBenchmark` 和 `ActionRecommendation` 生成 `brand_absent`、`low_recommendation_rate`、`source_gap`、`competitor_pressure`，返回 severity/threshold/metric/evidence refs/related actions/audit refs；Runtime Console 已展示 Runtime Alerts；负面情绪专用规则、实时推送、通知、订阅、SLA 和 Temporal 工作流待接）
