...
- `[~]` 通过 CI（lint + 测试 + 迁移可起）（GitHub Actions `contracts-and-runtime` 与本地 `make ci-local` 已覆盖 `make test`、`make web-build`、默认/LLM/scheduler/observability Compose config 和 `make runtime-e2e`；完整 lint/真实外部 provider E2E 待接）
...
- `[~]` (P0a) 空 CI：lint + 测试 + 迁移起服（GitHub Actions `contracts-and-runtime` 与本地 `make ci-local` 已覆盖 contract/runtime tests、Runtime Console build、默认/LLM/scheduler/observability Compose config 和 Postgres+MinIO runtime E2E；lint、真实外部 provider E2E 与生产迁移起服待补）
...
- `[~]` 一键起核心依赖；P0a/P0b/P0c 相关表可建可回滚；8 接口 stub + CI 绿（本地配置、repository runtime mapping、AU 启动包持久化、prompt-linked runtime evidence、runtime collection run summary、runtime score/runtime citation graph/runtime report/runtime traceability read model、Docker 写库验证、GitHub Actions runtime gate 和 `make runtime-e2e` Postgres+MinIO artifact/snapshot 归档验证已完成；lint、真实外部 provider E2E 与生产迁移起服待补）
