...
- `[~]` 可创建 `market=AU` 项目，配 1 行业模板 + 100 条澳洲问题集 + 3–5 竞品（可配置客户项目 API/控制台已落；`GENO_RUNTIME_PROJECT_ACCESS_CONTROL=1` 时已按 `X-GENO-Actor-Id + project_members` 做 API 级项目成员门禁，并接入 owner/admin/analyst/viewer 最小角色矩阵、成员删除审计和最后 owner 保护；DB RLS/JWT/Keycloak、邀请邮件、真实用户目录和真实客户数据验收待接）
