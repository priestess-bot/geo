...
- Webhook 签名补充：`make notification-delivery-worker` 已支持出站 HMAC 签名。订阅 metadata 只保存 `signing_secret_env` / `webhook_signing_secret_env` 环境变量名，worker 从订阅指定 env 或默认 `GENO_NOTIFICATION_WEBHOOK_SIGNING_SECRET` 读取 secret，不把 raw secret 写入数据库；签名覆盖 `timestamp.delivery_id.notification_id.payload_sha256`，请求携带 `x-geno-signature`、`x-geno-signature-timestamp`、`x-geno-signature-version` 和 `x-geno-signature-input`。订阅指定 env 缺失时 delivery 会按现有 retry/dead-letter 状态机处理并写入审计；secret rotation、双 secret 轮换窗口、接收端验证 SDK/服务、订阅鉴权回调、邮件/Slack 原生渠道仍待接。
