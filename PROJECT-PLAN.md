...
- `[~]` (P1) 代理商工作流：多客户/多项目/白标/导出历史 — `Step13`（当前已接入 Runtime Console 项目下拉/URL `project_id` 选择，并把 brand kit/prompt/evidence/export/alias/saved view/score/graph/report/action/content/traceability read path 收敛到选中项目；已接入客户项目创建表单、Project Members 成员读取/upsert/delete 表单、项目级 Brand Kit 表单、Logo Upload raw-body 上传、Brand Assets 审计型版本列表、历史 Logo 激活、Theme Editor 白标预览、Report History/导出历史面板和 `template=white_label` PDF 下载，按 `project_id` 读取最近 5 个 `ReportExport` 并展示冻结 URL、artifact 下载、白标 artifact path、signed URL 生成入口和报告审计摘要；Report History 已可对冻结报告追加 `internal_review/client_ready/archived` 管理事件，写入 `report_export_management_recorded` 审计但不更新 `report_exports`；报告导出任务已用 `report_export_jobs` 落最小持久化队列，Runtime Console `Report Export Queue` 可排队 Markdown/CSV/PDF/white-label PDF 导出、查看最近任务、推进 `queued/running/succeeded/failed/dead_letter/cancelled` 状态，并展示 attempt/lease/next attempt、queued/running/dead_letter、可重试、过期 running 和最早排队时间；`make report-export-worker` 已提供单次执行 worker，用 `FOR UPDATE SKIP LOCKED` 领取可执行 queued job 或 lease 过期 running job，渲染 runtime report artifact，配置对象存储时归档到 `s3://.../report-artifacts/...`，未配置对象存储时写 `runtime-report-artifact://...` fallback URL，失败按 retry backoff 重新排队，达到 max attempts 后进入 `dead_letter`；`/artifact/signed-url` 已可在配置 `GENO_REPORT_ARTIFACT_SIGNING_SECRET` 后生成带 `expires_at/signature` 的 HMAC 下载 URL，保护 report/type/template/filter/sort/actor 参数不被篡改；Logo 上传会先写对象存储 `brand-assets/<project_id>/...`，再把 `s3://...` 写回 Brand Kit 并记录 filename/content type/content hash/logo URI 审计；Brand Assets 从 `project_brand_logo_uploaded` / `project_brand_logo_version_activated` 审计事件读回 Logo 版本，激活历史 URI 时只更新 `project_brand_kits.logo_url` 并追加审计；Theme Editor 使用 `client_name/prepared_by/logo_url/primary_color/secondary_color/footer_text` 预览报告头、指标条、页脚、white-label artifact path 和最近 `project_brand_kit_saved/project_brand_logo_uploaded` 审计；API 级 project_members 门禁、header/HS256 JWT/inline、remote 或 OIDC-discovered JWKS RS256 actor 解析与显式 stale-if-error 兜底、owner/admin/analyst/viewer 最小角色矩阵、成员删除审计和最后 owner 保护已可通过环境变量开启，覆盖项目列表、项目创建、成员管理、主要项目级读写接口、管理写操作、分析复核写操作、report artifact/report management event/report export job/traceability/alias confirm 对象反查校验；PostgreSQL RLS/GUC 最小项目隔离、`geno_runtime_app` runtime role 和 API connection pool 已落；Temporal/长驻 worker、并发池、通知系统、完整素材库、图片裁剪/预览、对象存储原生版本、CDN 签名、Keycloak/OIDC login/session、前端 SSO 登录流、成员邀请、账单隔离、生产压测与客户级授权流转待接）

DoD：

- `[~]` 可导出含方法说明和证据附录的 Markdown/CSV/PDF，并通过 runtime report API 读取冻结快照和项目内报告历史、通过 artifact API 下载 Markdown/CSV/PDF/White-label PDF；artifact API 支持按当前 `project_id/platform/city/intent_type/status/sort` 即时过滤/排序证据附录并返回 hash、template hash 与 row count，不改写冻结报告；白标 PDF 可从项目级 Brand Kit 默认读取客户名、服务商名、Logo URL、主题色和页脚；worker 可把 Markdown/CSV/PDF 归档到 MinIO/S3-compatible bucket，且 `make runtime-e2e` 已验证临时 MinIO 中存在三类 report artifact；Runtime Console 已展示项目级查询路径、Brand Kit 表单、Logo Upload、Brand Assets 审计型版本列表、Theme Editor 白标预览、报告方法说明、证据附录详情、Report History、导出历史管理事件、signed artifact URL、白标 PDF 下载和 Report Export Queue；Report History 可追加 `internal_review/client_ready/archived` 审计状态；报告导出任务队列已用 `report_export_jobs` + `report_export_job_queued/status_updated` 最小落地，`make report-export-worker` 可领取 queued/expired running job、渲染 runtime artifact、归档或写 fallback URL、按 backoff 重试并在 max attempts 后进入 dead_letter，队列 stats endpoint 已展示核心积压指标；Temporal/长驻 worker、并发池、通知系统、完整素材库、对象存储原生版本/CDN 仍待接
