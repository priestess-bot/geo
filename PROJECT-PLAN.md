...
- `[~]` 可创建 `market=AU` 项目，配 1 行业模板 + 100 条澳洲问题集 + 3–5 竞品（可配置客户项目 API/控制台已落；`GENO_RUNTIME_PROJECT_ACCESS_CONTROL=1` 时已按 actor + `project_members` 做 API 级项目成员门禁，并接入 owner/admin/analyst/viewer 最小角色矩阵、成员删除审计和最后 owner 保护；默认 header demo 模式读取 `X-GENO-Actor-Id`，`GENO_RUNTIME_AUTH_MODE=jwt` 时已支持 HS256 Bearer JWT actor 解析，`GENO_RUNTIME_AUTH_MODE=jwks` 时已支持 inline/remote/OIDC-discovered JWKS RS256 Bearer JWT actor 解析，远端 JWKS 与 OIDC discovery document 均有 TTL 缓存；PostgreSQL RLS/GUC 最小项目隔离与 `geno_runtime_app` runtime role 已落并进入 runtime E2E；Keycloak/OIDC login/session、前端 SSO 登录流、邀请邮件、真实用户目录和真实客户数据验收待接）
