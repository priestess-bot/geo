...
- `[~]` (P1) 预警：负面/品牌缺失/竞品压制 — `E9-04/05/06`（Runtime Alerts 只读派生模型已落：`GET /v1/runtime-alerts` 从最新 `VisibilityScoreSnapshot`、`ScoreContribution`、`AnswerAnalysis`、`SourceGap`、`CompetitorBenchmark` 和 `ActionRecommendation` 生成 `brand_absent`、`low_recommendation_rate`、`negative_sentiment`、`source_gap`、`competitor_pressure`，返回 severity/threshold/metric/evidence refs/related actions/audit refs；Runtime Console 已展示 Runtime Alerts；项目内 inbox 与 webhook 投递队列已覆盖报告导出终态通知；alert 实时推送、订阅、SLA、ack 和 Temporal 工作流待接）
