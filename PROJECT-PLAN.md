...
- `[~]` 内容生成绑 evidence/source_gap/knowledge_fact，过人工审核（runtime content engine API 已读回 evidence/fact/action/prompt/manual distribution 关联，Runtime Console 已展示证据绑定细节，草稿默认 `pending_human_review`；Human Review Trail 已可记录 content draft 的人工审核留痕，只读 Review Queue 已可聚合待审草稿；专用审核工作台、状态回写和审批流待接）
