...
- `[x]` (P1) 批量导入 prompt（CSV/XLSX）— `E2-06`（CSV 文本导入 API、raw-body 文件导入 API、`.csv/.txt` UTF-8 与 `.xlsx` 第一工作表解析、Runtime Console 文本/文件表单已落，写入 `runtime_prompts_imported`；复杂多工作表、公式求值、导入前 diff 预览和错误行下载待接）
- `[~]` (P1) 操作审计日志 — `E1-06`（启动包创建已生成 `project_bootstrap_created`；prompt CSV/file 导入已生成 `runtime_prompts_imported` 并记录 `source_format/source_filename/source_content_type`；用户级 CRUD 审计待接）
...
- `[~]` (P0c) ReportExporter 接口 + Markdown/CSV/PDF 导出（方法说明 + 证据附录）、runtime report API、Markdown/CSV/PDF artifact 下载、附录级筛选/排序下载、项目级 Brand Kit 默认值、白标 PDF 模板 renderer 与 MinIO/S3-compatible artifact 归档；`make runtime-e2e` 已验证报告 Markdown/PDF/CSV artifact 在真实 MinIO profile 中存在；品牌资产上传/高级主题编辑待接 — `Step13`
