...
- `[~]` (P1) 预警：负面/品牌缺失/竞品压制 — `E9-04/05/06`（Runtime Alerts 已形成派生 alert + append-only 管理事件 + 显式通知入队最小闭环：`GET /v1/runtime-alerts` 从最新 `VisibilityScoreSnapshot`、`ScoreContribution`、`AnswerAnalysis`、`SourceGap`、`CompetitorBenchmark` 和 `ActionRecommendation` 生成 `brand_absent`、`low_recommendation_rate`、`negative_sentiment`、`source_gap`、`competitor_pressure`，返回 severity/threshold/metric/evidence refs/related actions/audit refs/management events；`POST /v1/runtime-alerts/{alert_id}/events` 可追加 acknowledged/resolved/snoozed/reopened 到 `runtime_alert_events` 并写 `runtime_alert_event_recorded`，不改写派生 alert 或原始证据；`POST /v1/runtime-alerts/notifications` 可把当前未 resolved/snoozed 的 alert 显式入队为 `runtime_alert` notification，并复用 webhook subscription/delivery 队列；Runtime Console 已展示 Runtime Alerts、管理历史、事件记录表单和通知入队按钮；实时扫描、SLA 升级、邮件/Slack 原生渠道和 Temporal 工作流待接）
