...
> 计划日期：2026-07-21
> 修订日期：2026-08-10（第 92 次修订）
> 计划状态：`LOCAL_PATHS_COMPLETE / CANONICAL_RUNTIME_ACTIVE / BLOCKED_EXTERNAL_INPUT`；唯一 canonical `geo` 栈已运行，真实外部验收仍按专项计划阻断
...
> 第 66 次修订变更：`0073--0075` 已完成 Metric Protocol 的 maker-checker 生命周期、不可变 Analysis Input Manifest、只接受已完成 Sampling Run 与批准 Protocol 的 semantic Job v2 原子准入，以及 manual/Provider Attempt trigger 与 claim 的正确分域。真实垂直测试在隔离 PostgreSQL 执行 `head -> 0074 -> head`，使用临时真实 MinIO 写入三份独立审批、加密且落盘前脱敏的手工工件，由 fenced Worker 形成 Observation 和 semantic snapshot，并经真实 outbox relay 将四条 wakeup 投递到临时 Valkey、按 exact Job/Project 消费。实施中修复了 manual Attempt 被 Provider trigger 误拒绝、manual claim 未投影 aggregate、manual completion SQL 参数数量不符，以及 outbox `dict_row` 确认失败四个真实缺陷。70 项定向 unit/API/migration 与 6 项隔离 PostgreSQL 集成回归通过；该本地实现证据不替代真实 Provider/消费者界面、独立 verifier 或最终 evidence manifest。
> 第 67 次修订变更：`0076--0077` 已完成 Comparison Plan/Drift Protocol 与 Alert Rule 的 maker-checker 生命周期、从 immutable semantic/comparison/drift projection 进行 selector-only server admission、异步统计/告警命令，以及 Workflow C Report 的独立批准状态机和 Customer approved-only projection。真实纵向测试在同一隔离 PostgreSQL/MinIO/Valkey 链路继续完成 paired comparison、drift、threshold alert、单一业务告警和三路通知 Job；实施中修复旧告警 completion RPC 的参数数量、缺失 trigger snapshot、PL/pgSQL 同名列歧义和 JSON 拼接优先级缺陷。133 项组合回归与 3 项 service-backed PostgreSQL 测试、Ruff、scoped mypy 均通过；connector failure、真实 Provider/live evidence、B official report 与独立 verifier 仍保持未完成。
> 第 68 次修订变更：Pack 03 已闭合五类 Provider 的本地发布合同与可重放 canary 工具：OpenAI、Gemini、Perplexity、Microsoft 和 Kimi 使用唯一 canonical source identity，Kimi 经 `0078` 成为独立 `kimi/kimi_api` 来源，Gemini 的业务平台 `google` 与 Gateway route `gemini` 不再混用。Provider Release 冻结模型、搜索能力、请求/结果/错误/引用/位置、保留/display 和完整 source commit；canary manifest 保存无内容的冻结 Task 分母，并必须结合独立 Release 语义重建，重算 hash 不能伪造完成率。CLI 会在写入前校验 Suite、重放 start/enqueue、轮询显式终态并双读 PostgreSQL 后输出 `0600` manifest。108 项组合回归、Ruff、scoped mypy、稳定 OpenAPI、Admin typecheck，以及隔离空 PostgreSQL 的 `head -> 0077 -> head` 均通过。五家真实 Secret、额度、live Run 和 maker/checker 仍为 `BLOCKED_EXTERNAL`，不得由 fixture 或其他 Provider 代替。
> 第 69 次修订变更：Pack 04 已闭合 Synthetic Lab 本地纵向链路。Profile/Review/Corpus/Offline Experiment 生产准入均只接收 selector，服务端冻结真实 Fact/Profile/QuestionSet/Prompt/runtime/Review lineage；Review Worker 固定四候选、claim/conflict/style/arbiter、至多两轮 revision 和一次 regenerate，并保存最终候选文本/hash。`0080_synthetic_corpus_execution` 将 Corpus task/result 纳入 PostgreSQL closed world；Offline Worker 保存 30 个 paired slot 及 completion/warning 三臂摘要。真实空库测试发现并修正 `0079` revision 超过 Alembic `varchar(32)` 的问题。255 项聚焦回归、78 模块 mypy、2 项隔离 PostgreSQL、4 项 Chromium、6 项稳定 OpenAPI 均通过，证据见 `execution-packs/pack-04-synthetic-lab-evidence.md`。九渠道真实账号、每渠道 200 样本、360 Case 人工签字及三家真实模型仍为 `BLOCKED_EXTERNAL`，未由 fixture 冒充。
> 第 70 次修订变更：Pack 05 已闭合消费者 Surface 工件解析的本地纵向链路。Google AI Overviews、Google AI Mode 和 Bing Copilot 使用三个独立 immutable Release，各自通过 30 个 fixture 的成功、有效缺失、阻断与 mutation 门槛；`0081_surface_parser_results` 将脱敏后、无正文/URL 的解析摘要原子绑定到 manual Import/Attempt/Observation，Worker 按冻结 SourceStratum 重检 Release，block/drift 只形成 ineligible evidence。Admin 只提供匹配 Release，强制 JSON transcript，并持续显示 `manual_ui`、`non-live` 和无澳洲出口证明。54 项聚焦回归、2 项隔离 PostgreSQL/MinIO/Valkey、4 项 Chromium、6 项稳定 OpenAPI、Ruff、scoped mypy 与双 TypeScript 合同通过，证据见 `execution-packs/pack-05-surface-parsers-evidence.md`。本修订不实现或验收 B 板块的授权、自动 Browser Capture、sticky 澳洲 egress 或 live Surface Release，故相关 `C-CONTRACT-02`、`EXT-UI/EGR-*` 与最终 Gate 均保持未勾选。
> 第 71 次修订变更：Pack 06 已闭合 Prompt/Recommendation 的本地纵向链路。`0082--0086` 分别补齐 Recommendation producer evidence、Prompt Release 单向退役、批准建议的显式 stale 传播与未开始草稿/outbox 阻断、Worker 最小结果写权限，以及不读取答案正文的 bounded producer summary。隔离 PostgreSQL 使用三份独立批准、加密且脱敏的真实非 B Observation，串联 Semantic Snapshot、Comparison、Rule、Fact、Question、Surface 与冻结 Recommendation Prompt，由 production Worker 输出仅因统计不足和 B Attribution 明确不可用而成立的 `insufficient_evidence`，零模型任务/调用且完整保留 lineage。341 项合同、3 项 PostgreSQL 纵向集成、5 项 Chromium、6 项稳定 OpenAPI、107 模块 mypy、Ruff 和 Admin typecheck 均通过，证据见 `execution-packs/pack-06-prompt-recommendation-evidence.md`。真实模型、B Attribution、Customer/live staging 与独立签字仍未完成，不据此勾选 `D-CONTRACT-01/04` 或最终 Gate。
> 第 72 次修订变更：Pack 07 已闭合本地迁移与运维加固。随机临时 schema 的 PostgreSQL 双写演练固定 Prompt/Protocol/Observation/Metric writer inventory，验证 initial backfill 期间增量写、事务回滚、单调 watermark、真实 cutover lock、连续两轮零差异、rollback-window 写入和旧 writer 退役；最新认证空环境恢复覆盖 `0086`、233 张表、五桶 12 个对象、历史 Secret/Provider/Recommendation/Workflow C/Synthetic keyring 与代表性解密。完整故障矩阵实际终止持 lease 子进程、停止/恢复 Valkey/PostgreSQL/MinIO 并注入 MinIO 部分写失败，`148 passed, 0 failed, 0 skipped`。性能 profile/workload 已冻结但完整 30 分钟生产等价运行、真实遗留数据 cutover、生产 key custodian 和独立签字仍为外部 Gate，详见 `execution-packs/pack-07-hardening-operations-evidence.md`。
> 第 73 次修订变更：Pack 08 已完成除 B 外所有本地可控实现、全量回归、跨角色权限/数据泄漏/提示词注入复查和 Admin/Customer 易用性修正；数据库 head 为 `0087_wfc_report_receipts`。最终干净 Gate 为 required non-live `2317/2317`、真实 PostgreSQL/MinIO/Valkey integration `117/117`、Chromium `46/46`、architecture `42/42`、infra contracts `71/71`、production network `2/2`、runtime `7/7`；认证空环境恢复覆盖 234 张表、109 个非 B 关系、87 个迁移 checksum 和五桶 12 个对象。机器 manifest 仍为 `BLOCKED_EXTERNAL`：真实 Provider/模型/九渠道账号与样本、生产等价性能/live staging、真实遗留数据切换、生产 key custodian 及独立 verifier 签字均未由本地 fixture 或 controlled run 冒充；B 的 Connector/归因/自动 Browser Capture/澳洲 egress 继续排除。
> 第 74 次修订变更：Prompt 工作区和四条既有业务生成链已迁移到 Dify 托管运行时，当前 Alembic head 为 `0093_dify_workflow_runtime`。最新干净 Gate 为 required non-live `2371/2371`、隔离 PostgreSQL/MinIO/Valkey integration `119/119`、Chromium `49/49`、Ruff、762 个 Python source 的 MyPy、六个 Web workspace typecheck、2,069 文件 Secret scan 和 architecture `43/43`，全部零失败、零 skip/flaky。认证空环境恢复已从基线升级到 `0093`，核对 238 张表、110 个非 B 关系、93 个迁移 checksum、五桶 12 个对象、ACL/RLS 与历史 Secret/Provider/Synthetic/Recommendation/Workflow C keyring，receipt SHA-256=`d0d446c3a2f58beba61a24169019217990778e8fff74ca001f71e32fc92251db`，临时 Compose 资源完整清理。四条真实 DeepSeek canary、四个业务 Durable Job、Dify 断开/恢复与禁止静默 native fallback 已通过；多租户许可证书面确认、生产等价性能、真实外部账号/样本和独立 verifier 仍为非阻塞外部项，B 板块继续排除。
> 第 75 次修订变更：四条核心生成链的 Prompt/模型/流程变量已改为 Dify 唯一可编辑来源；`0094_dify_published_snapshot` 保存发布图的只读快照并绑定 Execution Attempt。Admin 只读看板仅展示四条当前 Dify 工作流、真实 Prompt、模型、输入变量、同步与运行状态；刷新或任务执行立即采用最新发布版本，无 GEO 二次批准。Dify 断开时页面显示上次快照和失败原因，业务任务 fail closed 且无本地 Prompt fallback。2026-07-27 四条真实 DeepSeek canary、真实断开/恢复、22 项定向 Python 回归、2 项桌面/移动 Chromium 回归及 live 页面截图通过。其余十类 Prompt、工作流模板、项目级模板选择和多国家/locale 版本列为后续 backlog；GEO Fact/Evidence 知识库不迁移到 Dify。本修订不改变 B 板块排除范围，0094 的认证空环境恢复和最终全量 Gate 尚需重跑，故不沿用 0093 收据冒充当前恢复证据。
> 第 76 次修订变更：`0094_dify_published_snapshot` 的最终本地 Gate 已重跑完成。required non-live 为 `2376/2376`、隔离 PostgreSQL/MinIO/Valkey integration 为 `119/119`、稳定 OpenAPI 为 `7/7`、双 Web production build 与 Chromium 为 `46/46`、infra contracts 为 `71/71`、production network 为 `2/2`、runtime 为 `7/7`，均零失败、零 skip。`make quality` 通过 Ruff、764 个 Python source 的 MyPy、六个 Web workspace typecheck、2,076 文件 Secret scan 和 architecture `43/43`。认证空环境恢复覆盖当前 `0094`、239 张表、110 个非 B 关系、94 个迁移 checksum、五桶 12 个对象、ACL/RLS 与历史 keyring；receipt SHA-256=`7ace2d57d36ad57c274a277676b5dd082e7d15b8a47f57184af7da7232392659`，manifest SHA-256=`6a3258afd15943e72b891a60181f2fdda9b0273419162b68820213935ea1db13`。四条最终 Dify Release 与真实 DeepSeek canary 均成功，模型返回畸形 JSON 时现在保留 Dify task/run/workflow lineage 并进入 Durable retry，而不放宽结构化输出校验。本地 Gate 完成不替代生产等价性能、真实外部账号/样本、生产 key custodian 或独立 verifier；B 板块继续排除。
> 第 77 次修订变更：Dify Test Run 输入改为从 Dify 自身 `app-run` 历史自动注入，不新增 GEO API 或数据库投影。仅 `created_by_end_user.session_id` 以 `geo-job:` 开头、状态成功且五个 `geo_*` 输入完整的真实业务运行可选；canary/debug/失败/畸形记录与 `sys.*` 字段均排除。默认选最新兼容运行，允许在 Dify 内切换历史或显式解锁技术字段。实现以固定 Dify `1.16.0` commit 和仓库 patch 构建带 hash label 的自定义 Web 镜像，可一条命令回退官方 Web 镜像而不改 API、Worker 或数据。本项不改变 B 板块排除范围，也不把尚未完成的 live browser 复验记为已完成。
> 第 78 次修订变更：第 77 次修订的 live browser 与回退门禁已完成。自定义 production Web 镜像 `geo-dify-web:1.16.0-geo` 的 overlay label 为 `7c379f5e1814faaf0d7c80d21cd7154fca3ef381d19506273ec68681a14ed0ba`；四条真实工作流分别识别 `2/2/2/3` 条 `geo-job:` 历史，每条均完成最新记录自动选择、旧记录切换、五字段解锁/折叠且 workflow-run 请求无失败。容器已实际切换到官方 `langgenius/dify-web:1.16.0` 并健康返回，再恢复自定义镜像后重复四条浏览器验收通过，Dify API、Worker 和持久数据未重建。
...
> 第 84 次修订变更：合成测评实验室 Admin 已从单页控制台重构为实验室首页、生成仿真文案、任务与结果、语料实验、风格资源、测评套件和授权设置七个中文任务视图。Internal API 新增 Project-scoped Job 列表和 Review Result 读取，结果页只展示已持久化的最终文案、最终候选评估、Claim 冲突与修订 lineage；人工导入 503 仅禁用对应功能，不再拖垮首页或生成流程。7 项 API 回归、8 项 Synthetic Chromium、2 项运行地图 Chromium、Ruff、Admin typecheck/production build 与稳定 OpenAPI 均通过；`geo-advinsys-staging-v2` 的 Internal API/Admin 镜像已重建，真实 ADVINSYS 七个页面在桌面和移动端均返回 200、无控制台错误或横向溢出。当前真实项目仍为 `3/5` 生成前置就绪，缺冻结测评套件和已验证风格画像，真实生成任务/结果均为 0；因此相关外部业务 Gate 继续保持未完成。
> 第 85 次修订变更：首次套件即时刷新修复使用成功状态 effect 调用 `router.replace`；fixture Chromium 曾通过，但真实 ADVINSYS 点击后暴露成功状态在 RSC 更新期间被保留并重复导航，按钮持续停留“创建中”。Internal API 日志证明写入已 `201 Created`，问题位于客户端 pending/导航生命周期。该方案已由第 86 次修订替代，不作为完成证据。
> 第 86 次修订变更：套件写入成功后的导航改由 Server Action 在 `revalidatePath` 后发出唯一 Next 303，删除客户端成功 effect；新回归要求按钮恢复、目标 URL 和新行可见、浏览器内存标记保留，并在额外等待后验证 Review Suite GET 请求数不再增长。最终 Admin 镜像为 `sha256:eb74af04d88c7a086517e5b605b124c1a34325ea8eae3d5e4d11256d2c673ce1`。真实 ADVINSYS staging 以 `pending-fix-live-validation-2026-08-03` 创建 Quora 草稿套件版本 `3c4a209e-1e3e-56f3-a408-43db89633a05`：单次 POST 返回 `201`，约 5.7 秒后新行和目标 URL 可见，按钮恢复可用、`创建中` 按钮数为 0，额外等待 2.2 秒后 URL 与请求数保持稳定，桌面/390px 均无控制台错误或横向溢出。问题复现时实际已写入的草稿 `f008a0a1-204d-5714-910f-f95282fa4656` 与本次验收草稿均原样保留，未在无用户授权时删除。套件列表仍以不滚屏 Next Link 切换，当前项显示“当前已打开”。该修复不改变套件/画像冻结状态和真实生成结果。
> 第 87 次修订变更：第 84--86 次修订的七视图实验室已收敛为“生成工作台”和“渠道风格”两个连续操作视图。直接生成不再要求运营人员先理解或冻结 Review Suite、Case、正式 Style Profile、Corpus 或授权对象；Internal API 会从所选产品的当前 `approved_fact` Evidence、九渠道手工风格版本、六步 Prompt 覆盖的模型运行时和用户目标形成不可变 Direct Generation Snapshot，再执行 `generation -> claim_extraction -> conflict_check -> revision（仅在需要时） -> style_judge -> arbiter`。ADVINSYS 三个产品现均具备当前批准事实，九渠道均有 `manual_initial / pending_sample_calibration` 风格预设。真实浏览器点击创建的 Job `f26cf4e7-8358-5f0f-9a75-272fbe8f8837` 已成功：4 个候选、9 次 Dify workflow attempt、8 次原生模型调用，最终结果 `completed_with_warning`，V600 两条批准事实均显式命中且可跳回证据追溯页，`derived_or_unknown` 单独提示；本次合格候选没有触发修订，实际修订路径仍以既有正负向 live run 和回归测试为证据。历史正式 Suite/Profile/Corpus/Offline API 继续保留为内部高级能力，但不再暴露为主操作页面。本修订只证明直接生成路径、知识 lineage 和手工初始风格可用；九渠道每渠道 200 条合法人工明审样本、正式 Profile 发布、360 Case 和三臂离线实验的真实业务证据仍未完成，相关 Gate 不勾选。
> 第 88 次修订变更：直接生成 API、Presenter、Route 和 PostgreSQL admission/read projection 已按职责拆分，未增加豁免并重新通过 architecture `42/42`；合成实验室相关 Python/架构回归为 `327/327`，稳定 OpenAPI 两个 surface、Admin typecheck/production build 和 Synthetic/运行地图 Chromium `6/6` 均通过。最终运行镜像为 Internal API `sha256:34cb2478b84711efeb98f89ffb0116eefc39842f4795f39350eb5de7d11ed92d`、Task Worker `sha256:5121d28006b60e660de7e974ee01aa20aedc9c678693415dab4efeb09cddc741`、Admin `sha256:ce6ea1acc8b0b17c5565051a1f8cfec28dcf3c68fdc85a039e2c0cd4c11f7a0c`。重建并替换 `geo-advinsys-staging-v2` 后，真实 Admin 点击又创建 `20c8c73b-33e5-53de-b8c7-17fd1c2b0697` 和 `07994a63-f612-52c1-b77e-2929ceb5e240`，两者均在可重试 Provider 输出后自动恢复并进入 `succeeded`；当前 `review.case.run` 共 4 条真实成功任务。桌面/390px 生成工作台与九渠道风格均为 HTTP 200、无横向溢出或控制台错误，运行地图四视图在真实 `13001` 通过并将该链路显示为 8 个步骤。此验证只增加当前直接生成路径的完成证据，不改变第 87 次修订列出的真实样本、正式 Profile、固定 Case 和离线实验缺口。
> 第 89 次修订变更：测试问题生成、候选人工审核、QuestionSet 创建/批准/冻结及 draft 监测方案绑定已从 `GEO 投放` 迁入 `测量与告警 -> 测试问题`；旧 GEO `question_generation_job_id` 深链自动转到新位置，GEO 仅保留冻结 QuestionSet 的只读摘要和内部仿真入口。后端领域对象、Project/Campaign lineage 和既有持久化合同未改变。29 项 Admin 契约、12 项 GEO Chromium、6 项 Workflow C Chromium、2 项真实运行地图 Chromium、Admin typecheck/production build 均通过；真实 `13001` 验证 1 个 succeeded 任务和 3 条 pending_review 候选在 1440px/390px 下无横向溢出或控制台错误，Admin 镜像为 `sha256:490462b23896e396ae9a1a6ab48c218779bb8acca38ebfe99e4061b2a08ec6b1`。本次未代替业务人员审核候选，QuestionSet 仍为 0，不把页面迁移或模型成功计为采样业务完成。
> 第 90 次修订变更：`测量与告警 -> 测试问题` 已增加一次生成完整 100 题的主路径。服务器按 `au-cross-engine-balanced-v1` 冻结 50 条跨产品类目基准、40 条产品适配和 10 条品牌控制题，以 10 个批次执行；每批 5 条固定基准加 5 条 Dify/DeepSeek 生成题，成功批次形成不可变 checkpoint。`0122/0123` 补齐覆盖计划、候选修订、批次恢复和语义重复证据一致性；死信任务可由专用 Campaign-scoped 命令有限恢复，不经过 GEO 投放 Job 控制面。真实 ADVINSYS V600、V1000、SAT30 三条任务均 `succeeded`，各有 100 条不同问题文字及 `50/40/10` 配额，V600/V1000 共享恰好 50 条类目基准，冻结 Fact 越界引用为 0；疑似重复分别为 7/4/2 条并保留给人工排除。新 Dify Release `0fee4695-0fd0-4fb5-8774-6da4efbc3d4b` 已通过真实 DeepSeek Canary 后激活。当前仍为 300 条 `pending_review`、0 个由本批次冻结的 QuestionSet，未替代运营审核或真实采样业务 Gate。
> 第 91 次修订变更：100 题审核现在增加不可变后继路径：原始疑似重复证据保留，业务人员必须修订所有待修题，服务端确定性复核后才允许以 `series_id + previous_version_id` 创建恰好 100 条的新冻结版本；未产生新的 QuestionSet/真实采样结果前，不把该代码路径写成业务完成。Workflow C Sampling 对超过 10 条的冻结输入现在持久化恰好 10 个唯一 `question_set_item_ids`，Task 只从这份选择派生，避免 pilot 重试或页面状态改变分母。
> 第 91 次修订同时记录外部适配器就绪边界（历史记录，已撤销）：当时的 SerpAPI `provider_api` registry、Secret Store handle、错误分类和 canary CLI 仅通过本地合同，未形成真实 Observation；GSC/GA4 也只有 scope 校验和 canary/readiness 命令。SerpAPI 现已退役，当前以 LokiProxy Browser Capture 出口合同为准。
> 第 92 次修订变更（历史发布记录）：唯一 canonical Compose 项目 `geo` 已在兼容端口启动，旧运行容器已移除、旧卷只作为回滚副本保留；PostgreSQL、MinIO 与 Valkey 数据克隆经计数和 hash 对账，Alembic head 为 `0130_serpapi_secret_purpose`。启动验收发现 Connector Worker 镜像误打包宿主机嵌套 `.venv`，现已从 Docker context 排除，在镜像构建时验证 `dramatiq`、`psycopg` 和入口模块可导入，并由 GitHub deployment Gate 实际构建该镜像；`geo-stack doctor` 同时绑定 canonical Compose 文件集合、工作目录与 `oneoff=False` 身份，强制核对全部必需服务正在运行、声明健康的 GEO/Dify 服务实际为 `healthy`、迁移与 MinIO 初始化以 exit 0 完成、数据库实际 Alembic head 与源码一致，以及业务镜像 release identity。ADVINSYS SAT30 原冻结 v1（98 条）保持不变，两条疑似重复已通过正式编辑 API 形成 revision lineage，服务端随后冻结同一 series 的 exact-100 v2 `5b1583c1-efe6-5a01-8025-6709f07dbf46`，100 个维度全部覆盖、重复数 0、content hash=`2d8ecaf6f3bee65bda3d5eea46c8da278f3b9589ab64482e7a9d6c11fa62ad15`。这只使 10 题 pilot 有了合法冻结输入；SerpAPI Secret、GSC/GA4 property 凭据、澳洲 sticky proxy、真实 Observation/Sync/Capture、新主服务器恢复和真实 Revenue 旅程仍未完成。
> 第 93 次修订变更：SerpAPI 正式 Provider 方案已完全退役，`0130_serpapi_secret_purpose` 只保留为历史迁移说明；当前源码新增 `0131_lokiproxy_pool`，把 LokiProxy 定义为供应商托管的澳洲 residential/mobile sticky pool，而不是 GEO 自建 IP 池或回答型 Provider。每个 Browser Capture Attempt 使用唯一 `session_id`，并将同一 sticky lease 的 pre/target/post 出口证明绑定到 Attempt/Observation lineage。当前 ADVINSYS 的 LokiProxy pool 为 0、live Capture 为 0；AIO、AI Mode、Copilot 必须分别完成授权、代理会话证明和 Surface Release 保真验收，不能用其他 Surface、fixture、技术 canary 或裸 IP 代替。
> 第 94 次修订变更：本机 canonical `geo` 栈已从干净提交 `655a327bf8f753b0ea43caf915728fe6172d6c63` 重建，迁移容器以 exit 0 将 PostgreSQL 推进至 `0131_lokiproxy_pool`。`geo-stack doctor` 与 v3 release receipt 已核对源码、声明和全部受跟踪运行镜像使用同一 SHA，GEO/Dify 必需服务健康且数据库 head 与源码一致。该发布只证明 LokiProxy 池合同可运行；数据库实查仍为 0 Pool、0 Capture Session、0 Capture Job，因此真实澳洲出口和三个 Surface 的 live Gate 不变。
...
- [x] `SYN-CONTRACT-01` 按第 5.1 节实现 Style Source、Collection Run/Sample/Profile、Review Suite/Case/Run、Candidate/Evaluation/Revision、Corpus 与 Offline Experiment 的项目范围状态机、版本和 immutable lineage。最早 M1；验收：`M1-SYN-01`、迁移/RLS/API/Admin tests。实现证据见 `IMPL-SYNTHETIC-PACK-04-LOCAL-2026-07-24` 与第 87 次修订；真实样本/Profile/实验发布证据仍由 `SYN-CONTRACT-02/03/06/07` 单独约束。
...
- [x] `SYN-CONTRACT-04` `autonomous_scenario` 与 `guided_scenario` 都把 Fact/Catalog 身份作为权威输入；运营输入仅作创意参考，每 Case 默认四候选并完整保存 generation lineage。最早 M2；验收：`M2-SYN-03/04`、`M2-AC-03`。直接生成另以当前批准 Fact 和不可变输入快照执行；固定 360 Case 内容仍由未完成的 `M2-SYN-04/M3-SYN-01` 约束。
- [x] `SYN-CONTRACT-05` Claim extraction、conflict/subject check、style judge/arbiter、至多两轮 revision、一次 regenerate、warning 与 Fact 失效/cancel/lease-loss 终态遵循第 5.3 节。最早 M3；验收：`M3-SYN-02`、`LAB-FLOW-AC-01`、`M3-AC-02/03`。状态机、Dify/native 分工和真实直接运行证据见第 80、87 次修订；第 87 次运行未触发 revision，不以此替代既有修订负路径证据。
...
- [x] `M2-SYN-03` 实现 `autonomous_scenario`、`guided_scenario`、每 Case 四候选、claim extraction、基础 conflict/style judge 和候选 lineage。
...
- [x] `M2-AC-03` 两种 scenario mode 各自生成四候选；运营输入不能覆盖 Fact/Catalog 主体或成为事实来源。
...
- [x] `M3-SYN-02` 完成 conflict/subject check、最多两轮 revision、一次 regenerate batch、warning 直出、任务取消、lease 丢失和 Fact 失效状态机。
- [x] `M3-SYN-03` 完成 Corpus Version 与三臂配对 Offline Experiment；每题每臂 10 次，冻结 QuestionSet/模型/Prompt/Corpus/method/hash。
- [x] `M3-SYN-04` 实现 passed/warning 合并与独立分层、Admin 明确 synthetic/test-only 标签及 Customer 全路径拒绝。
...
- [x] `M3-AC-02` 两轮修订后只允许一个 regenerate batch；取消、lease/fencing 丢失或 Fact retired 不提交陈旧 Candidate/Corpus。
- [x] `M3-AC-03` `derived_or_unknown` 可形成 warning；明确 conflict/subject mix 必须修订，subject mix 总数为 0 才可发布。
- [x] `M3-AC-04` 三臂实验对相同冻结输入可重算同一 hash；warning 占比/分层始终可见；synthetic 无 Customer 读取路径。
...
- [x] `LAB-FLOW-AC-01` 两轮修订、一个 regenerate batch、Warning 直出、取消、lease/fencing 和 Fact 失效均有行为证据。
- [x] `LAB-CUST-AC-01` 所有 synthetic/Review/Candidate/Corpus/Offline Experiment 均保持 test-only，Customer/API/export 全路径不可见。
...
> 快照日期：2026-08-10。此表是实现推进账本，不是 evidence manifest，也不替代第 9、10、15
...
| `IMPL-A-SYNTHETIC` Style Collection、Profile、Review、Revision、Corpus、Offline Experiment | `LOCAL_COMPLETE / BLOCKED_EXTERNAL` | 九渠道 workload/release Gate、手工样本 maker-checker、raw/derived 隔离与 retention、Profile/Review 六 Prompt Worker、四候选、两轮 revision/一次 regenerate、Corpus maker-checker、三臂 10 次配对实验、warning 分层、selector-only Admin 和 Customer deny 均已落地；`0080`、255 项聚焦回归、2 项隔离 PostgreSQL、4 项 Chromium 与 78 模块 mypy 证据见 Pack 04 文件 | 九渠道真实/合规导入素材、每渠道 200 样本、正常登录账号、360 Case 人工 rubric、三家真实模型及最终独立签字 |
...
| `IMPL-B-CONNECTOR-ATTRIBUTION` Connector Core、GSC/GA4、官方报告、消费者 UI capture、事件入口、归因账本 | `LOCAL_COMPLETE / BLOCKED_EXTERNAL` | `0102--0131` 已实现 Connector Core/PyAirbyte GSC/GA4 Worker、scope 校验与 canary/readiness 命令、外部报告批准投影、Session→Revenue 账本，以及 AIO、AI Mode、Copilot 三个内置 Browser Capture Release、LokiProxy provider-managed 澳洲 residential/mobile pool profile、逐 Attempt 唯一 session 的 sticky pre/target/post 证明、逐 Surface 采样输入和原子批量入队。`0126` 对超过 10 条冻结输入持久化恰好 10 个唯一 `question_set_item_ids`，Task 只从该选择派生；`0131` 增加 LokiProxy pool health/cooldown、产品和并发合同。LokiProxy 只作为消费者 UI 的出口依赖，不是回答型 Provider。当前 canonical 数据尚未配置 LokiProxy pool，真实 Provider Observation、GSC/GA4 Sync 和 live Capture 仍为 0；已安装 3 个 approved Surface Release 与 1 个匿名 `en-AU` Profile。Worker、Outbox、API 和 Admin 的本地合同可运行，但真实凭据、代理连接日志/出口证明、逐 Surface live canary、真实业务 Revenue 旅程及独立 verifier 仍需按专项文件完成；技术 bootstrap、scope/canary 和本地测试不得算业务结果 |
...
> 当前状态校正（2026-08-10）：上表各长行保留其形成时的实现证据，当前部署身份以第 94 次修订和本节末尾 checklist 为准。`0130_serpapi_secret_purpose` 仅是历史迁移；本机 canonical 已从干净提交 `655a327bf8f753b0ea43caf915728fe6172d6c63` 重建至 `0131_lokiproxy_pool`，release receipt 已核对源码、声明、运行镜像和数据库 head。SAT30 exact-100 v2 已冻结，Connector/Browser Worker 已运行；当前 LokiProxy pool、Provider/GSC/GA4 live 数据和 Browser Capture 仍全部为 0，新主服务器恢复也未执行，故任何外部或最终 Gate 都不因此改变。
...
- [x] `IMPL-WORKFLOW-C-JOB-SPECS-2026-07-23` 0032/0034 的 `workflow_c_job_specs` 与 Durable Job 同事务创建，并由 `input_hash/spec_hash`、canonical schema v1、immutable/RLS/最小权限绑定。最终 Pack 08 已逐类覆盖两种 Sampling、三种 Analysis、两种 Metric child 和三种 Alert/Notification 的实际 admission/producer；递归拒绝 secret/credential/token/password/proxy/authorization 实值，Worker 只在当前 Project 的有效 lease/fence 下重建并验证输入。隔离真实 PostgreSQL/MinIO/Valkey integration、专用空库故障矩阵及跨 Project、过期 lease/fence、取消/重放和敏感字段负测均通过；该勾选只证明本地 closed-world producer 合同，不替代四个仍标为 `BLOCKED_EXTERNAL` 的 live 母项。
...
- [x] `IMPL-RECOMMENDATION-POSTGRES-LIFECYCLE-2026-07-23` Recommendation lifecycle 已由隔离 PostgreSQL 验证：最小 `geo_app` 角色可经 durable builder 创建、提交、独立复核和批准，批准只创建幂等且未开始的下游草稿；`0084_recommendation_stale` 使已批准 Fact 退役后追加显式 `stale` 版本，原子阻断仍为 draft 的草稿并取消 pending outbox，已经 started 的草稿不被历史重写。版本表保持 append-only，应用层 advisory lock、数据库 predecessor/trigger 和 prepare-action source recheck 共同拒绝 direct `UPDATE` 与执行旁路；到期路径仍独立形成 `expired`。该项不替代 B Attribution、完整 Customer/live staging、独立 verifier 或 live evidence。
- [x] `IMPL-RECOMMENDATION-GENERATION-POSTGRES-ENQUEUE-2026-07-23` Recommendation generation submission 与 production Worker 已在隔离 PostgreSQL 验证。入队使用冻结 Prompt binding、当前 producer evidence、项目角色和受控 RPC，首次调用原子创建 Durable Job、immutable Spec、broker outbox、event 与 receipt，同键重放不增行；Worker 以真实 Prompt resolver、lease/fence 和 `0085` 最小结果写权限完成任务。`0086` 只提供不含答案正文的 producer summary；证据不足时零模型调用并产生明确 Sampling Plan 草稿。真实 Provider 模型调用、B Attribution、live canary 和独立签字仍未完成。
...
- [x] `IMPL-WORKFLOW-C-SEMANTIC-V2-VERTICAL-2026-07-24` `0073_wfc_metric_protocols`、`0074_wfc_semantic_job_v2` 与 `0075_wfc_manual_attempt_scope` 已将批准 Metric Protocol、不可变输入 manifest、已完成 Sampling Run 解析、secret-free Job spec/outbox 和 fenced semantic snapshot 组成可运行的 durable vertical。`test_workflow_c_semantic_vertical_postgres.py` 在隔离数据库完成 `head -> 0074 -> head`，以最小 App/Worker role、临时真实 MinIO 和临时真实 Valkey验证三份独立审核的加密/脱敏 manual artifact 形成完整 Observation 分母，随后只在持有 lease 的 Worker 内解密、计算并持久化 denominator/valid count 均为 3 的 snapshot；relay 实际确认数据库 outbox 并在 Valkey 消费 exact semantic Job/Project wakeup，Job spec/manifest 均不包含答案、邮箱或 token。70 项定向 unit/API/migration、6 项隔离 PostgreSQL 集成、Ruff 及 11 个生产模块 mypy 已通过。该勾选仅代表 Pack 01 本地实现完成，不替代原始 `C-CONTRACT-04`、`M4-METRIC-01` 等仍要求真实 Provider、live evidence manifest 与独立 verifier 的验收项。
- [x] `IMPL-WORKFLOW-C-STAT-ALERT-REPORT-VERTICAL-2026-07-24` `0076_wfc_stat_protocols`、`0077_wfc_alert_report_api` 与 durable Internal API 已闭合 Pack 02 的本地纵向路径：App 只能提交 Protocol/Rule ID 和 immutable source hash/locator，服务端重读并校验 semantic/comparison/drift projection 后原子创建 Job/spec/outbox；Worker 以冻结 `delta/power/precision/min_pairs/seed/family` 计算 comparison/drift，告警 evaluator 从冻结输出产生可解释 trigger，首次命中只创建一个业务告警及 Admin inbox、本地 SMTP、签名内网 Webhook 三个通知 Job。Report 使用独立 `draft -> in_review -> approved -> stale|superseded|revoked` 状态机，maker 不得自批，Customer reader 每次重检 semantic source 完整性。隔离 PostgreSQL/真实临时 MinIO/Valkey 的同一测试链完成 manual Observation -> semantic baseline/candidate -> comparison -> drift -> threshold alert；另两项 PostgreSQL 兼容测试覆盖 disposition replay 和 approved-only report。133 项组合回归、3 项 service-backed 回归、Ruff 与 scoped mypy 通过。该勾选只代表 Pack 02 本地实现，不能替代 connector failure 告警、B official report、真实 Provider/live evidence、最终 manifest 或独立 verifier，因此 `C-CONTRACT-05/06`、`M4-*` 与 `M5-CUST-01` 的宽泛 Gate 仍不提前勾选。
- [x] `IMPL-WORKFLOW-C-FIVE-PROVIDER-CANARY-LOCAL-2026-07-24` Pack 03 已在既有 Secret Store、Model Gateway、Durable Sampling、工件与 lease/fencing 边界上闭合五类 Provider 的本地合同。`provider_sources.py` 固定 OpenAI、Gemini、Perplexity、Microsoft、Kimi 的业务 source/Gateway route，`0078_provider_source_identity` 使 `kimi/kimi_api` 可在 Monitoring/API/Web 中端到端表达；Provider Release 严格冻结请求/结果/错误/引用/位置、reported model、search capability、data policy、依赖与完整 commit。可重放 canary CLI 在写入前核对 immutable Suite，验证 start/enqueue 幂等，轮询显式终态并双读 PostgreSQL；脱敏 manifest 记录可重算的计划分母，verifier 必须结合独立 Release 重建全部语义，故重算 hash 不能伪造通过。108 项组合回归、稳定 OpenAPI 6 项、Admin typecheck、Ruff、scoped mypy，以及隔离 `pgvector:pg16` 的空库升级/回退/再升级均通过，证据见 `execution-packs/pack-03-provider-sampling-evidence.md`。此勾选仅表示本地实现完成；五家真实 Secret/额度/Run/发布签字保持 `BLOCKED_EXTERNAL`，所以 `M3-EXT-01`、`M3-AC-05` 与 `EXT-PROV-*` 不提前勾选。
- [x] `IMPL-SYNTHETIC-PACK-04-LOCAL-2026-07-24` Synthetic Lab 的本地可控实现已闭合：Production admission 从 Profile、Case、完成 Review、Corpus、QuestionSet 和 approved runtime selector 重建冻结 task；Review Worker 执行四候选、六 Prompt、两轮 revision/一次 regenerate，Corpus Worker 在 lease/fence 下冻结 passed/warning 候选，Offline Worker 固定 baseline/current/candidate 三臂每题 10 次并保存 slot 与 completion/warning 摘要。`0080_synthetic_corpus_execution` 的真实空库升级和 Corpus-evidence downgrade refusal、App/Worker RLS、Admin warning projection均通过；同时将超长 Alembic ID 修正为 `0079_synth_profile_runtime`。255 项聚焦测试、2 项隔离 PostgreSQL、4 项 Chromium、6 项稳定 OpenAPI、78 模块 mypy、Ruff、Admin/API-client typecheck 和 `git diff --check` 通过，详见 `execution-packs/pack-04-synthetic-lab-evidence.md`。此项只证明本地实现，不替代九渠道真实账号/素材、每渠道 200 人工明审样本、360 Case rubric、三家真实模型与独立发布签字，因此 `M2-SYN-01/02/04`、`M3-SYN-01`、`M3-AC-01/06` 仍不勾选。
- [x] `IMPL-SURFACE-PARSER-PACK-05-LOCAL-2026-07-24` Google AI Overviews、Google AI Mode 与 Bing Copilot 的三个独立 fixture/manual parser Release、严格工件 schema、有效缺失/阻断分类、逐 Release fidelity、`0081` immutable text-free summary、maker-checker manual Import、fenced Observation/semantic materialization 及 Admin non-live review 已完成。54 项聚焦测试、2 项隔离 PostgreSQL/真实临时 MinIO/Valkey、4 项 Workflow C Chromium、稳定 OpenAPI 与静态类型验证通过，详见 `execution-packs/pack-05-surface-parsers-evidence.md`。该勾选只覆盖供应 fixture/人工工件；B 板块的授权、自动消费者浏览器采集、sticky 澳洲出口、地域证明和 live Surface Release 明确未实现，不能据此勾选任何 `EXT-UI/EGR-*`、`REAL-EXT-*` 或最终 Gate。
- [x] `IMPL-PROMPT-RECOMMENDATION-PACK-06-LOCAL-2026-07-24` 十类本地 Prompt bootstrap、create/diff/test/approve/freeze/bind/retire 状态机、六类 Recommendation、四类 typed 下游草稿及真实非 B evidence-bound production Worker 已闭合。`0082--0086` 保留 producer lineage、单向退役、显式 stale、最小 Worker 写权限和不含答案正文的 bounded summary；隔离 PostgreSQL 验证三份独立批准的加密/脱敏 Observation 经 Semantic/Comparison/Rule/Fact/Question/Surface/Prompt 进入 Recommendation，统计不足和 B Attribution unavailable 产生零模型调用的可解释 `insufficient_evidence`。341 项合同、3 项 PostgreSQL vertical、5 项 Chromium、6 项 OpenAPI、107 模块 mypy、Ruff 与 Admin typecheck 通过，详见 `execution-packs/pack-06-prompt-recommendation-evidence.md`。真实模型、B Attribution、Customer/live staging 与独立签字仍为外部缺口，`D-CONTRACT-01/04` 及最终 Gate 不提前勾选。
- [x] `IMPL-PROMPT-EDITOR-WORKSPACE-2026-07-25` Prompt 程序页已重做为面向单一操作者的三栏工作区：左侧按业务流程分组和搜索，中间直接显示并编辑完整 System/User Prompt、拼接预览及历史版本，右侧插入运行时上下文槽位、选择模型并查看固定测试；草稿 1 秒自动保存并支持 `Ctrl/Cmd+S`，只有当前草稿对应的候选通过固定测试后才启用“发布并生效”。空项目可一次初始化默认 Prompt 集，初始化后会自动选中首个可编辑 Prompt；目录投影未及时内嵌草稿时由草稿端点补取。`0090_prompt_workspace` 为新旧 Prompt 原子初始化/回填可编辑草稿，CAS revision 防止覆盖，发布按钮在服务端一次完成 approve/freeze/bind；详情接口仅在单个 Release 读取时返回正文，列表仍保持 hash-only。真实 staging 已从 `0089` 升级到 `0090`；`0091_mgw_worker_admit` 仅授予受既有 admission trigger 约束的 `geo_worker` `INSERT`，使统一 Prompt 测试可持久化模型调用准入。槽位点击现在只插入 `{{slot_name}}`，完整 `<request_json>` 边界只能由显式“完整请求块”动作插入；编译预览和实际调用均追加同一份冻结 application output schema，以兼容 DeepSeek JSON mode。`0092_prompt_workspace_kinds` 已为问题生成、RAG 依据整理、投放内容生成和投放仿真分别建立独立 Program kind、输入/输出/应用校验、固定测试集与上下文槽位；它们现在在“问题与内容生成”分组中可编辑、可预览并可运行固定测试，不再显示“待接入统一运行时”。2026-07-26 受控 staging 使用 Secret Store 中的 DeepSeek reference 和 `deepseek-v4-flash` 成功完成五例固定测试（Job `cd4f7ee9-5d3f-519b-b1f0-acc98044e55e`，`5/5` calls、`100/100`、结果工件 hash `46fa68ea30357ce035c536fbefef4a436649e050e3e12e936b64331c68a51525`）；随后 `knowledge.question_generation` 工作草稿亦完成实际五例 DeepSeek 固定测试（Job `b0fc389f-3d11-5284-ac74-7a442403a903`，状态 `succeeded`、`passed=true`）。
- [x] `IMPL-PROMPT-LEGACY-FLOW-MIGRATION-2026-07-26` 状态 `COMPLETED`：四个流程均通过独立冻结的 Prompt Program binding、Dify Workflow Release 和 Secret Store app key 执行，且业务 Worker 继续使用各自的真实输入与精确输出合同，不复用 Synthetic Candidate 的 `generation` schema。live staging 已用真实 DeepSeek 完成四条语义 canary，并分别由 `knowledge.rag.extract` Job `2cfe3968-6998-5ffa-bf36-8c2b594e32d2`、`knowledge.question.generate` Job `2db1cd02-9e97-5c3f-8625-3e69c117ed80`、`prompt_simulation.generate` Job `f0eb72cb-9635-4bf8-b095-c23051fef017` 与 `placement.generate` Job `e740d7ee-1f7f-4161-8e66-1aac19ba704c` 形成四个成功的 business attempt；对应结果已落为 RAG revision、2 个问题候选及 MinIO Question Artifact、test-only Simulation Result/Artifact 和 Placement Package Version。停止 Dify API 的真实演练产生 `HTTP 502`、`retryable=true` 的失败 Attempt，恢复后同一 Release 重新成功，且 active binding 未静默回退 native。配置/enrol 重放保持同一四个 Dify app、Secret version 和 GEO Release；Dify Celery Redis、仅 loopback `15000` 入口及 Internal API 四张 active card 已复核。最终 QA 覆盖完整 required non-live `2371/2371`、隔离 integration `119/119` 和 Chromium `49/49`；Prompt/Dify 页面、四条业务 Worker、错误恢复与移动端均在同一发布门禁中通过。多租户许可证书面确认仍是非阻塞外部项，当前仅运行单一运营 workspace。
- [x] `IMPL-NONB-MIGRATION-CUTOVER-PACK-07-LOCAL-2026-07-25` 隔离 PostgreSQL 双写演练固定 Prompt/Protocol/Observation/Metric 两类旧 writer，真实验证 16 条 initial backfill、backfill 中增量追尾、new-side failure 的全事务回滚、advisory cutover lock 对第二连接的阻断、watermark `16 -> 17 -> 19`、两轮逐 scope 零差异、rollback-window 双写和 contract 后旧 writer 拒绝；收据 schema、源码/hash 重检、`0600` 与临时 schema 清理均由自动测试覆盖。该本地通用演练不冒充真实遗留业务数据 cutover，故 `M6-MIG-01/02`、`M6-AC-03` 和 `REPO-GATE-03` 仍等待真实数据及独立 verifier。
- [x] `IMPL-NONB-AUTHENTICATED-RESTORE-PACK-07-LOCAL-2026-07-25` 最终认证恢复 Gate 从空环境恢复至 `0087_wfc_report_receipts`，核对 234/234 张表、109 个非 B 关系、FK、ACL/RLS、SQL ledger 87 项和五桶 12/12 个对象。Secret Store 两条代表性 Secret、Provider 两个工件、Recommendation 一个真实生成任务工件、Workflow C 一个真实 approved manual artifact 及 Synthetic restricted/tier 工件均以历史 key version `1,2` 解密；十种错误/缺失 key/HMAC 路径拒绝，bundle/receipt 为 `0600`，临时明文、恢复副本、临时 key 和 Compose 资源均清理。最终 receipt 为 `artifacts/backup-restore-smoke-authenticated/20260725T035704Z-619020/receipt.json`，文件 SHA-256=`e25deddf96b97adcd19e0cdc8e947ea4d82114867a58b457307bba86553286c5`，内部 manifest SHA-256=`30350c7dedd8913f2addb217f287ccc423a3b6d4c681229035c75d9c2e8be1e4`。生产 key custodian、生产凭据/数据和独立签字仍未提供，最终 `M6-RESTORE-01` 与 `REPO-GATE-07` 不提前勾选。
...
- [x] `IMPL-B-SEARCH-AGGREGATION-PROTOTYPE-RETIRED-2026-08-07` 旧 SerpAPI Google AI Overview/Bing Copilot 即时路由、provider registry/canary、专用 adapter 和无密钥 mock fallback 已从当前 Internal API、Compose、OpenAPI 和运行源码删除；`0130_serpapi_secret_purpose` 只作为不可改写的历史迁移保留。OpenRouter/Perplexity API 仍按真实 Provider 身份保留，消费者界面统一进入 Project-scoped Browser Capture/Sampling，不再并存两套正式路径。
...
- [x] `IMPL-B-CONSUMER-BROWSER-CAPTURE-LOCAL-2026-08-07` AIO、AI Mode、Copilot 已使用三个独立内置 Release 与 Playwright/Chromium 版本；Admin 可配置 LokiProxy provider-managed residential/mobile 澳洲粘性池、加密保存 Secret Reference、执行出口测试、可选导入受限域名的登录 `storage_state`，并逐 Surface 注册匹配的 runtime option、授权策略、冻结 QuestionSet 和采样输入。`0124` 仅为 Browser 配置 Secret 开放 owner enablement，`0125` 在同一事务按 Run/Task 创建 Attempt、Browser Job、immutable spec、outbox 和预留；`0131` 增加 LokiProxy pool health/cooldown、产品和并发合同，并以项目级事务锁原子创建/切换唯一池。同一配置命令重放保留既有健康状态，失败后的界面重试会创建新出口测试而不是重放旧结果；每次 Attempt 使用唯一 session，实际 verification 只进入 Attempt/Observation lineage。Browser/Sampling 回归、稳定 OpenAPI、Admin typecheck、production build、PostgreSQL 纵向测试和 Chromium 桌面/移动交互通过；SerpAPI 不再是运行入口，当前 LokiProxy pool 与 live Capture 均为 0，Outbox Relay 与 Browser Worker 均在受跟踪运行单元中。
- [x] `IMPL-CANONICAL-GEO-CUTOVER-LOCAL-2026-08-10` 本机唯一运行项目已收敛为 canonical `geo`：13 个 GEO 服务与独立 Dify 运行组保持 `13001/13000/18000/18001/55433/19000/19001/16379/15000` 兼容入口；旧 staging 容器已移除，旧卷只保留回滚用途。克隆后的业务计数为 Project 2、Fact Candidate 941、Question Candidate 303、Durable Job 198，Dify 为 10 个 App/37 个 Workflow。2026-08-10 再以干净提交 `655a327bf8f753b0ea43caf915728fe6172d6c63` 重建，PostgreSQL head 为 `0131_lokiproxy_pool`；release receipt 保存源码/声明/运行 SHA、每个镜像 content address 和服务清单，`doctor` 已验证全部一致。LokiProxy Pool、Capture Session 与 Capture Job 实查仍为 0。该勾选只证明本机 canonical cutover，不替代新主服务器空环境恢复、历史 keyring canary 或生产外部输入验收。
- [ ] `IMPL-B-CONSUMER-BROWSER-CAPTURE-LIVE-AU-2026-08-07` 等待用户提供真实 LokiProxy residential/mobile provider-managed sticky pool 后，逐 Surface 执行 3 次 canary，核对同一 session lease 的 pre/target/post IP/ASN/AU 证明、页面类型、答案、引用、DOM/screenshot/HAR 与 Observation；全部通过后再从已冻结 SAT30 exact-100 v2 选择 10 题 pilot，验收通过后才扩展真实测量。当前 3 Surface Release、1 匿名 Profile、0 LokiProxy pool、0 live Capture，故本项和 `REAL-EXT-AC-03`、`EXT-UI/EGR-*`、最终 Gate 均不得勾选。AIO、AI Mode、Copilot 的成功、有效缺失和阻断分母必须分别冻结、分别验收。
...
- [x] 当前认证恢复覆盖更新：`0093_dify_workflow_runtime`、238 张表、110 个非 B 关系、93 个迁移 checksum、五桶 12 个对象和历史 keyring 均已在空环境恢复；新 receipt SHA-256=`d0d446c3a2f58beba61a24169019217990778e8fff74ca001f71e32fc92251db`，并取代上方 `0087` 收据作为当前 Gate 证据，上方收据只保留为历史基线。
- [x] 静态、单元、隔离 PostgreSQL/MinIO/Valkey、OpenAPI、双 Web build、Chromium 与故障/恢复演练的本地 Gate 全部完成；实际计数、明确排除项和 artifact hash 已写入 Pack 08 manifest/ledger，真实 live 与独立签字继续阻断最终 `ACCEPTED`。
- [x] 非 B 功能已完成自主交叉复查、权限/安全/正确性检查和 Admin/Customer 易用性修正；外部独立 verifier 仍列入统一请求，仓库根目录的临时持续执行指令可在证据文件冻结后删除。
- [x] `IMPL-DIFY-SOLE-PROMPT-SOURCE-2026-07-27` 四条核心链已删除运行请求中的 `geo_prompt_system`/`geo_prompt_user`，Prompt、模型和流程变量只在 Dify 修改；GEO 仅传结构化上下文、输入/上下文 hash、输出合同和用途。`0094_dify_published_snapshot` 以 append-only Project/RLS 投影保存发布 Prompt、输入变量、图节点与 hash，每次 Attempt 固定 snapshot ID 并校验响应 workflow ID。Admin “Dify 工作流”页只读展示四条真实发布内容，其他十类本地 Prompt 隐藏并登记为 backlog。live staging 四条真实 DeepSeek canary、Dify API 停止时四张缓存卡和 fail-closed canary、恢复后四张 `current` 卡均通过；22 项定向 Python 与 2 项 Chromium 桌面/移动回归通过。工作流模板、项目级选择、多国家/locale 和其余十类迁移为后续项，当前不迁移 GEO 知识库。
- [x] `IMPL-DIFY-0094-FINAL-GATE-2026-07-27` 已在 0094 后重新完成认证空环境恢复和完整发布 Gate：required non-live `2376/2376`、integration `119/119`、OpenAPI `7/7`、双 Web build、Chromium `46/46`、infra contracts `71/71`、production network `2/2`、runtime `7/7`，全部零失败、零 skip；`make quality` 同时通过 Ruff、764-source MyPy、六 Web typecheck、2,076-file Secret scan 与 architecture `43/43`。空环境恢复核对 239 张表、110 个非 B 关系、94 个 migration checksum、五桶 12 个对象及 ACL/RLS/历史 keyring，receipt SHA-256=`7ace2d57d36ad57c274a277676b5dd082e7d15b8a47f57184af7da7232392659`、manifest SHA-256=`6a3258afd15943e72b891a60181f2fdda9b0273419162b68820213935ea1db13`。四条最终 Dify Release、真实 DeepSeek canary、业务 Job、断开 fail-closed 和恢复均已验证；该勾选只表示本地可控 Gate 完成，生产等价性能、外部真实账号/样本和独立 verifier 仍保持阻塞。
- [x] `IMPL-DIFY-TEST-RUN-AUTO-INPUT-2026-07-27` Dify Test Run 已完成 `geo-job:` 历史过滤、最新运行自动注入、旧运行选择、五字段白名单、技术字段折叠/解锁、固定 commit patch、自定义 Web 镜像和官方镜像回退合同。Dify 定向单测 `10/10`、干净 TypeScript typecheck、三文件静态检查和 Next.js/vinext production build 均通过；live browser 对四条真实工作流分别识别 `2/2/2/3` 条业务历史，逐条验证自动选择、切换、五字段编辑和折叠，workflow-run 请求零失败。官方 `langgenius/dify-web:1.16.0` 实际回退健康，自定义镜像恢复后四条复验再次通过，当前运行镜像 label=`7c379f5e1814faaf0d7c80d21cd7154fca3ef381d19506273ec68681a14ed0ba`。
- [x] `IMPL-DIFY-SYNTHETIC-REVIEW-CLOSED-LOOP-2026-07-27` 四个 DSL、Purpose/API/Admin 目录、fail-closed 混合路由、双后端 child lineage、Dify 结果原子持久化/重放及 `0095` up/down/up 均已完成；staging head 为 `0095`，四个 Dify/Prompt binding 和四类真实 DeepSeek canary 已核对。正向 Job `4426834e-3e3b-5227-adcc-5b475c89031b` 成功且 Worker 重启前后 Dify 结果保持 `9 -> 9`；负路径 Job `9e3e79d3-32bc-5dc2-87ff-ab52f5fa26ba` 完成中断恢复、两轮 revision 和一次 regenerate，并在后续非法输出处 fail closed。该勾选只表示本迁移闭环和故障路径完成，不替代九渠道真实样本、三家模型、生产等价性能或独立 verifier。
...
- [x] `IMPL-SYNTHETIC-ADMIN-WORKSPACE-2026-08-03` 合成测评实验室已按连续操作拆成七个中文视图，首页提供五项准备度和唯一主入口，生成页只要求选择冻结套件/Case/运行时/阈值，结果页读取 Project-scoped Durable Job 及其最终文案、冲突和修订记录；语料、风格、套件和授权低频操作分别隔离。旧 `synthetic_job_id`、`synthetic_import_preview_id`、`synthetic_suite_id` 链接继续兼容，人工导入依赖失败只在风格资源页显示可行动 503。7 项 API、8 项 Synthetic Chromium、2 项运行地图 Chromium、Ruff、TypeScript、production build、稳定 OpenAPI 和 `13001/18000` live desktop/mobile 均通过。此项只勾选页面、读取合同和部署可用性；ADVINSYS 仍缺合法批准样本、verified frozen Profile 和 frozen Suite，真实生成 Job/Result 为 0，故不勾选 `M2-SYN-01/02/04`、`M3-SYN-01` 或最终业务 Gate。
- [x] `IMPL-SYNTHETIC-SUITE-INPLACE-UX-2026-08-03` 测评套件创建动作在写入及路径失效后通过 Server Action 单次 303 打开新版本，客户端不再持有会重复执行的成功导航 effect；列表切换使用 `scroll={false}` 的 Next Link，已选行不再显示可重复点击的伪“打开”操作。8 项前端静态合同、Admin typecheck及状态化 Chromium 桌面/390px 回归通过；回归明确验证“创建中”消失、按钮重新可用，并在稳定窗口内检查请求计数不再增长。真实 staging 验收与最终镜像摘要记录在第 86 次修订，不改变冻结套件和真实生成 Gate。
- [x] `IMPL-SYNTHETIC-DIRECT-WORKBENCH-2026-08-04` 合成测评主路径已改为同页填写、创建、轮询、历史切换和结果阅读；九渠道手工风格按版本冻结，ADVINSYS V600/V1000/SAT30 各自仅从当前、已验证 lineage 的 `approved_fact` 构建知识快照。真实浏览器提交 Job `f26cf4e7-8358-5f0f-9a75-272fbe8f8837` 并取得 `completed_with_warning` 结果，4 个候选均完成声明提取、冲突检查、风格判断和仲裁，最终文案显式关联两条 V600 批准事实且没有主体串用；最终镜像部署后又通过 `20c8c73b-33e5-53de-b8c7-17fd1c2b0697` 和 `07994a63-f612-52c1-b77e-2929ceb5e240` 两条真实点击任务验证自动重试与终态恢复，当前共 4 条真实成功 `review.case.run`。`327/327` Python/架构、`6/6` Synthetic/运行地图 Chromium、稳定 OpenAPI 和 production build 均通过。未覆盖推演以 warning 分层而非伪造 Fact；旧七视图和正式 Suite/Profile/Corpus 操作不再进入主导航。本项不替代每渠道 200 样本、正式 Profile、360 Case、真实离线实验或人工发布 Gate。
- [x] `IMPL-GEO-ENCRYPTED-GITHUB-MIGRATION-2026-08-04` 本机到新服务器的迁移合同已落地：`geo-runtime-migration-v2` 在 gzip 后以 GPG AES-256/SHA-512 加密，覆盖 GEO/Dify 两套 PostgreSQL、MinIO、Valkey/Redis、Weaviate、Dify `app/storage`、`plugin_daemon`、state 和排除迁移密码后的 Secret 树；manifest 固定来源 commit、环境、角色、payload hash 和文件级 hash。`scripts/geo_sync.py` 使用私有 GitHub Release 作为加密分片传输，基线导入要求空目标，切换后只接受 production `primary` 的父链 delta，并在临时目录物化后以显式 `--overwrite-test-replica` 覆盖本机测试副本；父归档 ID、循环、路径穿越、错误密码、非 0600 密码文件均 fail closed。`15 passed` 迁移/堆栈合同、Ruff 和物化增量回归已通过，Make、部署 manifest、迁移手册和运行地图已同步。2026-08-10 已从干净 commit `1877b29d779dcbc102d44ba03d6e23b16e91b6b7` 短暂停写导出 canonical `geo` 与 Dify 全量数据，生成 27 项、28,863,108 bytes 的加密 payload，上传到私有 Release `geo-migration-geo-runtime-20260810T031448Z`；随后从 GitHub 重新下载、重组、核对分片/hash 并完成真实解密验证，payload SHA-256=`22cea592a2ac0f62d19f724357ce9c2447a8b11034937ceb81ef1f6f12a543a1`。写入服务已自动恢复且 canonical doctor 再次通过。尚未执行新主服务器空环境导入、历史 keyring/Secret 业务 canary 或 production primary→test-replica live 增量演练，因此本项仍为 `LOCAL_COMPLETE / BLOCKED_EXTERNAL`，不勾选 `PLAN-FINAL-14/15/17/19` 或最终 `GO`。
- [x] `IMPL-MEASUREMENT-QUESTION-WORKSPACE-2026-08-04` 测试问题完整编辑链已归入 `测量与告警 -> 测试问题`，可按 Campaign 生成、选择任务、审核候选、建立/批准/冻结 QuestionSet 并绑定 draft 监测方案；旧 GEO 深链保留自动迁移，GEO 只读消费冻结版本并进入内部仿真。当前真实 ADVINSYS SAT30 页面可读取原 v1（98 条）和 exact-100 v2（100 条）两个冻结版本，v2 展开后显示 100 个问题与来源；V600/V1000 候选仍等待业务审核。桌面 1440px 与移动 390px 的 canonical `13001` 实际渲染均无横向溢出、控制台错误或路由异常。本项只关闭页面归属、兼容和可用性，不替代真实采样或最终业务 Gate。
- [x] `IMPL-MEASUREMENT-QUESTION-COVERAGE-100-2026-08-06` 一键 100 题纵向路径已闭合：服务端从 Campaign 当前产品、官方 URL 和当前批准 Fact 构建固定覆盖计划，按 10 个可恢复批次执行，前端提供搜索、分层、主题过滤、逐题编辑、全选/全排除以及 exact-100 后继冻结路径。`0123_question_semantic_dedup` 令应用与 PostgreSQL 对重复语义指纹的判断一致；每个模型槽位在调用前分配确定性 `grounding_fact`，应用忽略模型臆造的引用 ID。专用恢复命令幂等保留 checkpoint，并允许死信任务在总上限 6 次内一次增加一个人工尝试。真实 V600 Job `0b4f68e1-03ad-5b48-a7e7-71d207bba1ad`、V1000 Job `4d528cc7-b703-559c-994a-8edf9a2a9b49` 和 SAT30 Job `b41101f3-0d48-5757-a5de-b824ec5d54a2` 均完成 100 条不同文字、`50/40/10` 配额和零 Fact 越界；V600 从 2/10 checkpoint 经两次可行动恢复完成，证明恢复不是重新提交伪状态。SAT30 两条待修题已经正式编辑并保留 revision lineage；原 v1 以 98 条冻结且保持不可变，同一 series 的 v2 `5b1583c1-efe6-5a01-8025-6709f07dbf46` 已由服务端冻结恰好 100 条，覆盖率 1、重复数 0。V600/V1000 的 200 条候选仍为 `pending_review`，SAT30 v2 也尚未绑定真实 Sampling Suite 或形成 Observation；因此这里只关闭生成、恢复、编辑和 SAT30 exact-100 冻结，不替代真实采样或最终业务 Gate。
