...
> 计划日期：2026-07-21
> 修订日期：2026-08-10（第 91 次修订）
> 计划状态：`LOCAL_PATHS_COMPLETE / BLOCKED_CANONICAL_CUTOVER_AND_EXTERNAL_INPUT`；B 板块本地 MVP 已恢复实施，唯一 canonical 栈尚未切换，真实外部验收仍按专项计划阻断
...
| `IMPL-B-CONNECTOR-ATTRIBUTION` Connector Core、GSC/GA4、官方报告、消费者 UI capture、事件入口、归因账本 | `LOCAL_COMPLETE / BLOCKED_EXTERNAL` | `0102--0126` 已实现 Connector Core/PyAirbyte GSC/GA4 Worker、scope 校验与 canary/readiness 命令、外部报告批准投影、Session→Revenue 账本，以及 AIO、AI Mode、Copilot 三个内置 Browser Capture Release、加密澳洲代理/可选登录会话、逐 Attempt sticky pre/target/post 证明、逐 Surface 采样输入和原子批量入队。`0126` 对超过 10 条冻结输入持久化恰好 10 个唯一 `question_set_item_ids`，Task 只从该选择派生。正式 SerpAPI `provider_api` registry、Secret Store handle、错误分类和 canary CLI 已接入，但尚无 `search.serpapi` Secret reference，真实 Provider Observation 仍为 0。ADVINSYS staging 已安装 3 个 approved Surface Release 与 1 个匿名 `en-AU` Profile；Worker、Outbox、API 和 Admin 运行，当前仍为 0 Egress Endpoint、0 Capture | 提供真实 GSC/GA4/Provider 凭据、澳洲 residential/mobile sticky proxy、逐 Surface live canary、真实业务 Revenue 旅程及独立 verifier，按专项文件完成 live Gate；技术 bootstrap、scope/canary 和本地测试不得算业务结果 |
...
- [x] `IMPL-MEASUREMENT-QUESTION-WORKSPACE-2026-08-04` 测试问题完整编辑链已归入 `测量与告警 -> 测试问题`，可按 Campaign 生成、选择任务、审核候选、建立/批准/冻结 QuestionSet 并绑定 draft 监测方案；旧 GEO 深链保留自动迁移，GEO 只读消费冻结版本并进入内部仿真。真实 ADVINSYS 页面继续显示 Job `c974d38d-beaf-5a38-bd8e-b1d7992682c5` succeeded、3 条候选 pending_review、0 QuestionSet；未执行未经授权的业务审核。29 项静态契约、12 项 GEO Chromium、6 项 Workflow C Chromium、2 项真实运行地图 Chromium、TypeScript、production build 和 `13001` 桌面/390px 只读验收均通过，操作手册、真实截图与运行地图已同步。本项只关闭页面归属、兼容和可用性，不替代候选人工批准、QuestionSet 冻结、真实采样或最终业务 Gate。
- [x] `IMPL-MEASUREMENT-QUESTION-COVERAGE-100-2026-08-06` 一键 100 题纵向路径已闭合：服务端从 Campaign 当前产品、官方 URL 和当前批准 Fact 构建固定覆盖计划，按 10 个可恢复批次执行，前端提供搜索、分层、主题过滤、逐题编辑、全选/全排除以及 exact-100 后继冻结路径。`0123_question_semantic_dedup` 令应用与 PostgreSQL 对重复语义指纹的判断一致；每个模型槽位在调用前分配确定性 `grounding_fact`，应用忽略模型臆造的引用 ID。专用恢复命令幂等保留 checkpoint，并允许死信任务在总上限 6 次内一次增加一个人工尝试。真实 V600 Job `0b4f68e1-03ad-5b48-a7e7-71d207bba1ad`、V1000 Job `4d528cc7-b703-559c-994a-8edf9a2a9b49` 和 SAT30 Job `b41101f3-0d48-5757-a5de-b824ec5d54a2` 均完成 100 条不同文字、`50/40/10` 配额和零 Fact 越界；V600 从 2/10 checkpoint 经两次可行动恢复完成，证明恢复不是重新提交伪状态。新增后继路径保留原始疑似重复证据，要求修订所有待修题并由服务端确定性复核后只接受恰好 100 条；13 项覆盖单元/迁移、1 项 PostgreSQL 纵向集成、7 项 OpenAPI、2 项关键 Chromium、Admin/API-client typecheck、production build 和真实 `13001` 1440px/390px 渲染均通过。该勾选只关闭生成、恢复、编辑和候选可用性；300 条候选仍待业务确认，未自动冻结 QuestionSet、绑定监测方案或进入采样分母。
