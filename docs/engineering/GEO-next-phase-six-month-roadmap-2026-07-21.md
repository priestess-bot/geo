...
| `IMPL-B-CONNECTOR-ATTRIBUTION` Connector Core、GSC/GA4、官方报告、消费者 UI capture、事件入口、归因账本 | `LOCAL_COMPLETE / BLOCKED_EXTERNAL` | `0102--0125` 已实现 Connector Core/PyAirbyte GSC/GA4 Worker、外部报告批准投影、Session→Revenue 账本，以及 AIO、AI Mode、Copilot 三个内置 Browser Capture Release、加密澳洲代理/可选登录会话、逐 Attempt sticky pre/target/post 证明、逐 Surface 采样输入和原子批量入队。ADVINSYS staging 已安装 3 个 approved Surface Release 与 1 个匿名 `en-AU` Profile；Worker、Outbox、API 和 Admin 运行，当前仍为 0 Egress Endpoint、0 Capture | 提供真实 GSC/GA4/Provider 凭据、澳洲 residential/mobile sticky proxy、逐 Surface live canary、真实业务 Revenue 旅程及独立 verifier，按专项文件完成 live Gate；技术 bootstrap 和 canary 不得算业务结果 |
...
- [x] `IMPL-MEASUREMENT-QUESTION-COVERAGE-100-2026-08-06` 一键 100 题纵向路径已闭合：服务端从 Campaign 当前产品、官方 URL 和当前批准 Fact 构建固定覆盖计划，按 10 个可恢复批次执行，前端提供搜索、分层、主题过滤、逐题编辑、全选/全排除以及 90--100 条冻结门槛。`0123_question_semantic_dedup` 令应用与 PostgreSQL 对重复语义指纹的判断一致；每个模型槽位在调用前分配确定性 `grounding_fact`，应用忽略模型臆造的引用 ID。专用恢复命令幂等保留 checkpoint，并允许死信任务在总上限 6 次内一次增加一个人工尝试。真实 V600 Job `0b4f68e1-03ad-5b48-a7e7-71d207bba1ad`、V1000 Job `4d528cc7-b703-559c-994a-8edf9a2a9b49` 和 SAT30 Job `b41101f3-0d48-5757-a5de-b824ec5d54a2` 均完成 100 条不同文字、`50/40/10` 配额和零 Fact 越界；V600 从 2/10 checkpoint 经两次可行动恢复完成，证明恢复不是重新提交伪状态。13 项覆盖单元/迁移、1 项 PostgreSQL 纵向集成、7 项 OpenAPI、2 项关键 Chromium、Admin/API-client typecheck、production build和真实 `13001` 1440px/390px 渲染均通过。该勾选只关闭生成、恢复、编辑和候选可用性；300 条候选仍待业务确认，未自动冻结 QuestionSet、绑定监测方案或进入采样分母。
