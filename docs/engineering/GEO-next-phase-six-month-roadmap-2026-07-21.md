...
> 第 91 次修订同时记录外部适配器就绪边界：正式 SerpAPI `provider_api` registry、Secret Store handle、错误分类和 canary CLI 已通过本地合同，但当前没有 `search.serpapi` Secret reference，真实 Provider Observation 为 0；GSC/GA4 已有 scope 校验和 canary/readiness 命令，但当前没有真实 property 凭据、Connection 或 Sync。canonical `geo` Compose 尚未从当前 `geo-advinsys-staging-v2` 切换，不能用旧容器健康或本地 fixture 代替发布回执。
> 第 92 次修订变更：唯一 canonical Compose 项目 `geo` 已在兼容端口启动，旧运行容器已移除、旧卷只作为回滚副本保留；PostgreSQL、MinIO 与 Valkey 数据克隆经计数和 hash 对账，Alembic head 为 `0130_serpapi_secret_purpose`。启动验收发现 Connector Worker 镜像误打包宿主机嵌套 `.venv`，现已从 Docker context 排除，在镜像构建时验证 `dramatiq`、`psycopg` 和入口模块可导入，并由 GitHub deployment Gate 实际构建该镜像；`geo-stack doctor` 同时绑定 canonical Compose 文件集合、工作目录与 `oneoff=False` 身份，强制核对全部必需服务正在运行、声明健康的 GEO/Dify 服务实际为 `healthy`、迁移与 MinIO 初始化以 exit 0 完成、数据库实际 Alembic head 与源码一致，以及业务镜像 release identity。ADVINSYS SAT30 原冻结 v1（98 条）保持不变，两条疑似重复已通过正式编辑 API 形成 revision lineage，服务端随后冻结同一 series 的 exact-100 v2 `5b1583c1-efe6-5a01-8025-6709f07dbf46`，100 个维度全部覆盖、重复数 0、content hash=`2d8ecaf6f3bee65bda3d5eea46c8da278f3b9589ab64482e7a9d6c11fa62ad15`。这只使 10 题 pilot 有了合法冻结输入；SerpAPI Secret、GSC/GA4 property 凭据、澳洲 sticky proxy、真实 Observation/Sync/Capture、新主服务器恢复和真实 Revenue 旅程仍未完成。
...
| `IMPL-B-CONNECTOR-ATTRIBUTION` Connector Core、GSC/GA4、官方报告、消费者 UI capture、事件入口、归因账本 | `LOCAL_COMPLETE / BLOCKED_EXTERNAL` | `0102--0130` 已实现 Connector Core/PyAirbyte GSC/GA4 Worker、scope 校验与 canary/readiness 命令、外部报告批准投影、Session→Revenue 账本，以及 AIO、AI Mode、Copilot 三个内置 Browser Capture Release、加密澳洲代理/可选登录会话、逐 Attempt sticky pre/target/post 证明、逐 Surface 采样输入和原子批量入队。`0126` 对超过 10 条冻结输入持久化恰好 10 个唯一 `question_set_item_ids`，Task 只从该选择派生。正式 SerpAPI `provider_api` registry、Secret Store handle、错误分类和 canary CLI 已接入，但尚无 `search.serpapi` Secret reference，真实 Provider Observation 仍为 0。canonical `geo` 已安装 3 个 approved Surface Release 与 1 个匿名 `en-AU` Profile；Worker、Outbox、API 和 Admin 运行，当前仍为 0 Egress Endpoint、0 Capture | 提供真实 GSC/GA4/Provider 凭据、澳洲 residential/mobile sticky proxy、逐 Surface live canary、真实业务 Revenue 旅程及独立 verifier，按专项文件完成 live Gate；技术 bootstrap、scope/canary 和本地测试不得算业务结果 |
...
> 当前状态校正（2026-08-10）：上表各长行保留其形成时的实现证据，当前部署身份以第 92 次修订和本节末尾 checklist 为准。唯一运行项目为 canonical `geo`，Alembic head 为 `0130_serpapi_secret_purpose`；SAT30 exact-100 v2 已冻结，Connector/Browser Worker 已运行。SerpAPI/GSC/GA4/AU proxy 的 live 数据仍全部为 0，新主服务器恢复也未执行，故任何外部或最终 Gate 都不因此改变。
...
- [x] `IMPL-B-SEARCH-AGGREGATION-PROTOTYPE-RETIRED-2026-08-07` 旧 SerpAPI Google AI Overview/Bing Copilot 即时路由和无密钥 mock fallback 已从 Internal API、Compose 与示例环境删除；稳定 OpenAPI 明确拒绝四条旧路由。历史 parser/adapter 源码只保留为离线解析回归，不是生产调用入口。OpenRouter/Perplexity API 仍按真实 Provider 身份保留，消费者界面统一进入 Project-scoped Browser Capture/Sampling，不再并存两套正式路径。
...
- [x] `IMPL-B-CONSUMER-BROWSER-CAPTURE-LOCAL-2026-08-07` AIO、AI Mode、Copilot 已使用三个独立内置 Release 与 Playwright/Chromium 版本；Admin 可安装采集器、加密保存 residential/mobile 澳洲粘性代理、执行出口测试、可选导入受限域名的登录 `storage_state`，并逐 Surface 注册匹配的 runtime option、授权策略、冻结 QuestionSet 和采样输入。`0124` 仅为 Browser 配置 Secret 开放 owner enablement，`0125` 在同一事务按 Run/Task 创建 Attempt、Browser Job、immutable spec、outbox 和预留；单次 technical smoke 可用 repetition=1，但统计最低仍为 3。175 项 Browser/Sampling 回归、18 项会话/配置回归、稳定 OpenAPI、Admin typecheck、production build、PostgreSQL 纵向测试和 Chromium 桌面/移动交互通过；当前 canonical `geo` head 为 `0130_serpapi_secret_purpose`，Outbox Relay 与 Browser Worker 均在受跟踪运行单元中。
...
- [ ] `IMPL-B-CONSUMER-BROWSER-CAPTURE-LIVE-AU-2026-08-07` 等待用户提供真实 residential/mobile 澳洲 sticky proxy 后，逐 Surface 执行 3 次 canary，核对同 lease 的 pre/target/post IP/ASN/AU 证明、页面类型、答案、引用、DOM/screenshot/HAR 与 Observation；全部通过后再从已冻结 SAT30 exact-100 v2 选择 10 题 pilot，验收通过后才扩展真实测量。当前 3 Surface Release、1 匿名 Profile、0 Egress Endpoint、0 Capture，故本项和 `REAL-EXT-AC-03`、`EXT-UI/EGR-*`、最终 Gate 均不得勾选。
