...
- Commit `655a327bf8f753b0ea43caf915728fe6172d6c63` is the LokiProxy implementation baseline used for the first canonical rebuild. The local v3 release receipt is the authority for the current exact release commit and reports a clean source, exact GEO/Dify Compose identity, all required running/healthy/completed services, database head `0131_lokiproxy_pool` and content-addressed images; source, declared and runtime release commits must match. `0130_serpapi_secret_purpose` is retained only as historical migration evidence. This proves the LokiProxy pool contract is deployed, while the actual pool and live Capture counts remain `0/0`.
