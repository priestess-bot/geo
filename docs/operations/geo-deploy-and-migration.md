...
GPG 和 `uv`。服务器只需内网访问，不要求本项目自带 HTTPS 网关。默认端口保持当前行为：
...
export GEO_MIGRATION_KEY_FILE=/absolute/path/to/migration-passphrase
export GEO_MIGRATION_SECRET_ROOT=/home/ymm/ym/gz/20260608-geo/artifacts/staging-secrets
export GEO_MIGRATION_OUTPUT_ROOT=/srv/geo-migrations
...
export GEO_MIGRATION_PACKAGE=/srv/geo-migrations/geo-runtime-<timestamp>

./scripts/geo-stack.sh import \
  --package "$GEO_MIGRATION_PACKAGE" \
  --encryption-key-file "$GEO_MIGRATION_KEY_FILE" \
  --secret-root "$GEO_MIGRATION_SECRET_ROOT" \
...
3. 恢复 GEO MinIO、Valkey、Dify Redis 和 Weaviate 数据；
4. 恢复 Dify state 和密钥文件；
