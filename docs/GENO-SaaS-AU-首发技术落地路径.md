...
成员邀请的状态闭环已进一步补齐到可撤销/可过期/可发送/可接受：`POST /v1/project-member-invitations/runtime/action` 支持 owner/admin 对 pending invitation 执行 `revoke` 或 `expire`，repository 使用 `FOR UPDATE` 锁定目标邀请，非 pending 状态返回冲突，并分别写入 `project_member_invitation_revoked` / `project_member_invitation_expired`。创建邀请时若没有现存 pending 记录会生成新的 invitation id，避免撤销/过期后再次邀请同一邮箱和角色时被旧 pending 稳定 id 阻塞；原始 `invite_token` 仍只在创建响应、邮件正文、浏览器 URL/表单和接受请求中短暂出现，不进入数据库或审计链。`POST /v1/project-member-invitations/runtime/email` 会用 raw token 生成 `/invite/accept?invitation_id=...&invite_token=...` 链接并通过 SMTP 发送，发送成功后只记录 `project_member_invitation_email_sent`、token hash、accept URL hash、SMTP host/port/from、response status 和 response body hash。`/invite/accept` 独立页面可读取邮件链接里的 token 并提交到 accept endpoint。接受邀请的 token-hash 最小兑换流也已落：`POST /v1/project-member-invitations/runtime/accept` 用原始 token 计算 hash，在 RLS 开启时先写入 `geno.runtime_invitation_token_hash`，只允许匹配且未过期的 pending invitation 被读取和兑换；repository 随后把邀请邮箱写入 `project_members`，把 invitation 标记为 accepted，并追加 `project_member_saved` 与 `project_member_invitation_accepted`。该能力补齐的是最小邮件发送、前端兑换和审计证据，不等同于邮件模板/退订/服务商 API、用户目录同步、注册资料采集或 SSO 登录会话。
