...
Runtime Notification email 反馈事实已补成最小 durable event：新增 `runtime_notification_email_feedback_events` 项目级 RLS 表，`POST /v1/runtime-notification-deliveries/{delivery_id}/email-feedback` 会校验 delivery 存在且 `channel=email`，接受 `bounce/complaint/unsubscribe/suppressed` 反馈类型、recipient 或 recipient hash、provider、provider event id 或 hash、metadata 与 recorded_by，然后写入反馈事件并追加 `runtime_notification_email_feedback_recorded` 审计。Runtime Console 在 email delivery 下提供 `Record feedback` 表单，用于把 SMTP/ESP 回执或人工复盘结论落入同一条 delivery/notification/subscription 证据链。该链路只保存 `recipient_hash` 和 `provider_event_id_hash`，不把 raw recipient 或 raw provider event id 写入表或 audit refs；它也不自动改写 `email_suppressed_recipients`、全局 suppression list 或用户偏好。托管邮件服务商 webhook 验签、自动退订 token、自动 suppression mutation 和跨项目用户目录仍属于 P1 后续增强。
