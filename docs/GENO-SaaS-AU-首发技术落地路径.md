...
Runtime Notification SMTP email 的模板审计也已补齐到最小版本化和控制入口口径：`channel=email` 的 queued delivery 会用 `runtime_notification_email_template_v2` 冻结 subject/text，并在 payload metadata 与 `runtime_notification_delivery_queued` output refs 中记录 template version/hash、subject hash、body hash、reply-to hash 和 control URL hash；订阅 metadata 可配置 `email_reply_to`、`email_unsubscribe_url`、`email_unsubscribe_mailto` 与 `email_preferences_url`，Runtime Console 表单已暴露这些字段。入队 payload 会生成 `Reply-To`、`List-Unsubscribe`、`List-Unsubscribe-Post` 和 `X-GENO-Notification-Preferences-Url`，正文追加 Notification controls 控制入口；`runtime_notification_subscription_saved` 也会记录 reply-to/control URL hash refs。worker 发送 SMTP `EmailMessage` 时透传 `X-GENO-Email-Template-Version` 和上述邮件头，但 email 通道不使用 webhook HMAC header。该能力是纯文本模板、SMTP 控制头和可复算审计，不等同于富模板编辑器、本地化、多语言、完整用户偏好中心、token 化一键退订服务、托管邮件服务商 API 或 bounce/complaint 处理。
