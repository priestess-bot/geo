...
Runtime Notification email 偏好中心后端已补到最小可解释读写闭环：当过滤后仅剩一个收件人且订阅 metadata 提供 `email_preferences_url` 时，入队 payload 会额外生成 `manage` HMAC tokenized preferences URL，正文和 `X-GENO-Notification-Preferences-Url` 使用该 URL，metadata 与 `runtime_notification_delivery_queued` output refs 只记录 `email_preference_manage_token_hash` 与 `email_tokenized_preferences_url_hash`；`GET /v1/runtime-notification-email-preferences/status` 用 manage token 读回当前 recipient hash 的 subscribed/unsubscribed 状态、脱敏 delivery/notification/subscription 摘要、suppression/token hash 计数和最近 subscription audit events；`POST /v1/runtime-notification-email-preferences/resubscribe` 用同一 manage token 移除当前 recipient hash 的 suppression，追加 `email_resubscribe_token_hashes`，并写 `runtime_notification_email_preference_resubscribed`。Runtime Console 已把 `Preference status API`、`Resubscribe API` 和 `Unsubscribe API` 一并暴露。该能力用于 AU MVP 的 hash-only 自助偏好后端和审计复盘；完整偏好中心页面、用户目录同步、跨项目偏好、全局 suppression list、ESP 原生 suppression API、富模板和本地化仍放在后续增强。
