...
P0b Google work item 的 env setup 现在必须包含 `chmod 600 .env.au-p0b-google`，execution checklist 的 summary 必须暴露 `env_file_hygiene_ready`、hygiene error/warning count，并通过 verifier 交叉校验。模板文件 `.env.au-p0b-google.example` 仍允许被 git 跟踪，因为它不承载真实 selector 或 secret；真实 env 文件存在且含条目时才触发 hygiene hard gate。
