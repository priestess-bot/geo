...
工程门禁已从单一 Python contract 扩展为本地/CI 同构验证：本地 `make ci-local` 与 GitHub Actions `contracts-and-runtime` 会执行 `make test`、`make web-build`、默认/LLM/scheduler/observability Compose config 校验和 `make runtime-e2e`。这证明当前代码在无真实外部 provider key 的 fixture、Postgres 和 MinIO 环境下可构建、可解析部署配置、可写入审计链/解释链、可归档报告 artifact，并可验证项目级 RLS 隔离；但它仍不替代完整 lint、真实 Perplexity/OpenAI/Google provider E2E、生产迁移起服、压测和发布流水线。
