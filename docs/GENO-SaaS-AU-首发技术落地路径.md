...
工程当前已落最小运行时闭环：`GET /v1/entity-aliases/runtime/candidates` 从 canonical name、官方域名、产品线和母公司字段生成计算型候选，并排除已确认 alias；同一 read model 还会读取项目内最近已入库的 `raw_answers.answer_text` 与 `answer_citations.domain/url`，用高置信规则生成 `evidence_answer_text` / `evidence_citation_domain` 候选，候选 payload 会带回 `evidence_count`、`evidence_answer_run_ids`、`evidence_urls` 与 `supporting_sources`，便于审核时回查原始 answer run 或 citation。若证据命中的是已有官方域名候选，系统会把证据合并为 supporting evidence，而不是生成重复候选。`POST /v1/entity-aliases/runtime/confirm` 可单条确认，`POST /v1/entity-aliases/runtime/confirm-batch` 可批量确认当前候选，默认先预校验实体存在与项目访问权限，要求同一批 alias 属于同一个 project，并避免失败批次半写入。批量响应返回 `entity_alias_confirm_batch_v1`、成功/失败计数、confirmed records 和 `entity_alias_batch_confirmed` 摘要；系统还会写入一条项目级 `entity_alias_batch_confirmed` 聚合审计事件，但真实主事实仍是每个 alias 的 `entity_alias_confirmed`，后续 `--persist-analysis` 使用确认后的 alias 参与 `rule_based_v2_aliases` parser。候选审核决策已新增最小持久化表 `entity_alias_candidate_reviews`：`POST /v1/entity-aliases/runtime/candidates/review` 可把 `needs_review/rejected/approved`、reviewer、notes 和 answer/citation 证据引用按 `project_id + candidate_id` 幂等保存，写入 `entity_alias_candidate_review_recorded` 审计事件；`POST /v1/entity-aliases/runtime/candidates/review-batch` 可对最多 25 条同项目候选做批量 needs-review / reject / approved，默认先预校验项目和实体，失败不半写入，逐条写 `entity_alias_candidate_review_recorded`，并追加 `entity_alias_candidate_batch_reviewed` 聚合审计摘要。候选列表会返回 `latest_review`，并默认隐藏已 `rejected` 的候选；`GET /v1/entity-aliases/runtime/candidates/reviews` 会按项目、decision、entity_kind、assigned_to、assignment_status、priority 和 due_before 分页返回审核历史、分配队列及最近审计事件，因此被隐藏的 rejected 候选仍可被复盘，已分配 reviewer 的待办也可被单独读取。`POST /v1/entity-aliases/runtime/candidates/assign` 会在既有审核记录上保存 `assigned_to/assigned_by/assignment_status/priority/due_at/assignment_note`，并写入 `entity_alias_candidate_assigned` 审计事件，用于把 reviewer owner、优先级、截止时间和 `assigned/in_progress/blocked/completed` 状态推进纳入可审计链路；`POST /v1/entity-aliases/runtime/candidates/assignment-action` 支持 reviewer 自助 `claim/release`，repository 会用 `FOR UPDATE` 读取当前 owner，claim 只允许领取未分配或本人已领取的 review，release 只允许本人释放，完成态 review 不允许被 claim/release，成功后写入 `entity_alias_candidate_assignment_actioned`。`GET /v1/entity-aliases/runtime/candidates/assignment-stats` 会基于同一张审核表按项目计算 `total_count/active_count/unassigned_count/overdue_count/due_soon_count`、`status_counts`、`priority_counts`、`oldest_due_at` 和 `next_due_at`，以 read model 形式暴露 assignment queue 健康度；该统计不新增可变状态，也不替代 `entity_alias_candidate_assigned` / `entity_alias_candidate_assignment_actioned` 这类动作审计。`POST /v1/entity-aliases/runtime/candidates/assignment-notifications` 会扫描 overdue 且仍处于 `assigned/in_progress/blocked` 的分配项，写入 `runtime_notifications.notification_type=entity_alias_assignment_overdue`，target 指向 review id，priority 为 urgent 时映射 critical，其余为 warning，并复用现有 `runtime_notification_subscriptions/runtime_notification_deliveries` 与 `runtime_notification_created/runtime_notification_delivery_queued` 审计。`workers/notification_worker/run_entity_alias_assignment_notifications.py`、`make entity-alias-assignment-notification-worker` 和 Compose `scheduler` profile 已提供 AU market 或显式 project id 的单次扫描入口。Runtime Console 的 Entity Alias 面板已提供单条确认、候选一键确认、`Bulk Alias Review Queue`、批量 mark needs review、批量 reject、逐条 needs review/reject、`Alias Candidate Assignment Queue`、`Alias Assignment Queue Stats`、`Claim review`、`Release review`、`Queue overdue assignment notifications`、`Alias Candidate Review History` 恢复/改判入口，以及从审核历史分配 reviewer / priority / due date、从 assignment queue 推进 Start / Block / Complete 的轻量表单，并展示 evidence rows / answer run / review audit / assignment audit / overdue stats 引用。该实现仍不是多级审批流、完整 reviewer workbench、完整 SLA 自动升级、值班升级、批量领取/释放、自动重分配或黑箱 NLP 自动消歧。
