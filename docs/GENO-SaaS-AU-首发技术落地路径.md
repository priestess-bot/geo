...
Runtime Notification SMTP email 的模板审计也已补齐到最小版本化、控制入口和 suppression 口径：`channel=email` 的 queued delivery 会用 `runtime_notification_email_template_v2` 冻结 subject/text，并在 payload metadata 与 `runtime_notification_delivery_queued` output refs 中记录 template version/hash、subject hash、body hash、reply-to hash、control URL hash、filtered recipient count 和 suppressed recipient hash；订阅 metadata 可配置 `email_reply_to`、`email_unsubscribe_url`、`email_unsubscribe_mailto`、`email_preferences_url` 与 `email_suppressed_recipients`，Runtime Console 表单已暴露这些字段。入队 payload 会生成 `Reply-To`、`List-Unsubscribe`、`List-Unsubscribe-Post` 和 `X-GENO-Notification-Preferences-Url`，正文追加 Notification controls 控制入口；partial suppression 会过滤被抑制收件人后继续入队，全部收件人被过滤时不创建空 delivery，而是写 `runtime_notification_delivery_suppressed` 审计事件；`runtime_notification_subscription_saved` 也会记录 reply-to/control/suppression hash refs。worker 发送 SMTP `EmailMessage` 时透传 `X-GENO-Email-Template-Version` 和上述邮件头，但 email 通道不使用 webhook HMAC header。该能力是纯文本模板、SMTP 控制头、metadata 驱动收件人过滤和可复算审计，不等同于富模板编辑器、本地化、多语言、完整用户偏好中心、token 化一键退订服务、托管邮件服务商 API 或 bounce/complaint 处理。

Runtime Notification email 反馈事实已补成最小 durable event 和只读复盘入口：新增 `runtime_notification_email_feedback_events` 项目级 RLS 表，`POST /v1/runtime-notification-deliveries/{delivery_id}/email-feedback` 会校验 delivery 存在且 `channel=email`，接受 `bounce/complaint/unsubscribe/suppressed` 反馈类型、recipient 或 recipient hash、provider、provider event id 或 hash、metadata 与 recorded_by，然后写入反馈事件并追加 `runtime_notification_email_feedback_recorded` 审计；`GET /v1/runtime-notification-email-feedback-events` 可按 project、delivery、notification、subscription、feedback type 和 provider 查询反馈事件，并带回 delivery、notification、subscription 与最近 audit events。Runtime Console 在 email delivery 下提供 `Record feedback` 表单，并展示最近 email feedback events，用于把 SMTP/ESP 回执或人工复盘结论落入同一条 delivery/notification/subscription 证据链。该链路只保存 `recipient_hash` 和 `provider_event_id_hash`，不把 raw recipient 或 raw provider event id 写入表或 audit refs；它也不自动改写 `email_suppressed_recipients`、全局 suppression list 或用户偏好。托管邮件服务商 webhook 验签、自动退订 token、自动 suppression mutation 和跨项目用户目录仍属于 P1 后续增强。
