...
外部依赖 dry-run 还必须绑定 request context，而不是只输出 step 和 next command。`make au-external-dependency-clearance` 生成的每个 step 都要包含 `linked_request_context`，固定 request context version、clearance step id、request artifact id/title、output path、hash field、artifact hash、file sha256、build command、verify command、strict gate command 和 runtime endpoint；同时每个 step 和顶层 current step 都要包含 `recommended_sequence`、`recommended_sequence_count` 与 `strict_gate_command`。当前 `p0a_provider_credentials` 绑定 `p0a_credential_request`，从 `make au-p0a-credential-request`、`make verify-au-p0a-credential-request` 开始，随后进入 P0a env template/bootstrap/runbook/env/checklist/status，并以 `verify_au_p0a_credential_request_packet.py --require-credentials-ready` 收束；`p0b_google_environment` 绑定 `p0b_google_environment_fulfillment`，从 `make au-p0b-google-environment-fulfillment`、`make verify-au-p0b-google-environment-fulfillment` 开始，并以 `verify_au_p0b_google_environment_fulfillment.py --require-fulfilled` 收束，避免 request packet ready 被误读成 selector/session/manual/DB env 已满足；`p0b_google_manual_backfill` 绑定 `p0b_google_manual_backfill_fulfillment`，从 `make au-p0b-google-manual-backfill-fulfillment`、`make verify-au-p0b-google-manual-backfill-fulfillment` 开始，并以 `verify_au_p0b_google_manual_backfill_fulfillment.py --require-fulfilled` 收束，避免 request packet ready 被误读成 120-row JSONL 已满足；`p0b_google_phase_execution` 绑定 `p0b_google_phase_execution_fulfillment`，从 `make au-p0b-google-phase-execution-fulfillment`、`make verify-au-p0b-google-phase-execution-fulfillment` 开始，并以 `verify_au_p0b_google_phase_execution_fulfillment.py --require-fulfilled` 收束，避免 phase execution request ready 被误读成 env/smoke/manual/health/full spike/main scoring 六阶段已经满足；`p0a_real_batches` 和 `customer_report_handoff_gate` 分别绑定 P0a real batch request 和 customer handoff readiness。verifier 必须拒绝 request artifact id/path/hash field/build/verify/strict/endpoint 漂移、recommended sequence 缺 request/fulfillment 命令、hard gate 缺 request/fulfillment verifier 或 strict gate 的 dry-run artifact。Runtime Console Clearance dry-run 卡片必须展示当前 request context、artifact hash、build/verifier/strict gate、endpoint 和前几条 recommended sequence，让执行者能从一个入口复盘“请求什么、怎么验证、按什么顺序清障、何时算完成”。当前 P0b phase fulfillment artifact 为 `status=pass`、`phase_execution_fulfillment_ready=true`、`phase_execution_fulfilled=false`、`next_phase=environment`、`p0b_google_phase_execution_fulfillment_hash=bdfc8ee82e592fba7c9801ab845ace98070382f026ef1fb9765872b8bae9e640`；刷新后的 external handoff/clearance hash 分别为 `a9498a27f37b56a016e999e3a4a3ab2bc967ce4fc0017b61b08fe548ad480a79` 与 `4331aaf25cda39125c72294d1b4497d1278e423bbdd19b7ada1ad166891248f1`。
