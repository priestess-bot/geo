...
P0b Google work item 的 env setup 现在必须包含 `chmod 600 .env.au-p0b-google`，execution checklist 的 summary 必须暴露 `env_file_hygiene_ready`、hygiene error/warning count，并通过 verifier 交叉校验。handoff dossier 会把同一组 P0b env-file hygiene ready/error/warning count 提升到顶层 summary 和 Markdown，Runtime Console 会在 P0b execution checklist 与 Handoff dossier 两处展示，确保交付复盘时不需要打开底层 env report 才能判断 Google secret 文件是否满足 gitignored/not tracked/0600。模板文件 `.env.au-p0b-google.example` 仍允许被 git 跟踪，因为它不承载真实 selector 或 secret；真实 env 文件存在且含条目时才触发 hygiene hard gate。
