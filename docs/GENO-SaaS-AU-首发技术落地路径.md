...
Runtime Notification SMTP email 的模板审计也已补齐到最小版本化口径：`channel=email` 的 queued delivery 会用 `runtime_notification_email_template_v1` 冻结 subject/text，并在 payload metadata 与 `runtime_notification_delivery_queued` output refs 中记录 template version/hash、subject hash 和 body hash；worker 发送 SMTP `EmailMessage` 时透传 `X-GENO-Email-Template-Version`，但 email 通道不使用 webhook HMAC header。该能力是纯文本模板和可复算审计，不等同于富模板编辑器、本地化、多语言、退订管理、托管邮件服务商 API、bounce/complaint 处理或用户偏好中心。
