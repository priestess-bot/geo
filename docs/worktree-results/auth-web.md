...
- Security review follow-up: `eb721da` (`fix(auth-web): harden invitation delivery recovery`)
...
- Raw invitation tokens are held only in component memory. Middleware on both surfaces detects the legacy `invite_token` query key without reading its value, redirects to an allowlisted canonical URL, and sets `Referrer-Policy: no-referrer`. The canonical page asks users for the separately delivered one-time code or a reissued invitation.
- Production counterpart portal URLs are server-only, mandatory, HTTPS, absolute, and free of userinfo. HTTP and fallback values are accepted only for loopback development.
- Admin project detail redirects an upstream 401 to `/login` and renders distinct generic 403, 404, and service-failure states without exposing backend detail. Auth rate-limit errors preserve the stable `auth_preflight_rate_limited` code and a sanitized `Retry-After` header on both BFF surfaces.
...
# 64 contract checks plus Chromium desktop/mobile interaction

git diff --check
python3 -m py_compile scripts/run_auth_surface_session_e2e.py tests/test_auth_web_contracts.py
```

The mock E2E proves concurrent-first-prepare convergence on both surfaces, randomized recovery ciphertext, old-Session/prepared no-op behavior, lost-response replay with byte-identical Session delivery, delivered Session-fingerprint rejection, SSR without CSRF confirmation, legacy-query canonicalization, logged-customer safe project preservation, 429 header clamping, Admin 401/403/404/5xx handling, fixed landing, surface projection and selection of the 201st customer project.
...
```text
/tmp/geo-auth-web-playwright-e_zmamnb/admin-surface-mismatch.png
/tmp/geo-auth-web-playwright-e_zmamnb/admin-desktop.png
/tmp/geo-auth-web-playwright-e_zmamnb/customer-mobile.png
```
...
- Configure ingress/CDN/proxy query logging to redact `invite_token`, then verify the redaction in production-like access logs.
...
- Rotating the recovery secret invalidates pending recovery Cookies and changes their deterministic HMAC replay key. Coordinate rotation with the backend redemption-ledger retention window.
