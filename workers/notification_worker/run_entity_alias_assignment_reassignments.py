...
WORKER_ID = "entity-alias-assignment-reassignment-worker"


class EntityAliasAssignmentReassignmentRepository(Protocol):
...
def _discover_project_ids(
    repository: EntityAliasAssignmentReassignmentRepository,
    *,
...
def reassign_entity_alias_assignment_reviews_for_projects(
    *,
    repository: EntityAliasAssignmentReassignmentRepository,
    assigned_to: str,
    project_ids: tuple[str, ...] = (),
    market_code: str | None = None,
    from_assigned_to: str | None = None,
    from_assignment_status: str | None = "escalated",
    ...
    reason: str = "entity alias assignment reassignment worker scan",
    ...
    if not selected_project_ids:
        return {
            "processed": False,
            "status": "idle",
            "reason": "no runtime projects matched alias assignment reassignment worker filters",
            "project_count": 0,
        ...
        try:
            record = repository.reassign_entity_alias_candidate_reviews(
                EntityAliasCandidateAssignmentReassignmentInput(
                    ...
                    from_assigned_to=from_assigned_to,
                    from_assignment_status=from_assignment_status,
            ...
            project_reassignment_count = int(payload.get("reassignment_count") or 0)
            project_skipped_count = int(payload.get("skipped_count") or 0)
            reassignment_count += project_reassignment_count
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--assigned-to", required=True, help="Target reviewer assignee.")
    parser.add_argument("--from-assigned-to", default=None, help="Optional current reviewer assignee filter.")
    parser.add_argument("--from-assignment-status", default="escalated", help="Optional current status filter.")
    parser.add_argument("--from-priority", default=None, help="Optional current priority filter.")
    parser.add_argument("--due-before", default=None, help="Optional ISO datetime current due date cutoff.")
    parser.add_argument("--assignment-status", default="assigned", help="Target assignment status.")
    parser.add_argument("--priority", default="high", help="Target priority.")
    parser.add_argument("--due-at", default=None, help="Optional ISO datetime target due date.")
    parser.add_argument("--worker-id", default=WORKER_ID, help="Actor id used in reassignment audit events.")
    parser.add_argument("--reason", default="entity alias assignment reassignment worker scan", help="Audit reason.")
    parser.add_argument("--limit-per-project", type=int, default=50, help="Maximum reviews to reassign per project.")
...
def main() -> None:
    ...
    try:
        ...
        result = reassign_entity_alias_assignment_reviews_for_projects(
            ...
            from_assigned_to=args.from_assigned_to,
            from_assignment_status=args.from_assignment_status,
            from_priority=args.from_priority,
            due_before=_parse_datetime(args.due_before),
