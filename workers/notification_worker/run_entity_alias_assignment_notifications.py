...
class EntityAliasAssignmentNotificationRepository(Protocol):
...
def _discover_project_ids(
    repository: EntityAliasAssignmentNotificationRepository,
    *,
...
def enqueue_entity_alias_assignment_notifications_for_projects(
    *,
    repository: EntityAliasAssignmentNotificationRepository,
    ...
    reason: str = "entity alias assignment notification worker scan",
    max_projects: int = 50,
    page_size: int = 50,
) -> dict[str, Any]:
    ...
    if not selected_project_ids:
        return {
            "processed": False,
            "status": "idle",
            "reason": "no runtime projects matched alias assignment notification worker filters",
            "project_count": 0,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--reason", default="entity alias assignment notification worker scan", help="Audit reason.")
