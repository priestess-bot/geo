...
三条硬约束在图中的体现：**开源优先**（基础设施全部开源自托管）、**松耦合**（上层只依赖下层接口、模块间走数据契约）、**可插拔**（带 `«接口»` 的模块其后端可替换，见图 2）。
...
flowchart TB
  ...
  UI --> ORCH
  ORCH --> COL & PAR & SCO & CIT & BEN & ACT & REP
  COL --> WRK
  WRK --> GEO
  COL --> EVI
  COL --> LLM
...
flowchart LR
  ...
  RE["«ReportExporter»"] --> REi["模板引擎 / Metabase（PDF · CSV）"]
```
...
flowchart LR
  ...
  CG --> CB
  CB --> AP["Action Plan"]
  SC --> RPT["Evidence Report<br/>PDF / CSV"]
  CG --> RPT
  CB --> RPT
  AP --> RPT
  RPT --> RC["复测<br/>T+7 / 14 / 30"]
  RC -. 同口径回采 .-> RUN
```

阶段与里程碑对应：MarketProfile/Prompt Pack → M1，Runner/Evidence → M2，Parser/Score → M3，Citation/Benchmark → M4，Report → M5，Action/复测 → M6。
