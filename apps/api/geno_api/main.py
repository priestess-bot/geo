...
from geno_api.auth_routes import register_auth_routes
...
def _runtime_access_log_persistence_required() -> bool:
    return os.environ.get("GENO_RUNTIME_ACCESS_LOG_REQUIRE_DB", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
class RuntimeNotificationEmailFeedbackWebhookRequest(BaseModel):
    ...
    recipient: str | None = Field(default=None, max_length=320)
    recipient_hash: str | None = Field(default=None, min_length=64, max_length=64)
    provider: str | None = Field(default="geno", max_length=120)
    provider_event_id: str | None = Field(default=None, max_length=500)
    provider_event_id_hash: str | None = Field(default=None, min_length=64, max_length=64)
    occurred_at: datetime | None = None
    metadata: dict[str, object] = Field(default_factory=dict)
    ...
    try:
        ...
        return Response(
            ...
            headers={
                ...
                "X-GENO-Project-Lifecycle-Export-Hash": export.content_hash,
                "X-GENO-Project-Lifecycle-Project-Id": export.project_id,
                "X-GENO-Project-Lifecycle-Method-Version": export.method_version,
                "X-GENO-Project-Lifecycle-Row-Count": str(export.row_count),
                "X-GENO-Project-Lifecycle-Total-Count": str(export.total_count),
            },
        )
    finally:
        close_repository_connection(repository)
...
def runtime_score_weight_profiles(
    ...
    require_runtime_actor_id(x_geno_actor_id)
    try:
        repository = build_repository_from_env()
    except RuntimePersistenceError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
...
def runtime_citation_graphs_export_csv(
    ...
    try:
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f'attachment; filename="{export.filename}"',
                "X-GENO-Citation-Graph-Export-Hash": export.content_hash,
                "X-GENO-Citation-Graph-Project-Id": str(export.filters.get("project_id", "")),
                "X-GENO-Citation-Graph-Row-Count": str(export.row_count),
                "X-GENO-Citation-Graph-Total-Count": str(export.total_count),
            },
        )
    finally:
        close_repository_connection(repository)
...
def runtime_report_export_jobs_export_csv(
    ...
    try:
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f'attachment; filename="{export.filename}"',
                "X-GENO-Report-Export-Job-Export-Hash": export.content_hash,
                "X-GENO-Report-Export-Job-Project-Id": project_id,
                "X-GENO-Report-Export-Job-Status": status or "",
                "X-GENO-Report-Export-Job-Report-Export-Id": report_export_id or "",
                "X-GENO-Report-Export-Job-Row-Count": str(export.row_count),
                "X-GENO-Report-Export-Job-Total-Count": str(export.total_count),
            },
        )
    finally:
        close_repository_connection(repository)
...
def runtime_notification_subscriptions_export_csv(
    ...
    try:
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f'attachment; filename="{export.filename}"',
                "X-GENO-Notification-Subscription-Export-Hash": export.content_hash,
                "X-GENO-Notification-Subscription-Project-Id": project_id,
                "X-GENO-Notification-Subscription-Status": status or "",
                "X-GENO-Notification-Subscription-Row-Count": str(export.row_count),
                "X-GENO-Notification-Subscription-Total-Count": str(export.total_count),
            },
        )
    finally:
        close_repository_connection(repository)
...
@app.post("/v1/runtime-notification-email-feedback-webhooks/geno")
async def record_runtime_notification_email_feedback_webhook(request: Request) -> dict[str, object]:
    ...
    metadata = {
        ...
        **signature_metadata,
        ...
        "provider": payload.provider or "geno",
    }
    try:
        repository = build_repository_from_env()
    except RuntimePersistenceError as exc:
...
def runtime_action_plans_export_csv(
    ...
    try:
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f'attachment; filename="{export.filename}"',
                "X-GENO-Action-Plan-Export-Hash": export.content_hash,
                "X-GENO-Action-Plan-Project-Id": str(export.filters.get("project_id", "")),
                "X-GENO-Action-Plan-Status": str(export.filters.get("status", "")),
                "X-GENO-Action-Plan-Row-Count": str(export.row_count),
                "X-GENO-Action-Plan-Total-Count": str(export.total_count),
            },
        )
    finally:
        close_repository_connection(repository)
