...
from geno_api.auth_context import AuthContext
from geno_api.auth_contracts import (
    AuthErrorResponse,
...
from geno_core.auth_delivery import AuthDeliveryError
from geno_core.models import RuntimeSessionRevokeInput
from geno_core.runtime import RuntimePersistenceError
...
def register_auth_routes(
    ...
    *,
    ...
    def runtime_auth_me(
        ...
        x_geno_csrf_token: str | None = Header(default=None, alias=csrf_header_name),
    ) -> AuthMeResponse:
        context = resolve_auth_context(x_geno_actor_id)
        scope = _scope_from_context(context)
        csrf_cookie = request.cookies.get(csrf_cookie_name, "").strip()
        csrf_header = (x_geno_csrf_token or "").strip()
...
def _consume_preflight_rate_limit(
    ...
    *,
    ...
    limit = _bounded_env_int("GENO_AUTH_PREFLIGHT_RATE_LIMIT", default=20, minimum=1, maximum=1000)
    window_seconds = _bounded_env_int(
        "GENO_AUTH_PREFLIGHT_RATE_WINDOW_SECONDS",
        default=600,
        minimum=1,
        maximum=3600,
    )
    ...
    trusted_source_header = os.getenv("GENO_AUTH_PREFLIGHT_TRUSTED_SOURCE_HEADER", "").strip()
    ...
    source_limit = _bounded_env_int(
        "GENO_AUTH_PREFLIGHT_SOURCE_RATE_LIMIT",
        default=100,
        minimum=1,
        maximum=5000,
    )
