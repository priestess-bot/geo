...
def persist_runtime_http_access_log(
    *,
    ...
    request_content_type = request.headers.get("content-type") or "application/octet-stream"
    redacted_request_body = redact_secret_text(request_body, content_type=request_content_type)
    response_content_type = response_media_type or response_headers.get("content-type") or "application/octet-stream"
    redacted_response_body = redact_secret_text(response_body, content_type=response_content_type)
    redacted_request_body_bytes = (
        redacted_request_body if isinstance(redacted_request_body, bytes) else redacted_request_body.encode("utf-8")
    )
    redacted_response_body_bytes = (
        redacted_response_body if isinstance(redacted_response_body, bytes) else redacted_response_body.encode("utf-8")
    )
    ...
    try:
        repository.save_runtime_http_access_log(
            RuntimeHttpAccessLogInput(
                ...
                request_body_hash=sha256_bytes(redacted_request_body_bytes),
                request_body_size=len(redacted_request_body_bytes),
                ...
                response_body_hash=sha256_bytes(redacted_response_body_bytes),
                response_body_size=len(redacted_response_body_bytes),
...
def _redacted_headers(headers: Mapping[str, str]) -> dict[str, str]:
    sensitive_names = {"authorization", "cookie", "set-cookie", "x-api-key", "x-geo-actor-id"}
