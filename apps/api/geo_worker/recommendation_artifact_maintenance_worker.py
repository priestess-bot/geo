"""Dedicated least-privilege worker for Recommendation task-artifact deletion."""
...
from geo_core.recommendations.artifact_maintenance_composition import (
    build_recommendation_artifact_maintenance_composition,
)
from geo_core.recommendations.artifact_maintenance_worker import (
    RecommendationArtifactMaintenanceOperation,
)
...
class _HeartbeatMiddleware(Middleware):
    ...
    def after_process_boot(self, broker) -> None:
        del broker
        # Fail startup before reporting readiness if the deleter identity is absent.
...
class _Handler:
    operation: RecommendationArtifactMaintenanceOperation
...
def maintenance_dispatcher() -> PlacementWorkerDispatcher:
    ...
    object_store = build_object_store_from_prefix(
        "GEO_RECOMMENDATION_ARTIFACT_DELETER"
    )
    if object_store.bucket != _RECOMMENDATION_ARTIFACT_BUCKET:
        raise RuntimeError("Recommendation artifact deleter bucket is not isolated")
    store = PostgresDurableJobStore(connect)
    worker_id = os.getenv(
        "GEO_WORKER_ID", f"recommendation-artifact-maintenance:{socket.gethostname()}"
    ).strip()
    if not worker_id:
        raise RuntimeError("GEO_WORKER_ID cannot be empty")
    lease_for = timedelta(
        seconds=bounded_int_setting(
            "GEO_RECOMMENDATION_ARTIFACT_MAINTENANCE_JOB_LEASE_SECONDS",
            300,
            minimum=30,
            maximum=3600,
        )
    ...
    max_deletions = bounded_int_setting(
        "GEO_RECOMMENDATION_ARTIFACT_MAX_DELETIONS_PER_JOB",
        100,
        minimum=1,
        maximum=1000,
    )
    composition = build_recommendation_artifact_maintenance_composition(
        connection_factory=connect,
        object_store=object_store,
        deletion_lease_seconds=bounded_int_setting(
            "GEO_RECOMMENDATION_ARTIFACT_DELETION_LEASE_SECONDS",
            120,
            minimum=30,
            maximum=3600,
        ),
    ...
    return PlacementWorkerDispatcher(
        store=store,
        handlers={RECOMMENDATION_ARTIFACT_MAINTENANCE_JOB_KIND: _Handler(operation)},
        worker_id=worker_id,
        lease_for=lease_for,
    )
...
def process_recommendation_artifact_maintenance_job(
