...
export function geoHref(projectId: string, selection: GeoSelection, updates: { [key: string]: string | undefined }): string {
  ...
  const current: { [key: string]: string | undefined } = {
    section: selection.section, placement_stage: selection.placementStage, measurement_window: selection.measurementWindow, campaign_id: selection.campaignId,
    ...
    publication_id: selection.publicationId, submission_id: selection.submissionId, ...updates
  };
  Object.entries(current).forEach(([key, value]) => { if (value) params.set(key, value); });
  return `/projects/${projectId}/geo?${params.toString()}`;
}
