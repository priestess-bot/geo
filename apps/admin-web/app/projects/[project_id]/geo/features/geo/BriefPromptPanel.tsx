  ...
  return <div className={styles.columns}>
    ...
    <section className={styles.panel}>
      ...
      <ActionForm action={createBrief} submitLabel={brief ? "保存新版本" : "保存内容要求"} disabled={!opportunityId}>
        ...
        <label>目标受众<input name="audience" required defaultValue="澳大利亚消费者" placeholder="例如：澳大利亚中型草坪家庭" /></label>
...
function PromptAdministration({ projectId, data }: { projectId: string; data: GeoWorkspaceData }) {
  ...
  return <CommandPanel label="高级：Prompt 规则与版本管理">
    <div className={styles.threeColumns}>
      <section className={styles.unframed}><h3>Prompt 技能</h3><ActionForm action={installDefaultPromptCatalog} submitLabel="同步九平台默认 Prompt"><HiddenProject projectId={projectId} /></ActionForm><ResourceBlock resource={data.skills}>{(items) => <div className={styles.list}>{items.map((item) => <a key={item.id} className={item.id === data.selection.skillId ? styles.selectedRow : styles.row} href={geoHref(projectId, data.selection, { skill_id: item.id })}><strong>{channelLabel(item.skill_key.split(".").at(-2) || item.skill_key)}</strong><Status value={item.status} /></a>)}</div>}</ResourceBlock><ActionForm action={createPromptSkill} submitLabel="创建技能"><HiddenProject projectId={projectId} /><label>技能键<input name="skill_key" required placeholder="placement.productreview.review" /></label></ActionForm></section>
      <section className={styles.unframed}><h3>不可变版本</h3><ResourceBlock resource={data.releases}>{(items) => items.length ? <div className={styles.list}>{items.map((item) => <PromptReleaseRow item={item} key={item.id} projectId={projectId} />)}</div> : <Empty>选择一个 Prompt 技能查看其发布版本。</Empty>}</ResourceBlock><ActionForm action={createPromptRelease} submitLabel="创建新版本" disabled={!data.selection.skillId}><HiddenProject projectId={projectId} /><input type="hidden" name="skill_id" value={data.selection.skillId || ""} /><label>规则源<textarea name="source" required /></label><label>系统 Prompt<textarea name="system_template" required /></label><label>用户 Prompt<textarea name="user_template" required /></label><label>输出结构 JSON<textarea name="output_schema" required defaultValue={DEFAULT_OUTPUT_SCHEMA} /></label><label>客户端变量<textarea name="client_variable_names" placeholder="每行一个" /></label></ActionForm></section>
      <section className={styles.unframed}><h3>当前渠道任务绑定</h3><PromptBindingIdentity binding={currentBinding} /><ActionForm action={bindOpportunityPromptRelease} submitLabel="确认并追加绑定" disabled={!data.selection.campaignId || !data.selection.opportunityId || !currentBinding || approvedReleases.length === 0}><HiddenProject projectId={projectId} /><input type="hidden" name="campaign_id" value={data.selection.campaignId || ""} /><input type="hidden" name="opportunity_id" value={data.selection.opportunityId || ""} /><input type="hidden" name="expected_binding_version" value={currentBinding?.binding_version || ""} /><label>已批准发布版本<select name="template_release_id" required defaultValue=""><option value="" disabled>选择明确版本</option>{approvedReleases.map((item) => <option key={item.id} value={item.id}>{item.skill_key} · 技能 v{item.skill_version} · 发布版本 {item.release_number}</option>)}</select></label><label>变更原因<textarea name="reason" required /></label></ActionForm><ResourceBlock resource={data.promptBindingHistory}>{(items) => items.length ? <div className={styles.list}>{items.map((item) => <div className={styles.row} key={item.id}><span className={styles.rowHeader}><strong>绑定 v{item.binding_version}</strong><Status value={item.status} /></span><span className={styles.meta}>{new Date(item.changed_at).toLocaleString("zh-CN")} · {item.changed_by || "历史执行人未知"}</span><TechnicalInfo><code>{item.template_release_id || "未绑定"}</code><code>{item.release_hash || "-"}</code></TechnicalInfo></div>)}</div> : <Empty>当前渠道任务还没有 Prompt 绑定历史。</Empty>}</ResourceBlock></section>
    </div>
  </CommandPanel>;
}
...
function PromptReleaseRow({ item, projectId }: {
  ...
  return <div className={styles.row}>
    <span className={styles.rowHeader}><strong>技能 v{item.skill_version} · 发布版本 {item.release_number}</strong><Status value={item.status} /></span>
    ...
    {command ? <ActionForm action={transitionPromptRelease} submitLabel={command === "approve" ? "批准发布版本" : "撤销发布版本"} danger={command === "revoke"}>
...
function PromptBindingIdentity({ binding }: {
  ...
  if (!binding || binding.status !== "bound") return <Empty>尚未绑定已批准 Prompt 发布版本。</Empty>;
  return <div className={styles.keyValues}>
    <div><span className={styles.meta}>技能</span><br /><strong>{binding.skill_key || "-"}</strong></div>
    <div><span className={styles.meta}>技能版本 ID</span><br /><code>{binding.skill_version_id}</code></div>
    <div><span className={styles.meta}>发布版本</span><br /><strong>v{binding.release_version}</strong></div>
    <div><span className={styles.meta}>发布版本 ID</span><br /><code>{binding.template_release_id}</code></div>
    <div><span className={styles.meta}>发布版本哈希</span><br /><code>{binding.release_hash}</code></div>
    <div><span className={styles.meta}>绑定记录</span><br /><strong>v{binding.binding_version}</strong><span className={styles.meta}>{binding.changed_by || "历史执行人未知"} · {new Date(binding.changed_at).toLocaleString("zh-CN")}</span></div>
  </div>;
}
