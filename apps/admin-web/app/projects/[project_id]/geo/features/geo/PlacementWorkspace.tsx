...
export function PlacementWorkspace({ projectId, data, catalog }: { projectId: string; data: GeoWorkspaceData; catalog: CatalogLoadResult }) {
  ...
  return <section className={styles.workspace}>
    <header className={styles.pageHeading}><div><h2>内容生产</h2><p>为一个明确的产品和渠道完成内容准备、证据校验、生成、审核和人工发布。</p></div>
      <a className="button secondary" href={geoHref(projectId, selection, { placement_stage: "simulation" })}>打开 TEST ONLY 预览</a>
    </header>
    <section className={styles.panel}>
      {opportunityOptions.length ? <RouteSelect label="当前渠道任务" options={opportunityOptions} value={opportunity?.id} /> : <strong>当前 Campaign 没有渠道任务</strong>}
    ...
    {selection.placementStage !== "simulation" ? <nav className={styles.stepper} aria-label="内容生产步骤">{stages.map((stage) => <a key={stage.id} className={selection.placementStage === stage.id ? styles.active : ""} href={geoHref(projectId, selection, { placement_stage: stage.id })}><span>步骤 {stage.number}</span><strong>{stage.label}</strong></a>)}</nav> : null}
    {selection.placementStage === "brief" ? <BriefPromptPanel projectId={projectId} data={data} catalog={catalog} mode="brief" /> : null}
    {selection.placementStage === "evidence" ? <BriefPromptPanel projectId={projectId} data={data} catalog={catalog} mode="evidence" /> : null}
    {selection.placementStage === "generation" ? <GenerationPackagePanel projectId={projectId} data={data} mode="generation" /> : null}
    {selection.placementStage === "review" ? <GenerationPackagePanel projectId={projectId} data={data} mode="review" /> : null}
