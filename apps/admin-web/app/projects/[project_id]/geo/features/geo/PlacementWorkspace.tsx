...
export function PlacementWorkspace({ projectId, data, catalog }: { projectId: string; data: GeoWorkspaceData; catalog: CatalogLoadResult }) {
  ...
  return <section className={styles.workspace}>
    <header className={styles.pageHeading}><div><h2>内容生产</h2><p>为一个明确的产品和渠道完成内容准备、证据校验、生成、审核和人工发布。</p></div>
      {opportunity || hasLegacySimulations ? <a className="button secondary" href={geoHref(projectId, selection, { placement_stage: "simulation" })}>{opportunity ? "打开 TEST ONLY 预览" : "查看历史 TEST ONLY 预览"}</a> : null}
    </header>
    ...
    {!opportunity && selection.placementStage !== "simulation" ? <div className={styles.empty}>请选择一个渠道任务。</div> : null}
    ...
    {(opportunity || hasLegacySimulations) && selection.placementStage === "simulation" ? <PromptSimulationPanel projectId={projectId} data={data} catalog={catalog} /> : null}
  </section>;
}
