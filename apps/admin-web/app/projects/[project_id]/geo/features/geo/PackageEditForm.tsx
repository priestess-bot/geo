...
export function PackageEditForm({ projectId, version, claims }: {
  projectId: string; version: PackageVersionView; claims: ClaimView[];
}) {
  ...
  const contentJson = useMemo(() => JSON.stringify({ ...version.content_json, rendered_text: text }), [text, version.content_json]);
  return <ActionForm action={editPackage} submitLabel="保存为新版本">
    <input name="project_id" type="hidden" value={projectId} /><input name="package_id" type="hidden" value={version.package_id} /><input name="base_version_id" type="hidden" value={version.id} /><input name="base_content_hash" type="hidden" value={version.content_hash} />
