...
export async function PublicationPanel({ projectId, data }: { projectId: string; data: GeoWorkspaceData }) {
  const version = data.packageVersion.data || data.packages.data.find((item) => item.workflow_status === "approved") || null;
  const publication = data.publications.data.find((item) => item.id === data.selection.publicationId) || data.publications.data[0];
  ...
  return <div className={styles.workspace}>
    ...
    <header className={styles.pageHeading}><div><h2>发布与测量</h2><p>显式创建发布任务，记录人工提交，验证公开 URL，并按冻结口径复测。</p></div>
      <CommandPanel label="创建发布任务"><CreatePublicationForm projectId={projectId} versionId={version?.id} versionStatus={version?.workflow_status} destinations={usableDestinations} /></CommandPanel>
    </header>
    ...
    <div className={styles.columns}>
      ...
      <section className={styles.panel}><div className={styles.sectionHeader}><div><p>URL lifecycle</p><h2>公开 URL 验证</h2></div>{submission ? <Status value={submission.status} /> : null}</div>
        {submission ? <><p>{submission.submitted_url ? <a href={submission.submitted_url} target="_blank" rel="noreferrer">{submission.submitted_url}</a> : "尚未回填公开 URL"}</p><TechnicalInfo><code>{submission.id}</code>{submission.verification_result ? <pre className={styles.code}>{JSON.stringify(submission.verification_result, null, 2)}</pre> : null}</TechnicalInfo></> : <Empty>选择一条提交记录。</Empty>}
        <ActionForm action={setSubmissionUrl} submitLabel="保存公开 URL" disabled={!submission}><HiddenProject projectId={projectId} /><input type="hidden" name="submission_id" value={submission?.id || ""} /><label>公开 URL<input name="submitted_url" type="url" required defaultValue={submission?.submitted_url || ""} /></label></ActionForm>
        <div className={styles.toolbar}><ActionForm action={verifySubmission} submitLabel="请求验证" disabled={!submission?.submitted_url}><HiddenProject projectId={projectId} /><input type="hidden" name="submission_id" value={submission?.id || ""} /></ActionForm><ActionForm action={blockSubmission} submitLabel="标记受阻" danger disabled={!submission}><HiddenProject projectId={projectId} /><input type="hidden" name="submission_id" value={submission?.id || ""} /><input type="hidden" name="reason" value="Blocked by Admin operator" /></ActionForm></div>
      </section>
    </div>
