...
export async function PublicationPanel({ projectId, data }: { projectId: string; data: GeoWorkspaceData }) {
  ...
  return <div className={styles.workspace}>
    ...
    <section className={styles.panel}><div className={styles.sectionHeader}><div><p>发布意图</p><h2>发布任务</h2></div><span className={styles.meta}>{data.publications.data.length} 个</span></div>
    ...
    <div className={styles.columns}>
      ...
      <section className={styles.panel}><div className={styles.sectionHeader}><div><p>URL 生命周期</p><h2>公开 URL 验证</h2></div>{submission ? <Status value={submission.status} /> : null}</div>
...
function VerificationEvidence({ attempts, hasLegacyProjection }: {
  ...
  return <div className={styles.list}>
    <div className={styles.row}>
      ...
      <TechnicalInfo label="验证规则与证据哈希"><code>规则 {latest.verifier_version}</code><code>结果 {latest.result_hash}</code><code>规则哈希 {latest.verification_rule_hash || "无"}</code><code>内容 {latest.content_rule_hash || "无"}</code><code>正文 {latest.body_hash || "无"}</code><code>可见文本 {latest.visible_text_hash || "无"}</code><code>元数据 {latest.metadata_hash || "无"}</code><code>任务 {latest.job_id}</code></TechnicalInfo>
    </div>
    {attempts.length > 1 ? <TechnicalInfo label="历史验证尝试">{attempts.slice(1).map((attempt) => <code key={attempt.id}>第 {attempt.attempt_number} 次 · {attempt.outcome} · {attempt.result_hash}</code>)}</TechnicalInfo> : null}
  </div>;
}
