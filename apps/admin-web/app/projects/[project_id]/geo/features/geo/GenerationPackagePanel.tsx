...
function GenerationStep({ projectId, data }: { projectId: string; data: GeoWorkspaceData }) {
  const bundle = data.bundles.data.find((item) => item.id === data.selection.bundleId);
  const isLegacyBundle = Boolean(bundle && bundle.prompt_release_binding_id === null && bundle.prompt_release_binding_version === null);
  ...
  return <div className={styles.workspace}>
    ...
    {isLegacyBundle ? <div className={styles.notice} data-testid="legacy-prompt-bundle"><span>迁移历史生成输入只读，不能启动新版生成任务。</span><a href={geoHref(projectId, data.selection, { placement_stage: "evidence", bundle_id: undefined, job_id: undefined })}>返回准备证据并重建</a></div> : null}
    <div className={styles.columns}>
      <section className={styles.panel}><div className={styles.sectionHeader}><div><p>DeepSeek 生成任务</p><h2>生成文案</h2></div>{data.job.data ? <Status value={data.job.data.status} /> : null}</div>
        {isLegacyBundle ? <Empty>这个生成输入来自旧版，缺少已批准 Prompt Release 绑定。请返回证据步骤重新冻结生成输入。</Empty> : <ActionForm action={createGenerationJob} submitLabel="开始生成" disabled={!bundle}><HiddenProject projectId={projectId} /><input type="hidden" name="campaign_id" value={data.selection.campaignId || ""} /><label>生成输入<select name="bundle_id" required defaultValue={bundle?.id || ""}><option value="" disabled>选择生成输入</option>{data.bundles.data.map((item, index) => <option key={item.id} value={item.id}>生成输入 {data.bundles.data.length - index}{item.prompt_release_binding_id ? "" : " · 迁移历史（只读）"}</option>)}</select></label><details><summary>模型设置</summary><div className={styles.formInset}><label>模型<input name="configured_model" defaultValue="deepseek-v4-flash" required /></label><label>总调用预算<input name="model_call_budget" type="number" min="1" max="5" defaultValue="2" required /></label></div></details></ActionForm>}
        <ResourceBlock resource={data.job}>{(job) => job ? <div className={styles.row}><span className={styles.rowHeader}><strong>生成任务</strong><Status value={job.status} /></span><span className={styles.meta}>{new Date(job.updated_at).toLocaleString("zh-CN")} · {job.result_ref || job.error_code || "等待模型结果"}</span><div className={styles.toolbar}>{(["retry", "replay", "cancel"] as const).map((command) => <ActionForm action={controlJob} submitLabel={command === "retry" ? "重试" : command === "replay" ? "重新执行" : "取消"} key={command} danger={command === "cancel"}><HiddenProject projectId={projectId} /><input type="hidden" name="campaign_id" value={data.selection.campaignId || ""} /><input type="hidden" name="job_id" value={job.id} /><input type="hidden" name="command" value={command} /></ActionForm>)}</div><TechnicalInfo><code>{job.id}</code><code>{job.kind}</code></TechnicalInfo></div> : <Empty>开始生成后，这里会显示排队、处理和完成状态。</Empty>}</ResourceBlock>
      </section>
  ...
  return <div className={styles.workspace}>
    ...
    <div className={styles.columns}>
      ...
      <section className={styles.workspace}>
        ...
        {version ? <CommandPanel label="人工修改并创建新版本"><PackageEditForm campaignId={data.selection.campaignId || ""} projectId={projectId} version={version} claims={data.claims.data} /></CommandPanel> : null}
      </section>
    </div>
    <section className={styles.panel}><div className={styles.sectionHeader}><div><p>Export is not publication · 导出不是发布</p><h2>导出与交付</h2></div></div><ActionForm action={createExport} submitLabel="创建不可变导出" disabled={!version}><HiddenProject projectId={projectId} /><input type="hidden" name="campaign_id" value={data.selection.campaignId || ""} /><input type="hidden" name="version_id" value={version?.id || ""} /></ActionForm>
