  ...
  return <div className={styles.workspace}>
    ...
    <div className={styles.columns}>
      <section className={styles.panel}><div className={styles.sectionHeader}><div><p>DeepSeek 生成任务</p><h2>生成文案</h2></div>{data.job.data ? <Status value={data.job.data.status} /> : null}</div>
        <ActionForm action={createGenerationJob} submitLabel="开始生成" disabled={!bundle}><HiddenProject projectId={projectId} /><input type="hidden" name="campaign_id" value={data.selection.campaignId || ""} /><label>生成输入<select name="bundle_id" required defaultValue={bundle?.id || ""}><option value="" disabled>选择生成输入</option>{data.bundles.data.map((item, index) => <option key={item.id} value={item.id}>生成输入 {data.bundles.data.length - index}</option>)}</select></label><details><summary>模型设置</summary><div className={styles.formInset}><label>模型<input name="configured_model" defaultValue="deepseek-v4-flash" required /></label><label>总调用预算<input name="model_call_budget" type="number" min="1" max="5" defaultValue="2" required /></label></div></details></ActionForm>
