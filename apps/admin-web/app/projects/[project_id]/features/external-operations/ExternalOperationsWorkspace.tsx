...
function BrowserSection({ data, projectId }: { data: ExternalOperationsData; projectId: string }) {
  ...
  const endpoints = data.browser.egress_endpoints.filter((item) =>
  ...
  return <section className={styles.section}>
    ...
    <div className={styles.columns}>
      ...
      <div><h4>LokiProxy 澳洲 IP 池</h4>
        <p className={styles.empty}>密码和 Username 模板统一在“观测与统计 → 采样”中配置；此处只展示运行状态，避免重复维护两套代理入口。</p>
        <a href={`/projects/${encodeURIComponent(projectId)}?tab=measurement&workflow_view=sampling#consumer-capture-heading`}>前往配置与测试 IP 池</a>
        <Rows empty="尚未配置 LokiProxy 澳洲 IP 池。" items={data.browser.egress_endpoints.map((item) => <div className={styles.row} key={item.id}><span><strong>{item.name}</strong><small>{item.provider === "lokiproxy" ? `${item.pool_product === "mobile" ? "移动" : "旋转住宅"} · ${Math.round(item.session_ttl_seconds / 60)} 分钟粘性 · 并发 ${item.max_concurrency}` : "旧版出口，不参与新采样"}{item.last_error_class ? ` · ${item.last_error_class}` : ""}</small></span><Status value={item.provider === "lokiproxy" ? item.health_status : item.status} />{item.status === "approved" && item.provider === "lokiproxy" && item.health_status !== "cooldown" ? <ActionForm action={externalOperation} submitLabel="测试出口"><Hidden projectId={projectId} command="test_egress" /><input name="endpoint_id" type="hidden" value={item.id} /></ActionForm> : null}{["approved", "disabled"].includes(item.status) && item.provider === "lokiproxy" ? <ActionForm action={externalOperation} submitLabel={item.status === "approved" ? "停用 IP 池" : "重新启用"}><Hidden projectId={projectId} command="set_egress_status" /><input name="endpoint_id" type="hidden" value={item.id} /><input name="status" type="hidden" value={item.status === "approved" ? "disabled" : "approved"} /></ActionForm> : null}</div>)} />
...
function statusLabel(value: string) { return ({ draft: "草稿", approved: "已批准", active: "运行中", disabled: "已停用", suspended: "已暂停", detected: "已检测", untested: "未测试", healthy: "健康", degraded: "连接异常", cooldown: "冷却中", queued: "排队中", running: "执行中", cancel_requested: "正在取消", succeeded: "成功", failed: "失败", cancelled: "已取消", in_review: "审核中", rejected: "已拒绝", stale: "已过期", revoked: "已撤销", info: "提示", warning: "警告", critical: "严重" } as Record<string, string>)[value] || value; }
