  ...
  return (
    <section>
      <header className={styles.controlHeading}>
        <div><p>Protocol control</p><h2>创建与状态变更</h2></div>
      ...
      <div className={styles.lifecycleList}>
        ...
        {statisticalProtocols.map((protocol) => (
          <ProtocolLifecycle
            ...
            key={protocol.id}
            label={protocol.kind === "comparison_plan" ? "Comparison" : "Drift"}
...
function MetricProtocolCreateForm({
  ...
  return (
    <form action={action} className={alertStyles.commandForm}>
      ...
      <label><span>Primary subject key</span><input disabled={disabled} name="primary_subject_key" required /></label>
      <label><span>Brand aliases</span><textarea disabled={disabled} name="brand_aliases" required rows={2} /></label>
      <label><span>Product aliases</span><textarea disabled={disabled} name="product_aliases" required rows={2} /></label>
      <label><span>Competitor key</span><input disabled={disabled} name="competitor_key" /></label>
      <label><span>Competitor aliases</span><textarea disabled={disabled} name="competitor_aliases" rows={2} /></label>
      <label><span>Verified URLs</span><textarea disabled={disabled} name="verified_urls" required rows={2} /></label>
      <label><span>Question clusters · ID|cluster</span><textarea disabled={disabled} name="question_clusters" required rows={2} /></label>
      <label><span>Prompt Release ID</span><input disabled={disabled} name="prompt_release_id" required /></label>
      <label><span>Prompt Release SHA-256</span><input disabled={disabled} maxLength={64} minLength={64} name="prompt_release_hash" required /></label>
      <label><span>Judge version</span><input defaultValue="metric-judge-v1" disabled={disabled} name="judge_version" required /></label>
      <label><span>Judge model identity</span><input disabled={disabled} name="model_identity" required /></label>
      <label><span>Fact Snapshot ID</span><input disabled={disabled} name="fact_snapshot_id" required /></label>
      <label><span>Fact Snapshot SHA-256</span><input disabled={disabled} maxLength={64} minLength={64} name="fact_snapshot_hash" required /></label>
      <label><span>Corpus Version ID</span><input disabled={disabled} name="corpus_version_id" required /></label>
      <label><span>Corpus version</span><input disabled={disabled} name="approved_corpus_version" required /></label>
      <label><span>Corpus Version SHA-256</span><input disabled={disabled} maxLength={64} minLength={64} name="corpus_version_hash" required /></label>
      <label><span>Minimum valid completion</span><input defaultValue="0.8" disabled={disabled} max="1" min="0.8" name="minimum_valid_completion" required step="0.01" type="number" /></label>
      <SupersedesSelect disabled={disabled} items={supersedes} />
      <button disabled={disabled} type="submit">{pending ? "创建中..." : "创建 Metric Protocol"}</button>
    </form>
  );
}
...
function ComparisonProtocolCreateForm({ action, commandKey, disabled, pending, projectId, supersedes }: CreateFormProps) {
  return (
    <form action={action} className={alertStyles.commandForm}>
      ...
      <label><span>Family</span><input disabled={disabled} name="family" required /></label>
      <label><span>Question clusters</span><textarea disabled={disabled} name="question_clusters" required rows={3} /></label>
      <NumberField defaultValue="0.05" disabled={disabled} label="Alpha" max="0.99" min="0.01" name="alpha" />
      <NumberField defaultValue="0.05" disabled={disabled} label="Practical delta" min="0" name="delta" />
      <NumberField defaultValue="0.8" disabled={disabled} label="Target power" max="1" min="0.8" name="target_power" />
      <NumberField defaultValue="0.1" disabled={disabled} label="Precision" min="0.01" name="precision" />
      <NumberField defaultValue="3" disabled={disabled} label="Minimum pairs" min="1" name="min_pairs" step="1" />
      <label><span>Power plan SHA-256</span><input disabled={disabled} maxLength={64} minLength={64} name="power_plan_hash" required /></label>
      <NumberField defaultValue="0.9" disabled={disabled} label="A-priori design power" max="1" min="0" name="a_priori_design_power" />
      <NumberField defaultValue="0.8" disabled={disabled} label="Minimum completion ratio" max="1" min="0.8" name="minimum_completion_ratio" />
      <NumberField defaultValue="10000" disabled={disabled} label="Bootstrap iterations" min="100" name="bootstrap_iterations" step="1" />
      <SupersedesSelect disabled={disabled} items={supersedes} />
      <button disabled={disabled} type="submit">{pending ? "创建中..." : "创建 Comparison Protocol"}</button>
    </form>
  );
}
...
function DriftProtocolCreateForm({ action, commandKey, disabled, pending, projectId, supersedes }: CreateFormProps) {
  return (
    <form action={action} className={alertStyles.commandForm}>
      ...
      <NumberField defaultValue="3" disabled={disabled} label="Minimum question count" min="1" name="minimum_question_count" step="1" />
      <SupersedesSelect disabled={disabled} items={supersedes} />
      <button disabled={disabled} type="submit">{pending ? "创建中..." : "创建 Drift Protocol"}</button>
    </form>
  );
}
...
function operationLabel(operation: "submit" | "approve" | "retire"): string {
  ...
  if (operation === "approve") return "批准 Protocol";
  return "退役 Protocol";
}
