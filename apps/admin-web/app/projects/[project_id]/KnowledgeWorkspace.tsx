  ...
  return (
    <div className={styles.workspace}>
      <header className={styles.header}>
        <div><p className="eyebrow">企业知识</p><h2>导入、清洗与事实治理</h2></div>
        <p>企业来源经过解析、清洗、分块、事实提取和质量检查后，才能进入证据包。</p>
      </header>
...
function Summary({ data }: { data: KnowledgeWorkspaceData }) {
  ...
  const metrics = [
    ...
    ["活动文本分块", dashboard.active_chunks], ["待审事实", dashboard.pending_facts],
...
function ImportView({ data, projectId }: { data: KnowledgeWorkspaceData; projectId: string }) {
  return <div className={styles.stack}>
    <section className={styles.section}>
      <div className={styles.sectionHeading}><div><p>来源接入</p><h3>导入企业知识</h3></div><span>单个来源上限 5 MB</span></div>
    ...
    <section className={styles.section}>
      <div className={styles.sectionHeading}><div><p>来源资产</p><h3>已导入来源</h3></div><span>{data.sources.data.length} 条</span></div>
...
function ProcessingView({ data, projectId }: { data: KnowledgeWorkspaceData; projectId: string }) {
  ...
  return <div className={styles.processingGrid}>
    <section className={styles.section}><div className={styles.sectionHeading}><div><p>可持久化任务</p><h3>处理任务</h3></div><span>{data.runs.data.length} 次</span></div><div className={styles.runList}>{data.runs.data.map((run) => <a className={run.id === selectedId ? styles.selectedRun : ""} href={`${href(projectId, "processing")}&pipeline_run_id=${run.id}`} key={run.id}><span><strong>{run.source_title}</strong><small>{formatDate(run.created_at)}</small></span><Status value={run.status} /></a>)}</div></section>
    <section className={styles.section}><div className={styles.sectionHeading}><div><p>处理阶段</p><h3>解析与清洗进度</h3></div><span>{data.stages.data.length}/6</span></div><ol className={styles.stageList}>{data.stages.data.map((stage) => <li key={stage.id}><span className={styles.stageIndex}>{stage.ordinal}</span><div><strong>{stageLabel(stage.stage_key)}</strong><small>{Object.entries(stage.metrics || {}).map(([key, value]) => `${key}: ${String(value)}`).join(" · ") || stage.error_detail || "等待处理"}</small></div><Status value={stage.status} /></li>)}</ol>{!data.stages.data.length ? <p>选择处理任务后查看六阶段状态。</p> : null}</section>
  </div>;
}

function ChunksView({ chunks, projectId }: { chunks: KnowledgeChunk[]; projectId: string }) {
  return <section className={styles.section}><div className={styles.sectionHeading}><div><p>清洗后语料</p><h3>文本分块可视化</h3></div><span>{chunks.length} 条</span></div><div className={styles.chunkList}>{chunks.map((chunk) => <article key={chunk.id}><header><span><strong>{chunk.source_title}</strong><small>文本分块 {chunk.chunk_index} · {chunk.char_count} 字符 · {chunk.text_hash.slice(0, 10)}</small></span><Status value={chunk.status} /></header><p>{chunk.text}</p><footer><span>{chunk.quality_flags.join(" · ") || "无质量标记"}</span>{chunk.status === "active" ? <KnowledgeActionForm action={disableKnowledgeChunk} className={styles.inlineForm} submitLabel="禁用"><input name="project_id" type="hidden" value={projectId} /><input name="chunk_id" type="hidden" value={chunk.id} /></KnowledgeActionForm> : null}</footer></article>)}</div>{!chunks.length ? <p>成功处理来源后将在这里显示可追踪文本分块。</p> : null}</section>;
}

function SearchView({ data, projectId }: { data: KnowledgeWorkspaceData; projectId: string }) {
  return <section className={styles.section}><div className={styles.sectionHeading}><div><p>语料检索</p><h3>知识检索</h3></div><span>{data.chunks.data.length} 条结果</span></div><form action={`/projects/${projectId}`} className={styles.searchForm} method="get"><input name="tab" type="hidden" value="knowledge" /><input name="knowledge_tab" type="hidden" value="search" /><label>检索词<input defaultValue={data.query} name="knowledge_query" placeholder="产品、能力、保修或使用场景" /></label><button type="submit">检索</button></form><div className={styles.searchResults}>{data.chunks.data.map((chunk) => <article key={chunk.id}><strong>{chunk.source_title} · 文本分块 {chunk.chunk_index}</strong><p>{chunk.text}</p></article>)}</div></section>;
}

function DashboardView({ data }: { data: KnowledgeWorkspaceData }) {
  const latest = data.runs.data[0];
  return <div className={styles.stack}><section className={styles.section}><div className={styles.sectionHeading}><div><p>就绪状态</p><h3>知识库看板</h3></div><Status value={latest?.status || "empty"} /></div><div className={styles.dashboardGrid}><DashboardItem label="来源治理" value={`${data.dashboard.data.sources} 个来源`} note="URL、文件和授权文本" /><DashboardItem label="处理成功率" value={`${data.dashboard.data.succeeded_runs}/${data.runs.data.length}`} note={`失败 ${data.dashboard.data.failed_runs}`} /><DashboardItem label="可用语料" value={`${data.dashboard.data.active_chunks} 个文本分块`} note="仅活动文本分块可进入后续治理" /><DashboardItem label="事实审核" value={`${data.dashboard.data.pending_facts} 待处理`} note="批准后才可转为正式证据" /></div></section></div>;
}

function QualityView({ data, projectId }: { data: KnowledgeWorkspaceData; projectId: string }) {
  return <section className={styles.section}><div className={styles.sectionHeading}><div><p>质量门槛</p><h3>质检发现</h3></div><span>{data.findings.data.length} 条</span></div><div className={styles.findingList}>{data.findings.data.map((item) => <article key={item.id}><header><strong>{item.finding_code}</strong><span className={styles[item.severity] || ""}>{item.severity}</span></header><p>{item.message}</p><small>{item.source_title} · {item.status}</small>{item.status === "open" ? <KnowledgeActionForm action={reviewKnowledgeFinding} className={styles.inlineForm} submitLabel="确认处置"><input name="project_id" type="hidden" value={projectId} /><input name="finding_id" type="hidden" value={item.id} /><label>处置<select name="decision"><option value="resolved">问题已解决</option><option value="accepted">接受风险</option></select></label></KnowledgeActionForm> : null}</article>)}</div>{!data.findings.data.length ? <p>当前没有开放的质量问题。</p> : null}</section>;
}
...
function TraceView({
  ...
  return <div className={styles.stack}>
    <section className={styles.section}><div className={styles.sectionHeading}><div><p>证据追溯</p><h3>事实候选与证据追踪</h3></div><span>{facts.length} 条</span></div><div className={styles.factList}>{facts.map((fact) => <article className={fact.id === data.selectedFactId ? styles.selectedFact : ""} key={fact.id}><header><span><strong>{fact.source_title}</strong><small>文本分块 {fact.chunk_id.slice(0, 8)} · {fact.statement_hash.slice(0, 10)}</small></span><Status value={fact.status} /></header><p>{fact.statement}</p>{fact.status === "pending_review" ? <KnowledgeActionForm action={reviewKnowledgeFact} submitLabel="保存审核"><input name="project_id" type="hidden" value={projectId} /><input name="fact_id" type="hidden" value={fact.id} /><div className={styles.reviewFields}><label>结论<select name="decision"><option value="approved">批准</option><option value="rejected">拒绝</option></select></label><label>审核说明<input name="notes" placeholder="说明事实主体、时效或使用限制" /></label></div></KnowledgeActionForm> : <div className={styles.factFooter}><small>{fact.review_notes || "已完成审核"}</small>{fact.status === "approved" ? <a href={factHref(projectId, fact.id)}>证据与追溯链</a> : null}</div>}</article>)}</div>{!facts.length ? <p>处理流程生成事实候选后将在此人工审核。</p> : null}</section>
...
function FactEvidenceDetail({
  ...
  return <section className={styles.section}>
    <div className={styles.sectionHeading}><div><p>受治理证据</p><h3>{existing ? existing.evidence.title : "正式证据提升"}</h3></div><Status value={existing ? "ready" : proposal.promotable ? "approved" : "blocked"} /></div>
    <div className={styles.traceGrid}>
      <div><span>来源</span><strong>{proposal.source.title}</strong><small>{proposal.source.id}</small><code>{shortHash(proposal.source.content_hash)}</code></div>
      <div><span>文档</span><strong>{proposal.document.parser_version}</strong><small>{proposal.document.id}</small><code>{shortHash(proposal.document.cleaned_text_hash)}</code></div>
      <div><span>文本分块 {proposal.chunk.chunk_index}</span><strong>{proposal.chunk.status}</strong><small>{proposal.chunk.id}</small><code>{shortHash(proposal.chunk.text_hash)}</code></div>
      <div><span>已批准事实</span><strong>{proposal.fact.status}</strong><small>{proposal.fact.id}</small><code>{shortHash(proposal.fact.statement_hash)}</code></div>
    </div>
    {existing ? <div className={styles.lineage}>
      <dl><div><dt>证据 ID</dt><dd>{existing.evidence.id}</dd></div><div><dt>契约</dt><dd>{existing.lineage.lineage_contract_version}</dd></div><div><dt>幂等键</dt><dd>{existing.lineage.idempotency_key}</dd></div><div><dt>请求 SHA-256</dt><dd>{existing.lineage.promotion_request_hash}</dd></div><div><dt>证据 SHA-256</dt><dd>{existing.lineage.evidence_snapshot_hash}</dd></div><div><dt>提升时间</dt><dd>{formatDate(existing.lineage.promoted_at)}</dd></div></dl>
      <div className={styles.detailActions}><a href={`/projects/${encodeURIComponent(projectId)}?tab=geo&geo_section=placement&placement_stage=brief`}>进入证据包</a></div>
    </div> : <>
...
function blockerLabel(value: string) { return ({ fact_not_approved: "事实尚未批准", fact_review_metadata_missing: "缺少审核记录", source_not_ready: "来源尚未就绪", source_content_hash_missing: "来源缺少内容哈希", pipeline_run_not_succeeded: "处理任务尚未成功", source_content_hash_mismatch: "来源内容哈希不匹配", document_cleaned_text_hash_mismatch: "文档哈希不匹配", chunk_disabled: "文本分块已禁用", chunk_integrity_mismatch: "文本分块完整性校验失败", fact_statement_hash_mismatch: "事实哈希不匹配" } as Record<string, string>)[value] || value; }
