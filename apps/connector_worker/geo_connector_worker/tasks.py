"""Dramatiq actor isolated to Connector sync Jobs."""
...
from geo_connector_worker.composition import build_connector_dispatcher
from geo_core.connectors.routing import CONNECTOR_SYNC_ACTOR, CONNECTOR_SYNC_QUEUE
    ...
    def _heartbeat(self) -> PeriodicHeartbeat:
        with self._lock:
            if self._periodic is None:
                ...
                identity = _heartbeat_identity("connector_worker")
...
@lru_cache(maxsize=1)
def dispatcher():
    return build_connector_dispatcher(
        database_url=_secret_setting("GEO_DATABASE_URL"),
        worker_id=os.getenv(
            "GEO_CONNECTOR_WORKER_ID", f"connector:{socket.gethostname()}"
        ).strip(),
    )
...
def process_connector_sync_job(job_id: str, project_id: str) -> dict[str, object]:
