...
worker 输出会包含 `p0a_readiness_gate`。默认 fixture slice 的 `--sample-size 1` 只用于 smoke test，会因 P0a k=3 要求 fail；`--mode fixture --prompt-limit 1 --sample-size 3` 会通过本地 gate。真实 `--mode api` 必须同样通过 required platforms、必备元数据、`answer_present/surface_triggered`、citation、screenshot/HTML 和 k=3 检查后，才可把该批次视为 design partner 试点证据。Google spike worker 会同时输出 `google_spike_gate` 与 `google_spike_readiness_gate`：前者检查 Google AIO 成功率，后者检查 browser / third_party_api / manual 中是否至少有两条采集路径；`--persist-analysis` 还会把 `score_input_policy` 写入评分审计和报告 Method Disclosure，只有两个 gate 同时通过时 Google answer runs 才能进入主评分分母。默认 browser-only fixture 会通过成功率 gate 但失败两路径 readiness gate，因此只进入 limited coverage 证据附录。
