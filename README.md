...
真实 Perplexity/OpenAI key 到位后，用 `make api-preflight` 跑 AU P0a 最小官方 API smoke（1 个 prompt × Sydney × k=3 × 2 平台）。该目标会先检查 collector health，再要求 `P0ACollectionReadinessGate` 通过；缺 key 时底层 worker 以退出码 `3` 在采集前失败，避免把配置缺失误判为真实采集结论。预检 JSON 会默认写入 `docs/runtime_preflight/api-preflight-latest.json`（可用 `GENO_API_PREFLIGHT_OUTPUT_PATH` 覆盖），内容与 stdout 一致，包含 `collector_health`、`collector_health_gate`、可选 `p0a_readiness_gate`、`preflight_summary`、`preflight_audit_checklist` 和 `preflight_output_path`；`preflight_summary` 会给出本次预检 phase、exit code、是否可进入 design partner、失败原因摘要和建议下一步，`preflight_audit_checklist` 会把 collector health、P0a readiness、collection failures、output path 和 replay args 折叠成可审计 checklist，便于复盘是否可扩大到 design partner；该目录下的运行时 JSON 已被 gitignore，适合保留本地审计证据但不提交凭证相关上下文。
