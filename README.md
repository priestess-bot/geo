...
运行时预警已补成“派生 alert + append-only 管理事件 + 显式通知入队 + 计划任务扫描”模型：`GET /v1/runtime-alerts` 会基于最新 `VisibilityScoreSnapshot`、`ScoreContribution`、`AnswerAnalysis`、`SourceGap`、`CompetitorBenchmark` 和 `ActionRecommendation` 派生 `brand_absent`、`low_recommendation_rate`、`negative_sentiment`、`source_gap`、`competitor_pressure` 五类 alert，返回 severity、阈值、指标值、证据 refs、相关 action、评分审计 refs 和最近管理事件。`POST /v1/runtime-alerts/{alert_id}/events` 会把 acknowledged/resolved/snoozed/reopened 写入 `runtime_alert_events`，并追加 `runtime_alert_event_recorded` 审计事件，不改写派生 alert 或原始证据。`POST /v1/runtime-alerts/notifications` 可把当前未 resolved/snoozed 的派生 alert 入队为 `runtime_notifications.notification_type=runtime_alert`，并复用 `runtime_notification_subscriptions/deliveries` 生成 webhook delivery；订阅表单默认支持 `report_export_job,runtime_alert`。`workers/notification_worker/run_runtime_alert_notifications.py` 与 `make runtime-alert-notification-worker` 可按项目或 AU market 扫描并批量调用同一入队逻辑，Compose `scheduler` profile 已提供对应服务入口。Runtime Console 的 Runtime Alerts 面板会展示预警、证据链、管理历史、记录表单和通知入队按钮；SLA 升级、邮件/Slack 原生渠道和 Temporal/长驻队列化调度仍待后续接入。
