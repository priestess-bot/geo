...
运行时预警已补成只读解释模型：`GET /v1/runtime-alerts` 会基于最新 `VisibilityScoreSnapshot`、`ScoreContribution`、`SourceGap`、`CompetitorBenchmark` 和 `ActionRecommendation` 派生 `brand_absent`、`low_recommendation_rate`、`source_gap`、`competitor_pressure` 四类 alert，返回 severity、阈值、指标值、证据 refs、相关 action 和评分审计 refs。Runtime Console 的 Runtime Alerts 面板会展示这些预警与证据链；当前不新增告警表，不做实时推送、通知、SLA 或订阅策略。
...
Runtime Alerts 是只读派生层，不新增事实表：后端用最新评分快照和贡献项判断品牌缺失、推荐率过低，用 source gaps 输出信源缺口风险，用 competitor benchmarks 对比竞品提及率并识别竞品压制；每条 alert 都返回 `rule_version=runtime_alerts_v1`、`evidence_refs`、`related_actions` 和评分审计事件，便于从预警一路回查到分数、信源缺口、竞品 benchmark、answer run 和 action/retest 链路。
