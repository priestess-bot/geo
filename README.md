...
补充：报告导出任务队列已落最小执行闭环，并补了项目内通知 inbox。`report_export_jobs` 记录项目、目标报告、artifact type、template、筛选条件、排序、请求人、状态、artifact URL、错误信息、attempt count、max attempts、lease expiry 和 next attempt；`GET/POST /v1/report-export-jobs/runtime` 可查询或排队导出任务，`GET /v1/report-export-jobs/runtime/stats` 可读回 queued/running/dead_letter、可重试、过期 running、达到最大尝试次数和最早排队时间，`POST /v1/report-export-jobs/runtime/{job_id}/status` 可把任务推进为 `queued/running/succeeded/failed/dead_letter/cancelled`，并写入 `report_export_job_queued` / `report_export_job_status_updated` 审计事件。任务进入 `succeeded/failed/dead_letter/cancelled` 终态时会同步写入 `runtime_notifications`，生成 `runtime_notification_created` 审计事件；`GET /v1/runtime-notifications` 可读取项目内 inbox 与 unread count，`POST /v1/runtime-notifications/{notification_id}/status` 可标记 read/unread 并写 `runtime_notification_status_updated`。Runtime Console 已新增 `Report Export Queue` 和 `Runtime Notifications` 面板，支持按当前项目和筛选条件排队导出、查看最近任务、attempt/lease/next attempt、队列指标、终态通知和 read/unread 状态；`make report-export-worker` 会运行 `workers/report_export_worker/run_report_export_jobs.py`，用 `FOR UPDATE SKIP LOCKED` 领取最早可执行 queued job 或 lease 过期的 running job，渲染对应 runtime report artifact，配置对象存储时归档到 `s3://.../report-artifacts/...`，未配置时写入 `runtime-report-artifact://...` fallback URL，失败时按 `--retry-backoff-seconds` 回到 queued，达到 `--max-attempts` 后进入 `dead_letter`。该能力仍不等同于 Temporal、长驻 worker、并发池、外部推送订阅、SLA 或公网 CDN 交付。
