...
Runtime Console 的 Brand Assets 面板已接入上述审计型版本库，可展示 Logo 历史版本、active 标记、来源文件/content hash 和激活操作；Asset Register / Asset Library 面板已接入 `project_brand_assets` 专表，可登记和读取项目级素材分类、状态、hash、storage version 与最近审计事件。这里的“白标资产库增强待接”后续特指图片裁剪与预览、对象存储原生版本、CDN 签名、公网交付策略、病毒扫描和跨客户素材权限。
