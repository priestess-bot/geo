...
运行时健康检查分两层：`/health` 只证明 API 进程可响应；`/ready` 会执行 PostgreSQL `SELECT 1`，DB 不可用时返回 503；`/v1/runtime-diagnostics` 返回 database/object_store/runtime_auth 三类诊断，包含连接池配置快照、对象存储配置完整性和 runtime auth/JWT 配置状态。对象存储诊断默认只做配置检查，不主动访问 MinIO/S3，避免健康检查产生副作用：
