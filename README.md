...
如需把同一批成功采集记录继续解析、评分并保存 `AnswerAnalysis`、`VisibilityScoreSnapshot` 与 `ScoreContribution`：
...
这些接口从 PostgreSQL 读取 `AnswerRun -> PromptQuestion -> RawAnswer -> Citation/Asset/Log/Cost/Audit` 聚合页、`VisibilityScoreSnapshot -> ScoreContribution -> ScoreSnapshotRun -> AnswerRun/PromptQuestion -> AnswerAnalysis/AuditEvent` 评分解释页、`SourceGraph -> SourceGraphEvidence -> SourceGap -> CompetitorBenchmark` 图谱/竞品页，以及 `ReportExport -> ReportEvidence -> ScoreSnapshot -> CitationGraph` 报告快照页；未配置 `DATABASE_URL` 时返回 503。
