...
默认地址：API `http://localhost:8000/health`，控制台 `http://localhost:3000`。控制台会读取 runtime evidence / score / graph / report / action / content / traceability API，并在 Traceability Detail 面板展示报告到评分、证据、图谱、行动、内容、审计事件和 evidence links 的聚合链路；如果还没有数据，先运行 worker profile 写入一批 fixture runtime 数据。
