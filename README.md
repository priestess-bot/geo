...
补充：报告导出任务队列已落最小持久化闭环。`report_export_jobs` 记录项目、目标报告、artifact type、template、筛选条件、排序、请求人、状态、artifact URL 和错误信息；`GET/POST /v1/report-export-jobs/runtime` 可查询或排队导出任务，`POST /v1/report-export-jobs/runtime/{job_id}/status` 可把任务推进为 `queued/running/succeeded/failed/cancelled`，并写入 `report_export_job_queued` / `report_export_job_status_updated` 审计事件。Runtime Console 已新增 `Report Export Queue` 面板，支持按当前项目和筛选条件排队导出、查看最近任务和追加状态更新。该能力目前只负责可审计任务状态，不等同于后台 worker、Temporal、重试租约、并发控制、死信队列或真实异步 PDF 渲染。
