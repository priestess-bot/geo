...
补充：白标资产库已先落一层审计型 Logo 版本库最小版。`GET /v1/project-brand-kits/runtime/assets` 会从 `project_brand_logo_uploaded` 和 `project_brand_logo_version_activated` 审计事件读回项目 Logo 版本、来源文件、content hash、上传人和 active 状态；`POST /v1/project-brand-kits/runtime/assets/activate` 可把历史 Logo URI 重新激活为 Brand Kit 默认值，并追加 `project_brand_logo_version_activated` 审计事件。该能力用于 MVP 交付复盘，不等同于完整素材库、图片裁剪/预览、病毒扫描、CDN、公网对象存储 presigned URL 或对象存储原生版本控制。
...
Runtime Console 的 Brand Assets 面板已接入上述审计型版本库，可展示 Logo 历史版本、active 标记、来源文件/content hash 和激活操作。这里的“白标资产库版本管理待接”后续特指完整资产库能力：素材分类、裁剪与预览、对象存储原生版本、CDN 签名、公网交付策略和跨客户素材权限。
