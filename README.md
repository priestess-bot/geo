...
人工复核已从单纯留痕推进到最小只读队列：`GET /v1/human-reviews/runtime/queue` 会从 `visibility_score_snapshots` 与 `content_drafts` 聚合待审对象，返回 `target_type/target_id/title/queue_status/priority/reason/latest_review/evidence_refs`，并在 Runtime Console 的 Human Review 面板展示 Review Queue。队列不改写评分、草稿或审核状态；正式状态回写、分配、通知、审批流和抽样校准仍放在后续阶段。
