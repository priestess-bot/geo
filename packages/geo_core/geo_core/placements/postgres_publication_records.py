...
class PostgresPublicationRecordMixin:
    ...
    def create_publication_request(self, **values: Any) -> PublicationRequest:
        ...
        for field in ("workflow_status", "content_json", "input_snapshot"):
            context.pop(field)
    ...
    def enqueue_verification(
        ...
        context = _one(
            self._db.execute(
                """SELECT campaign_id, opportunity_id, submitted_url, status
                   FROM publication_submissions
        ...
        active = _many(
            self._db.execute(
                """SELECT job.id, job.project_id, job.campaign_id, job.kind,
                          job.status, job.idempotency_key
        ...
        if active:
            record = active[0]
            if record.pop("idempotency_key") != idempotency_key:
                raise PlacementConflict("publication verification is already active")
            return JobReference(**record)
