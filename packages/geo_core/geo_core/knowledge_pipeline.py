...
from geo_core.object_store import S3CompatibleObjectStore, StoredObject
...
class KnowledgePipelineRepository:
    ...
    def set_runtime_scope(self, *, actor_id: str, project_id: str | None = None, roles: str = "owner,admin,analyst") -> None:
        with self.connection.cursor() as cursor:
            ...
            cursor.execute("SELECT set_config('geo.runtime_project_access_control', 'true', false)")
            cursor.execute("SELECT set_config('app.actor_id', %s, false)", (actor_id,))
            cursor.execute("SELECT set_config('geo.runtime_actor_id', %s, false)", (actor_id,))
            ...
            if project_id:
                ...
                cursor.execute("SELECT set_config('geo.runtime_project_id', %s, false)", (project_id,))
            else:
                cursor.execute("SELECT set_config('app.project_id', '', false)")
                cursor.execute("SELECT set_config('geo.runtime_project_id', '', false)")
            self.connection.commit()
    ...
    def set_maintenance_scope(self, *, worker_id: str) -> None:
        ...
        with self.connection.cursor() as cursor:
            ...
            cursor.execute("SELECT set_config('geo.runtime_actor_id', %s, false)", (worker_id,))
            cursor.execute("SELECT set_config('app.project_id', '', false)")
            cursor.execute("SELECT set_config('geo.runtime_project_id', '', false)")
