"""OpenRouter Web Search adapter for search aggregation.

Uses ``httpx`` to call OpenRouter's chat completions endpoint with the
``openrouter:web_search`` server tool enabled, then normalizes the response into
the same ``AiOverviewResult`` model used by Google/Bing adapters.

Important naming note: this is *not* OpenAI's official Responses API web search,
nor is it ChatGPT webpage Search. It is OpenRouter acting as a proxy to an
OpenAI model while invoking OpenRouter's own web_search tool.
"""
...
class OpenRouterWebSearchProvider(SearchProvider):
    """Fetch AI answers with web citations through OpenRouter."""
    ...
    async def fetch_ai_overview(self, query: AiOverviewQuery) -> AiOverviewResult:
        """Call OpenRouter and normalize the answer into ``AiOverviewResult``."""
...
def _extract_citations(payload: dict[str, Any]) -> list[AiOverviewReference]:
    """Return deduplicated references from ``annotations.url_citation``."""
