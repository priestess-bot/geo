...
def _api_snapshot_payload(
    *,
    ...
    snapshot_url = f"geo-api-snapshot://{collector_backend_id}/{payload_hash}.html"
...
class FixtureAICollector:
    ...
    def collect(
        ...
        *,
        ...
        evidence_prefix = f"s3://geo-fixtures/{self._backend_id}/{market.market_code}/{city}/{prompt_slug}"
...
class PlaywrightChatGPTSearchCollector:
    def __init__(
        ...
        *,
        ...
        self._enabled = _env_truthy(os.getenv("GEO_BROWSER_COLLECTOR_ENABLED")) if enabled is None else enabled
        self._start_url = (
            _optional_env(start_url)
            or _optional_env(os.getenv("GEO_BROWSER_START_URL"))
            or _optional_env(os.getenv("CHATGPT_BROWSER_URL"))
            or "https://chatgpt.com/"
        )
        self._browser_name = (
            _optional_env(browser_name)
            or _optional_env(os.getenv("GEO_BROWSER_NAME"))
            or "chromium"
        )
        self._prompt_selector = _optional_env(prompt_selector) or _optional_env(
            os.getenv("GEO_BROWSER_PROMPT_SELECTOR")
        )
        self._submit_selector = _optional_env(submit_selector) or _optional_env(
            os.getenv("GEO_BROWSER_SUBMIT_SELECTOR")
        )
        self._answer_selector = _optional_env(answer_selector) or _optional_env(
            os.getenv("GEO_BROWSER_ANSWER_SELECTOR")
        )
        self._citation_selector = _optional_env(citation_selector) or _optional_env(
            os.getenv("GEO_BROWSER_CITATION_SELECTOR")
        )
        self._storage_state_path = _optional_env(storage_state_path) or _optional_env(
            os.getenv("GEO_BROWSER_STORAGE_STATE")
        )
        self._artifact_dir = _optional_env(artifact_dir) or _optional_env(os.getenv("GEO_BROWSER_ARTIFACT_DIR"))
        self._headless = (
            not _env_truthy(os.getenv("GEO_BROWSER_HEADFUL"))
            if headless is None
            else headless
        )
        self._timeout_seconds = timeout_seconds or float(os.getenv("GEO_BROWSER_TIMEOUT_SECONDS") or "45")
    ...
    def capabilities(self) -> dict[str, object]:
        return {
            ...
            "supports_html_snapshot": True,
            "access_method": "browser",
            "requires_enable_env": "GEO_BROWSER_COLLECTOR_ENABLED",
            "required_selectors": ["GEO_BROWSER_PROMPT_SELECTOR", "GEO_BROWSER_ANSWER_SELECTOR"],
            "optional_selectors": ["GEO_BROWSER_SUBMIT_SELECTOR", "GEO_BROWSER_CITATION_SELECTOR"],
            "artifact_dir_env": "GEO_BROWSER_ARTIFACT_DIR",
        }
    ...
    def _ensure_ready(self) -> None:
        ...
        raise CollectorConfigurationError(
            ...
            f"{health}. Set GEO_BROWSER_COLLECTOR_ENABLED=1, install Playwright, "
            "and configure GEO_BROWSER_PROMPT_SELECTOR / GEO_BROWSER_ANSWER_SELECTOR."
        )
    ...
    def _artifact_urls(
        ...
        *,
        ...
        if not self._artifact_dir:
            return (
                f"geo-browser-snapshot://{self.id()}/{snapshot_id}.html",
                f"geo-browser-screenshot://{self.id()}/{snapshot_id}.png",
            )
...
class DeepSeekChatCollector:
    ...
    def _load_api_key() -> str | None:
        ...
        if direct:
            return direct
        key_path = os.getenv("GEO_DEEPSEEK_API_KEY_FILE", "").strip()
        if not key_path:
            return None
        try:
    ...
    def collect(
        ...
        *,
        ...
        if not self._api_key:
            raise CollectorConfigurationError("DEEPSEEK_API_KEY or GEO_DEEPSEEK_API_KEY_FILE is required")
        response = self._http_client.post_json(
            url=self._endpoint,
            headers={"Authorization": f"Bearer {self._api_key}"},
            payload=self.build_payload(prompt=prompt, market=market, city=city, language=language),
