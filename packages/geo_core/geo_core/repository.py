...
from geo_core.auth import assert_auth_writes_enabled, surface_for_role
...
from geo_core.runtime_project_access_repository import (
...
from geo_core.scoring import get_score_formula, list_score_weight_profiles, normalize_score_weights
...
def _runtime_notification_slack_payload(
    *,
    ...
    title = str(notification.get("title") or "GEO runtime notification").strip()
    ...
    return {
                ...
                "elements": [
                    {
                        "type": "mrkdwn",
                        "text": (
                            f"GEO runtime · notification `{notification.get('id')}` · "
        ...
        "metadata": {
            "event_type": "geo_runtime_notification",
            "event_payload": {
...
def _runtime_notification_email_payload(
    *,
    ...
    headers = {
        "X-GEO-Notification-Id": str(notification.get("id")),
        "X-GEO-Project-Id": str(notification.get("project_id")),
        "X-GEO-Notification-Type": notification_type,
        "X-GEO-Severity": severity,
        "X-GEO-Email-Template-Version": rendered_email.template_version,
    }
    ...
    if manage_preferences_url or control_metadata.get("email_preferences_url"):
        headers["X-GEO-Notification-Preferences-Url"] = manage_preferences_url or control_metadata["email_preferences_url"]
...
class PostgresEvidenceRepository(RuntimeProjectAccessRepositoryMixin):
    """DB-API style repository for the GEO runtime evidence chain."""
    ...
    def get_project_member_role(self, *, project_id: str, actor_id: str) -> str | None:
        ...
        with self.connection.cursor() as cursor:
            cursor.execute(
                """
                SELECT role FROM geo_runtime_resolve_header_member(%s::uuid, %s)
                """,
                (_uuid(project_id), actor_id),
            )
            row = cursor.fetchone()
    ...
    def get_project_member_tenant_id(self, *, project_id: str, actor_id: str) -> str | None:
        ...
        with self.connection.cursor() as cursor:
            cursor.execute(
                "SELECT tenant_id FROM geo_runtime_resolve_header_member(%s::uuid, %s)",
                (_uuid(project_id), actor_id),
            )
            row = cursor.fetchone()
        if not row:
    ...
    def create_runtime_project_member_invitation(
        ...
        status = "pending"
        invite_token = f"geo-invite-{uuid4().hex}"
    ...
    def send_runtime_project_member_invitation_email(
        ...
        smtp_env_prefix = email_input.smtp_env_prefix.strip() or "GEO_NOTIFICATION_SMTP"
        if not project_id:
            raise ValueError("project_id is required")
        if not invitation_id:
            raise ValueError("invitation_id is required")
        ...
        with self.connection.cursor() as cursor:
            ...
            delivery_result = send_runtime_email_message(
                ...
                text=rendered_email.text,
                headers={
                    "X-GEO-Project-Id": project_id,
                    "X-GEO-Invitation-Id": invitation_id,
                    "X-GEO-Invitation-Token-Hash": invite_token_hash,
                    "X-GEO-Email-Template-Version": rendered_email.template_version,
                },
    ...
    def save_project_brand_kit(self, brand_kit: RuntimeProjectBrandKitInput) -> RuntimeProjectBrandKit:
        ...
        prepared_by = brand_kit.prepared_by.strip() or "GEO SaaS AU"
    ...
    def upload_project_brand_logo(self, upload: RuntimeProjectBrandLogoUpload) -> RuntimeProjectBrandKit:
        ...
        with self.connection.cursor() as cursor:
            ...
            after = {
                ...
                "prepared_by": existing.get("prepared_by") or "GEO SaaS AU",
                "logo_url": logo_url,
    ...
    def _index_knowledge_fact_embeddings(
        ...
        *,
        cursor: DbCursor,
        facts: tuple[LocalizedKnowledgeFact, ...],
        actor_id: str = "geo-core.knowledge",
        embedding_model: str = KNOWLEDGE_EMBEDDING_MODEL,
    ) -> AuditEvent | None:
        if not facts:
            return None
