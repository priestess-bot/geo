...
def _render_api_snapshot_html(*, record: RawEvidenceRecord, asset: EvidenceAsset) -> str:
    snapshot = record.raw_answer.raw_payload.get("_geo_api_snapshot")
    ...
    return "\n".join(
        [
            ...
            "<head>",
            '  <meta charset="utf-8">',
            f"  <title>GEO API Snapshot {html.escape(record.answer_run.id)}</title>",
            "</head>",
            "<body>",
            "  <h1>GEO Official API Response Snapshot</h1>",
            "  <dl>",
