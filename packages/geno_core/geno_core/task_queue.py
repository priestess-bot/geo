...
def task_queue_enabled() -> bool:
    return os.getenv("GENO_TASK_QUEUE_ENABLED", "false").strip().lower() in TASK_QUEUE_ENABLED_VALUES


def task_queue_required() -> bool:
    deployment = os.getenv("GENO_DEPLOYMENT_ENVIRONMENT", "development").strip().lower()
    return deployment in {"production", "prod"} or (
        os.getenv("GENO_TASK_QUEUE_REQUIRED", "false").strip().lower() in TASK_QUEUE_ENABLED_VALUES
    )


def task_queue_broker_url() -> str:
    return os.getenv("GENO_TASK_QUEUE_BROKER_URL", "").strip()
...
def dispatch_background_task(task_name: str) -> TaskDispatchReceipt:
    ...
    if not task_queue_enabled():
        if task_queue_required():
            raise RuntimeError("GENO_TASK_QUEUE_ENABLED=1 is required")
