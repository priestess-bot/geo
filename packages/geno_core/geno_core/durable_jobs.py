...
_INTERNAL_LEASE_ENVIRONMENT_KEYS = (
    "GENO_INTERNAL_DURABLE_JOB_TABLE",
    "GENO_INTERNAL_DURABLE_JOB_ID",
    "GENO_INTERNAL_DURABLE_PROJECT_ID",
    "GENO_INTERNAL_DURABLE_WORKER_ID",
    "GENO_INTERNAL_DURABLE_LEASE_TOKEN",
    "GENO_INTERNAL_DURABLE_ATTEMPT_COUNT",
    "GENO_INTERNAL_DURABLE_LEASE_SECONDS",
)
...
def internal_lease_environment(claim: LeaseClaim, *, lease_seconds: int) -> dict[str, str]:
    ...
    return {
        "GENO_INTERNAL_DURABLE_JOB_TABLE": claim.spec.table,
        "GENO_INTERNAL_DURABLE_JOB_ID": str(claim.job_id),
        "GENO_INTERNAL_DURABLE_PROJECT_ID": str(claim.project_id),
        "GENO_INTERNAL_DURABLE_WORKER_ID": claim.worker_id,
        "GENO_INTERNAL_DURABLE_LEASE_TOKEN": str(claim.lease_token),
        "GENO_INTERNAL_DURABLE_ATTEMPT_COUNT": str(claim.attempt_count),
        "GENO_INTERNAL_DURABLE_LEASE_SECONDS": str(max(3, int(lease_seconds))),
    }
...
def lease_claim_from_internal_environment(
    ...
    try:
        table = env.get("GENO_INTERNAL_DURABLE_JOB_TABLE", "").strip()
        if not table:
            return None
        required = {
            "job_id": env.get("GENO_INTERNAL_DURABLE_JOB_ID", "").strip(),
            "project_id": env.get("GENO_INTERNAL_DURABLE_PROJECT_ID", "").strip(),
            "worker_id": env.get("GENO_INTERNAL_DURABLE_WORKER_ID", "").strip(),
            "lease_token": env.get("GENO_INTERNAL_DURABLE_LEASE_TOKEN", "").strip(),
            "attempt_count": env.get("GENO_INTERNAL_DURABLE_ATTEMPT_COUNT", "").strip(),
            "lease_seconds": env.get("GENO_INTERNAL_DURABLE_LEASE_SECONDS", "").strip(),
        }
