-- A Metric Judge batch needs no Arbiter when every admitted Judge has returned
-- the same parsed output.  Recompile this RPC from version-controlled source
-- rather than transforming pg_get_functiondef output: PostgreSQL may format an
-- equivalent function definition differently across supported releases.
...
DECLARE agreed_candidate_id uuid;
DECLARE agreed_output_hash text;
BEGIN
    ...
    ELSE
        ...
        WHERE judge.project_id = p_project_id AND judge.batch_id = child.batch_id
          AND judge.role = 'metric_judge' AND judge.status = 'succeeded'
          AND judge.output_hash IS NOT NULL;
        IF judge_count >= 2 AND judge_output_count = 1
           AND NOT EXISTS (
               SELECT 1 FROM workflow_c_metric_model_children AS judge
               WHERE judge.project_id = p_project_id AND judge.batch_id = child.batch_id
                 AND judge.role = 'metric_judge'
            ...
            SELECT judge.candidate_id, judge.output_hash
            INTO agreed_candidate_id, agreed_output_hash
            FROM workflow_c_metric_model_children AS judge
            WHERE judge.project_id = p_project_id AND judge.batch_id = child.batch_id
              AND judge.role = 'metric_judge' AND judge.status = 'succeeded'
              AND judge.output_hash IS NOT NULL
            ...
            LIMIT 1;
            UPDATE workflow_c_metric_judge_batches
            SET status = 'completed', selected_candidate_id = agreed_candidate_id,
                selected_output_hash = agreed_output_hash,
                aggregate_version = workflow_c_metric_judge_batches.aggregate_version + 1,
                completed_at = clock_timestamp()
            WHERE project_id = p_project_id AND id = child.batch_id
              AND status IN ('queued', 'running');
