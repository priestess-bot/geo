-- Connection tests must prove every active Google Scope bound to a Connection.
-- Existing v1 jobs are left untouched.  A live v1 job would be executable by
-- neither the old nor the new contract during a rolling migration, so fail
-- before replacing the enqueue function instead of silently changing it.
DO $$
BEGIN
    IF EXISTS (
        SELECT 1
         ...
         WHERE test.status IN ('queued', 'running')
           AND spec.spec_payload->>'schema_version' IS DISTINCT FROM '2'
    ) THEN
        RAISE EXCEPTION
            'cannot upgrade Connector connection-test scopes while nonterminal v1 Jobs exist'
            USING ERRCODE = '55000';
    END IF;
END;
$$;
...
BEGIN
    ...
    SELECT jsonb_agg(
     ...
     WHERE scope.project_id = p_project_id
       AND scope.connection_id = connection_row.id
       AND scope.status = 'active';
    IF frozen_scopes IS NULL OR jsonb_array_length(frozen_scopes) < 1 THEN
        RAISE EXCEPTION 'Connector Connection must have at least one active Scope'
            USING ERRCODE = '23514';
    END IF;

    payload := jsonb_build_object(
        'schema_version', 2, 'kind', 'connector.connection_test',
...
) IS 'Enqueues a scope-bound Connector connection test with an immutable ordered Scope snapshot.';
