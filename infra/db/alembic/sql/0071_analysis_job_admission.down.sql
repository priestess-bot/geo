-- Restore 0034's generic producer exactly: comparison/drift were not admitted
-- through it before this revision.  The helpers are dropped only after the
-- producer no longer depends on them.
...
BEGIN
    ...
    END IF;
    expected_wakeup := jsonb_build_object('job_id', durable.id::text, 'project_id', p_project_id::text);
    ...
    IF stored_outbox.id IS NULL OR stored_outbox.job_id <> durable.id
       OR stored_outbox.topic <> p_kind OR stored_outbox.payload IS DISTINCT FROM expected_wakeup THEN
...
DROP FUNCTION geo_workflow_c_analysis_stratum_is_valid(jsonb);
DROP FUNCTION geo_workflow_c_analysis_decimal_is_valid(jsonb);
DROP FUNCTION geo_workflow_c_analysis_nonempty_text_is_valid(jsonb);
