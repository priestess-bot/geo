CREATE FUNCTION geo_observation_source_stratum_v3_canonical(
    capture_method text,
    platform text,
...
LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $$
    SELECT
        ...
        || ',"platform_detail":'
        || COALESCE(to_jsonb(platform_detail)::text, 'null')
        ...
        || ',"surface_detail":'
        || COALESCE(to_jsonb(surface_detail)::text, 'null')
        || ',"surface_kind":' || to_jsonb(surface_kind)::text || '}'
$$;

CREATE FUNCTION geo_observation_source_stratum_v3_hash(
    capture_method text,
    platform text,
    platform_detail text,
    surface text,
        ...
        digest(
            convert_to(
                geo_observation_source_stratum_v3_canonical(
                    capture_method, platform, platform_detail, surface,
                    surface_kind, surface_detail, engine, configured_model_state,
                    configured_model, reported_model_state, reported_model, locale,
                    region, language, device, client_kind, search_enabled, search_mode
                ),
                'UTF8'
            ),
            'sha256'
...
CREATE FUNCTION geo_source_stratum_v3_json_valid(source_stratum jsonb)
RETURNS boolean
LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $$
    SELECT jsonb_typeof(source_stratum) = 'object'
       AND source_stratum = jsonb_build_object(
            ...
            'platform_detail', source_stratum ->> 'platform_detail',
            'surface', source_stratum ->> 'surface',
            'surface_kind', source_stratum ->> 'surface_kind',
            'surface_detail', source_stratum ->> 'surface_detail',
       ...
       AND (
            (source_stratum ->> 'platform' = 'other'
                AND geo_ascii_nonempty(source_stratum ->> 'platform_detail'))
            OR (source_stratum ->> 'platform' <> 'other'
                AND source_stratum ->> 'platform_detail' IS NULL)
       )
       AND (
            (source_stratum ->> 'surface' = 'other'
                AND geo_ascii_nonempty(source_stratum ->> 'surface_detail'))
            OR (source_stratum ->> 'surface' <> 'other'
                AND source_stratum ->> 'surface_detail' IS NULL)
       )
       AND geo_observation_surface_matches(
            source_stratum ->> 'capture_method',
            source_stratum ->> 'platform',
            source_stratum ->> 'platform_detail',
            source_stratum ->> 'surface',
            source_stratum ->> 'surface_kind',
            source_stratum ->> 'surface_detail'
       )
...
CREATE FUNCTION geo_source_stratum_v3_hash_from_json(source_stratum jsonb)
RETURNS text
LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $$
    SELECT geo_observation_source_stratum_v3_hash(
        source_stratum ->> 'capture_method',
        source_stratum ->> 'platform',
        source_stratum ->> 'platform_detail',
        source_stratum ->> 'surface',
...
CREATE FUNCTION geo_source_strata_v3_inventory_hash(source_strata jsonb)
RETURNS text
LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $$
    WITH canonical AS (
        SELECT geo_observation_source_stratum_v3_canonical(
            item ->> 'capture_method', item ->> 'platform',
            item ->> 'platform_detail', item ->> 'surface',
            item ->> 'surface_kind', item ->> 'surface_detail', item ->> 'engine',
...
ALTER TABLE monitoring_observations
    ...
    ALTER COLUMN source_contract_version SET DEFAULT 'geo-observation-source-v3',
    ADD CONSTRAINT monitoring_observations_source_contract_version_check CHECK (
        source_contract_version IN (
            'legacy-v1', 'geo-observation-source-v2', 'geo-observation-source-v3'
        )
    );
...
BEGIN
    ...
    FOR item IN SELECT value FROM jsonb_array_elements(NEW.source_strata_snapshot) LOOP
        IF NOT geo_source_stratum_v3_json_valid(item) THEN
            RAISE EXCEPTION 'protocol source stratum is invalid'
                USING ERRCODE = '23514';
        END IF;
    END LOOP;
    IF (
        SELECT count(DISTINCT geo_source_stratum_v3_hash_from_json(value))
        ...
        RAISE EXCEPTION 'protocol source strata must be unique'
            USING ERRCODE = '23514';
    END IF;
    IF NEW.source_strata_hash <> geo_source_strata_v3_inventory_hash(
...
LANGUAGE plpgsql AS $$
BEGIN
    IF NEW.source_contract_version <> 'geo-observation-source-v3'
       OR NEW.capture_method IN ('unknown', 'official_report_import') THEN
        RAISE EXCEPTION 'new answer observations require a typed non-official source'
            USING ERRCODE = '23514';
    END IF;
    IF (NEW.platform = 'other' AND NOT geo_ascii_nonempty(NEW.platform_detail))
       OR (NEW.platform <> 'other' AND NEW.platform_detail IS NOT NULL)
       OR (NEW.surface = 'other' AND NOT geo_ascii_nonempty(NEW.surface_detail))
       OR (NEW.surface <> 'other' AND NEW.surface_detail IS NOT NULL)
       OR NOT geo_observation_surface_matches(
    ...
    IF (NEW.raw_evidence_kind = 'answer' AND (
        ...
        RAISE EXCEPTION 'observation raw evidence does not match its kind'
            USING ERRCODE = '23514';
    END IF;
    IF NEW.source_stratum_hash IS DISTINCT FROM
        geo_observation_source_stratum_v3_hash(
            NEW.capture_method, NEW.platform, NEW.platform_detail, NEW.surface,
            NEW.surface_kind, NEW.surface_detail, NEW.engine,
    ...
    IF NEW.eligible AND NOT EXISTS (
        ...
        WHERE protocol.id = NEW.protocol_id AND protocol.project_id = NEW.project_id
          ...
          AND geo_source_stratum_v3_hash_from_json(strata.item)
...
LANGUAGE plpgsql AS $$
BEGIN
    IF NEW.source_contract_version <> 'legacy-v1' AND (
...
BEGIN
    IF NEW.source_contract_version <> 'geo-observation-source-v3'
       ...
       OR NOT geo_source_stratum_v3_json_valid(NEW.source_stratum)
       OR NEW.capture_method <> NEW.source_stratum ->> 'capture_method'
       OR NEW.source_stratum_hash IS DISTINCT FROM
            geo_source_stratum_v3_hash_from_json(NEW.source_stratum)
       OR NOT EXISTS (
            SELECT 1
            FROM monitoring_protocols AS protocol,
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE snapshot.id = NEW.snapshot_id
          ...
          AND observation.source_contract_version = 'geo-observation-source-v3'
...
REVOKE ALL ON FUNCTION
    ...
    geo_source_stratum_v3_hash_from_json(jsonb),
    geo_source_strata_v3_inventory_hash(jsonb)
