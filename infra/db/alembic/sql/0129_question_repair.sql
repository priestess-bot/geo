-- Keep the original candidate and dedup evidence immutable while allowing a
-- corrected possible duplicate to re-enter review only after a new revision.
...
DECLARE
    ...
    semantic_duplicate_found boolean := false;
    repair_revision_found boolean := false;
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
        ...
        IF OLD.workflow_status IS DISTINCT FROM NEW.workflow_status THEN
            IF OLD.workflow_status = 'rejected'
               AND NEW.workflow_status = 'pending_review' THEN
                SELECT EXISTS (
                    SELECT 1
                    FROM knowledge_question_candidate_revisions AS revision
                    WHERE revision.candidate_id = OLD.id
                      AND revision.generated_by_job_id = OLD.generated_by_job_id
                      AND revision.project_id = OLD.project_id
                      AND revision.campaign_id = OLD.campaign_id
                      AND OLD.reviewed_at IS NOT NULL
                      AND revision.created_at > OLD.reviewed_at
                      AND revision.query_text_hash <> OLD.query_text_hash
                      AND revision.query_text_hash = encode(
                            digest(convert_to(revision.query_text, 'UTF8'), 'sha256'),
                            'hex'
                      )
                      AND NOT EXISTS (
                          SELECT 1
                          FROM knowledge_question_candidate_revisions AS newer
                          WHERE newer.candidate_id = revision.candidate_id
                            AND newer.revision_number > revision.revision_number
                      )
                ) INTO repair_revision_found;
                IF OLD.dedup_status <> 'possible_duplicate'
                   OR NOT repair_revision_found
                   OR NEW.reviewed_by IS NOT NULL
                   OR NEW.review_notes IS NOT NULL
                   OR NEW.reviewed_at IS NOT NULL THEN
                    RAISE EXCEPTION 'invalid Question candidate repair transition'
                        USING ERRCODE = '23514';
                END IF;
            ELSIF OLD.workflow_status <> 'pending_review'
    ...
    SELECT EXISTS (
        SELECT 1
        FROM knowledge_question_candidates AS candidate
        WHERE candidate.generated_by_job_id = NEW.generated_by_job_id
          AND candidate.project_id = NEW.project_id
          AND candidate.campaign_id = NEW.campaign_id
          AND candidate.semantic_fingerprint = NEW.semantic_fingerprint
    ) INTO semantic_duplicate_found;

    IF (NEW.dedup_status = 'unique' AND (
            (NEW.nearest_similarity IS NOT NULL
             AND NEW.nearest_similarity >= duplicate_threshold)
            OR semantic_duplicate_found
       ))
       OR (NEW.dedup_status = 'possible_duplicate' AND (
            NEW.nearest_similarity IS NULL
            OR (NEW.nearest_similarity < duplicate_threshold
                AND NOT semantic_duplicate_found)
       ))
       OR (NEW.dedup_status = 'exact_duplicate' AND (
        ...
        RAISE EXCEPTION 'Question candidate dedup result differs from its frozen threshold or semantic evidence'
            USING ERRCODE = '23514';
    END IF;
    RETURN NEW;
END;
