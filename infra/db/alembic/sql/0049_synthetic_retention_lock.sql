-- The active-job lookup cannot lock a row that does not exist. Serialize each
-- project's lookup/create/wake transaction so concurrent relays cannot both
-- choose replay_nonce 0 or emit duplicate maintenance wakeups.
...
BEGIN
    ...
    LOOP
        PERFORM pg_advisory_xact_lock(hashtextextended(
            'synthetic-artifact-maintenance:' || candidate.project_id::text, 0
        ));
