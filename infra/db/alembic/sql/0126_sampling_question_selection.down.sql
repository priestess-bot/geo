-- Restore the pre-0126 function.  Existing Suites are never rewritten; the
-- guard prevents a downgrade from discarding the explicit selection marker.
DO $$
BEGIN
    IF EXISTS (
        SELECT 1
          FROM workflow_c_sampling_suites
         WHERE payload ? 'question_set_item_ids'
    ) THEN
        RAISE EXCEPTION 'cannot downgrade Sampling question selection after pilot Suites exist'
            USING ERRCODE = '55000';
    END IF;
END;
$$;
