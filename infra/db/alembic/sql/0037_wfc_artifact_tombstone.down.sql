-- Restore the 0031 function body when rolling back the tombstone completion
-- compatibility fix.  The predecessor intentionally retains its original
-- unqualified predicate for an exact schema rollback.
...
BEGIN
    SELECT * INTO STRICT queued FROM workflow_c_artifact_deletion_queue
    ...
    IF queued.status IN ('completed', 'retry_wait')
       ...
       AND ((queued.status = 'completed' AND p_retry_not_before IS NULL)
            OR (queued.status = 'retry_wait'
                AND queued.next_attempt_at IS NOT DISTINCT FROM p_retry_not_before)) THEN
