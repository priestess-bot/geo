...
BEGIN
    IF EXISTS (
        ...
        WHERE platform = 'kimi' OR surface = 'kimi_api'
    ) OR EXISTS (
        SELECT 1
        FROM monitoring_protocols AS protocol,
             LATERAL jsonb_array_elements(protocol.source_strata_snapshot) AS source(item)
        WHERE source.item ->> 'platform' = 'kimi'
           OR source.item ->> 'surface' = 'kimi_api'
    ) THEN
        RAISE EXCEPTION 'cannot remove Kimi source identity while governed lineage exists';
    END IF;
END
$migration$;
...
BEGIN
    FOREACH function_name IN ARRAY ARRAY[
        ...
        IF strpos(function_source, '''microsoft'', ''kimi'', ''anthropic''') = 0 THEN
