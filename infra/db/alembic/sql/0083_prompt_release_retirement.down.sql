...
BEGIN
    IF EXISTS (
        SELECT 1 FROM prompt_program_release_states WHERE status = 'retired'
    ) OR EXISTS (
        SELECT 1 FROM prompt_program_command_receipts WHERE operation = 'retire'
    ) THEN
        RAISE EXCEPTION 'cannot downgrade: Prompt Release retirement evidence exists'
            USING ERRCODE = '55000';
    END IF;
END;
$$;
...
CHECK (status IN ('draft', 'tested', 'approved', 'frozen'));
