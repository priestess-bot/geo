...
CREATE FUNCTION geno_v2_create_geo_campaign(
...
BEGIN
    SELECT * INTO context_row FROM public.geno_v2_resolve_session_context();
...
CREATE FUNCTION geno_v2_create_project_destination(
...
CREATE FUNCTION geno_v2_qualify_project_destination(p_destination_id uuid, p_project_id uuid)
RETURNS project_destinations
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog AS $$
...
CREATE FUNCTION geno_v2_create_placement_opportunity(
...
CREATE FUNCTION geno_v2_read_geo_campaigns(p_project_id uuid)
...
LANGUAGE sql SECURITY DEFINER SET search_path = pg_catalog AS $$
    ...
    WHERE campaign.project_id = p_project_id
      AND EXISTS (
        SELECT 1 FROM public.geno_v2_resolve_session_context() AS context_row
        WHERE public.geno_v2_session_has_project_permission(
...
ALTER FUNCTION geno_v2_create_geo_campaign(uuid, uuid, text, text) OWNER TO geno_v2_authz_owner;
ALTER FUNCTION geno_v2_create_project_destination(uuid, uuid, text, text, text, text, jsonb) OWNER TO geno_v2_authz_owner;
ALTER FUNCTION geno_v2_qualify_project_destination(uuid, uuid) OWNER TO geno_v2_authz_owner;
ALTER FUNCTION geno_v2_create_placement_opportunity(uuid, uuid, uuid, uuid, text, text, text) OWNER TO geno_v2_authz_owner;
ALTER FUNCTION geno_v2_read_geo_campaigns(uuid) OWNER TO geno_v2_authz_owner;
REVOKE ALL ON FUNCTION geno_v2_create_geo_campaign(uuid, uuid, text, text),
    geno_v2_create_project_destination(uuid, uuid, text, text, text, text, jsonb),
    geno_v2_qualify_project_destination(uuid, uuid),
    geno_v2_create_placement_opportunity(uuid, uuid, uuid, uuid, text, text, text),
    geno_v2_read_geo_campaigns(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION geno_v2_create_geo_campaign(uuid, uuid, text, text),
    geno_v2_create_project_destination(uuid, uuid, text, text, text, text, jsonb),
    geno_v2_qualify_project_destination(uuid, uuid),
    geno_v2_create_placement_opportunity(uuid, uuid, uuid, uuid, text, text, text),
    geno_v2_read_geo_campaigns(uuid) TO geno_v2_runtime;
