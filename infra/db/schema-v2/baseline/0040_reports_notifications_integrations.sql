...
CREATE OR REPLACE FUNCTION geno_v2_enqueue_durable_job_dispatch() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog AS $$
...
CREATE TRIGGER report_jobs_enqueue_dispatch AFTER INSERT ON report_generation_jobs FOR EACH ROW EXECUTE FUNCTION geno_v2_enqueue_durable_job_dispatch();

CREATE FUNCTION geno_v2_claim_report_generation_job(
...
END; $$;
CREATE FUNCTION geno_v2_heartbeat_report_generation_job(p_job_id uuid,p_worker_id text,p_lease_token uuid,p_lease_seconds integer)
...
CREATE FUNCTION geno_v2_complete_report_generation_job(p_job_id uuid,p_worker_id text,p_lease_token uuid,p_report_version_id uuid)
RETURNS report_generation_jobs LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog AS $$
DECLARE r public.report_generation_jobs%ROWTYPE; BEGIN
...
CREATE FUNCTION geno_v2_fail_report_generation_job(p_job_id uuid,p_worker_id text,p_lease_token uuid,p_error_code text,p_error_message text,p_retryable boolean,p_retry_delay_seconds integer DEFAULT 0)
RETURNS report_generation_jobs LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog AS $$
DECLARE r public.report_generation_jobs%ROWTYPE; BEGIN
 ...
 IF NOT FOUND THEN RAISE EXCEPTION 'report job lease is lost' USING ERRCODE='55000'; END IF; RETURN r; END; $$;
CREATE FUNCTION geno_v2_create_report_generation_job(
...
DECLARE r public.report_generation_jobs%ROWTYPE; h text; next_version integer; BEGIN
 IF NOT public.geno_v2_session_has_project_permission(p_project_id,p_tenant_id,'report.generate') THEN
...
CREATE FUNCTION geno_v2_persist_report_generation_result(
...
LANGUAGE sql SECURITY DEFINER SET search_path=pg_catalog AS $$
 ...
 WHERE s.project_id=p_project_id AND s.enabled
   AND EXISTS (SELECT 1 FROM public.geno_v2_resolve_session_context() c
...
CREATE FUNCTION geno_v2_reject_report_immutable_update() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog AS $$ BEGIN RAISE EXCEPTION 'report traceability rows are immutable' USING ERRCODE='55000'; END; $$;
CREATE FUNCTION geno_v2_require_finalized_report_artifact()
...
BEGIN
 ...
 RETURN NEW;
END; $$;
DO $$ DECLARE t text; BEGIN FOREACH t IN ARRAY ARRAY['report_version_score_snapshots','report_version_retests','report_version_facts','report_version_actions','report_exports'] LOOP EXECUTE format('CREATE TRIGGER %I BEFORE UPDATE OR DELETE ON public.%I FOR EACH ROW EXECUTE FUNCTION public.geno_v2_reject_report_immutable_update()',t||'_immutable',t);END LOOP; FOREACH t IN ARRAY ARRAY['reports','report_versions','report_exports','report_version_score_snapshots','report_version_retests','report_version_facts','report_version_actions','report_generation_jobs','integration_connectors','notification_recipients','notifications','notification_deliveries','portal_project_settings','portal_report_visibility'] LOOP EXECUTE format('ALTER TABLE public.%I ENABLE ROW LEVEL SECURITY',t);EXECUTE format('ALTER TABLE public.%I FORCE ROW LEVEL SECURITY',t);END LOOP; END; $$;
CREATE TRIGGER report_exports_finalized_artifact_guard BEFORE INSERT ON report_exports
FOR EACH ROW EXECUTE FUNCTION geno_v2_require_finalized_report_artifact();

ALTER FUNCTION geno_v2_enqueue_durable_job_dispatch() OWNER TO geno_v2_job_command_owner; ALTER FUNCTION geno_v2_create_report_generation_job(uuid,uuid,uuid,text,jsonb,text) OWNER TO geno_v2_job_command_owner; ALTER FUNCTION geno_v2_claim_report_generation_job(text,integer,uuid) OWNER TO geno_v2_job_owner; ALTER FUNCTION geno_v2_heartbeat_report_generation_job(uuid,text,uuid,integer) OWNER TO geno_v2_job_owner; ALTER FUNCTION geno_v2_complete_report_generation_job(uuid,text,uuid,uuid) OWNER TO geno_v2_job_owner; ALTER FUNCTION geno_v2_fail_report_generation_job(uuid,text,uuid,text,text,boolean,integer) OWNER TO geno_v2_job_owner; ALTER FUNCTION geno_v2_persist_report_generation_result(uuid,text,uuid,text,text,text,text,text) OWNER TO geno_v2_result_owner; ALTER FUNCTION geno_v2_claim_notification_delivery(text,integer,uuid) OWNER TO geno_v2_job_owner; ALTER FUNCTION geno_v2_heartbeat_notification_delivery(uuid,text,uuid,integer) OWNER TO geno_v2_job_owner; ALTER FUNCTION geno_v2_complete_notification_delivery(uuid,text,uuid,text) OWNER TO geno_v2_job_owner; ALTER FUNCTION geno_v2_fail_notification_delivery(uuid,text,uuid,text,text,boolean,integer) OWNER TO geno_v2_job_owner; ALTER FUNCTION geno_v2_read_portal_reports(uuid) OWNER TO geno_v2_authz_owner; ALTER FUNCTION geno_v2_reject_report_immutable_update() OWNER TO geno_v2_result_owner; ALTER FUNCTION geno_v2_require_finalized_report_artifact() OWNER TO geno_v2_result_owner;
REVOKE ALL ON reports,report_versions,report_exports,report_version_score_snapshots,report_version_retests,report_version_facts,report_version_actions,report_generation_jobs,integration_connectors,notification_recipients,notifications,notification_deliveries,portal_project_settings,portal_report_visibility FROM PUBLIC,geno_v2_runtime,geno_v2_worker;
REVOKE ALL ON FUNCTION geno_v2_create_report_generation_job(uuid,uuid,uuid,text,jsonb,text),geno_v2_claim_report_generation_job(text,integer,uuid),geno_v2_heartbeat_report_generation_job(uuid,text,uuid,integer),geno_v2_complete_report_generation_job(uuid,text,uuid,uuid),geno_v2_fail_report_generation_job(uuid,text,uuid,text,text,boolean,integer),geno_v2_persist_report_generation_result(uuid,text,uuid,text,text,text,text,text),geno_v2_claim_notification_delivery(text,integer,uuid),geno_v2_heartbeat_notification_delivery(uuid,text,uuid,integer),geno_v2_complete_notification_delivery(uuid,text,uuid,text),geno_v2_fail_notification_delivery(uuid,text,uuid,text,text,boolean,integer),geno_v2_read_portal_reports(uuid),geno_v2_reject_report_immutable_update(),geno_v2_require_finalized_report_artifact() FROM PUBLIC;
GRANT SELECT ON reports,report_versions,report_generation_jobs TO geno_v2_job_command_owner; GRANT INSERT,UPDATE ON report_generation_jobs,reports TO geno_v2_job_command_owner; GRANT SELECT,UPDATE ON report_generation_jobs,notification_deliveries TO geno_v2_job_owner; GRANT SELECT ON report_versions TO geno_v2_job_owner; GRANT SELECT,INSERT ON report_versions TO geno_v2_result_owner; GRANT SELECT,UPDATE ON report_generation_jobs TO geno_v2_result_owner; GRANT EXECUTE ON FUNCTION geno_v2_create_report_generation_job(uuid,uuid,uuid,text,jsonb,text),geno_v2_read_portal_reports(uuid) TO geno_v2_runtime; GRANT EXECUTE ON FUNCTION geno_v2_claim_report_generation_job(text,integer,uuid),geno_v2_heartbeat_report_generation_job(uuid,text,uuid,integer),geno_v2_complete_report_generation_job(uuid,text,uuid,uuid),geno_v2_fail_report_generation_job(uuid,text,uuid,text,text,boolean,integer),geno_v2_persist_report_generation_result(uuid,text,uuid,text,text,text,text,text),geno_v2_claim_notification_delivery(text,integer,uuid),geno_v2_heartbeat_notification_delivery(uuid,text,uuid,integer),geno_v2_complete_notification_delivery(uuid,text,uuid,text),geno_v2_fail_notification_delivery(uuid,text,uuid,text,text,boolean,integer) TO geno_v2_worker; GRANT SELECT ON reports,report_versions,report_exports,portal_project_settings,portal_report_visibility TO geno_v2_authz_owner;
